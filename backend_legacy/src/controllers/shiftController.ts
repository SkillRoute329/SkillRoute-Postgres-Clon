...
export const updateShiftStatus = async (req: Request, res: Response) => {
    ...
    try {
        ...
        // 3. Completed (Daily Accrual - UCOT 2026 Engine)
        if (status === 'Completed' && currentShift.status !== 'Completed') {
            try {
                const { PayrollService } = await import('../services/payrollService');
                const financials = await PayrollService.accrueSalary(updatedShift.id);
                console.log(`💰 Salario devengado para turno #${updatedShift.id}: $${financials?.netSalary}`);
            } catch (payError) {
                console.error('Payroll Accrual Error:', payError);
                // Non-blocking but logged fatal for Ops
            }
        }
