  ...
  try {
    ...
    const addTenantId = async (tableName) => {
      ...
      const res = await client.query(
        `SELECT COUNT(*) FROM "${tableName}" WHERE "tenantId" IS NULL`,
      );
      ...
      try {
        ...
        console.log(
          `Notice: Could not set NOT NULL/DEFAULT on ${tableName} (might already be set). continuing...`,
        );
      }
    ...
    try {
      ...
      await client.query(
        `ALTER TABLE "SystemConfig" ADD CONSTRAINT "SystemConfig_pkey" PRIMARY KEY ("tenantId", "key");`,
      );
    } catch (e) {
      console.error('Error on SystemConfig:', e.message);
    }
    ...
    try {
      ...
      console.error('Error on User constraints:', e.message);
    }
    ...
    try {
      ...
      console.error('Error on Category constraints:', e.message);
    }
