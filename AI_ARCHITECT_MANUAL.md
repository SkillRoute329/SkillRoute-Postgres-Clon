...
- **Arquitectura Híbrida (JSONB):**
  ...
  - _Incorrecto:_ `User.create({ ..., classification: 'Admin' })` (Rompe el build si la columna no existe).
  - _Correcto:_ `User.create({ ..., metadata: { classification: 'Admin' } })` (Funciona siempre).
