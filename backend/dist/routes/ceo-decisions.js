 ...
 */
router.get('/decision-status', (req, res) => {
    const orchestrator = req.app.locals?.masterOrchestrator;
    if (!orchestrator) {
 ...
 */
router.post('/execute-decision', async (req, res) => {
    const orchestrator = req.app.locals?.masterOrchestrator;
    if (!orchestrator) {
        return res.status(503).json({
            ok: false,
 ...
 */
router.get('/impacto-financiero', (req, res) => {
    const proyecciones = {
        mes_1: {
