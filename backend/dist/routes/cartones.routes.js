...
const comparacionServicioService_1 = require("../services/comparacionServicioService");
...
router.post('/bulk', async (req, res) => {
    try {
        const cartones = (req.body?.cartones ?? []);
        ...
        for (const c of cartones) {
            ...
            try {
                const existing = await (0, database_1.default)('cartones_completados').where('id', c.id).first();
                const row = {
                    id: c.id,
                ...
                if (existing) {
                    await (0, database_1.default)('cartones_completados').where('id', c.id).update(row);
                    updated++;
                }
                else {
                    await (0, database_1.default)('cartones_completados').insert(row);
                    inserted++;
                }
            }
            catch (err) {
        ...
        logger_1.default.error('[cartones/bulk]', { error: String(err) });
...
router.get('/oficiales', async (req, res) => {
    try {
        ...
        cartones.sort((a, b) => String(a.linea).localeCompare(String(b.linea), undefined, { numeric: true }) ||
            String(a.id).localeCompare(String(b.id), undefined, { numeric: true }));
        res.json({
            ok: true,
            total: cartones.length,
            cartones: cartones.slice(0, limit),
            meta: (0, cartonOficial_1.cartonMeta)(),
        });
    }
    catch (err) {
        logger_1.default.error('[cartones/oficiales]', { error: String(err) });
        res.status(500).json({ ok: false, error: 'Error leyendo cartón oficial', cartones: [] });
    }
});
router.get('/oficiales/:id', (req, res) => {
    try {
        ...
        logger_1.default.error('[cartones/oficiales/:id]', { error: String(err) });
...
router.get('/count', async (req, res) => {
    try {
        ...
        const q = (0, database_1.default)('cartones_completados')
            .select('agency_id', 'line', database_1.default.raw('COUNT(*) AS cnt'))
        ...
        logger_1.default.error('[cartones/count]', { error: String(err) });
 ...
 */
router.get('/triangulacion', async (req, res) => {
    try {
        const linea = req.query.linea || '';
        const sentido = (req.query.sentido || 'IDA').toUpperCase();
        const fecha = req.query.fecha || new Date().toISOString().slice(0, 10);
        const serviceNumber = req.query.service_number;
        const agencyId = req.query.agency_id || '70';
        if (!linea) {
        ...
        let carton = null;
        if (serviceNumber) {
            const cartonRow = await (0, database_1.default)('cartones_completados')
        ...
        const tripQ = await database_1.default.raw(`
      ...
      LIMIT 100
    `, [linea, dirId]);
        const tripStops = tripQ.rows.filter((r) => r.stop_name);
        ...
        const eventsQ = await (0, database_1.default)('vehicle_events')
            .where('agency_id', agencyId)
            .where('linea', linea)
        ...
        logger_1.default.error('[cartones/triangulacion]', { error: String(err) });
 ...
 */
router.get('/coche/:idBus', async (req, res) => {
    try {
        const idBus = req.params.idBus;
        const fecha = req.query.fecha || new Date().toISOString().slice(0, 10);
        const agencyId = req.query.agency_id || '70';
        // 1. Cartón asignado al coche para la fecha
        const carton = await (0, database_1.default)('cartones_completados')
            .where('agency_id', agencyId)
        ...
        const eventos = await (0, database_1.default)('vehicle_events')
            .where('id_bus', idBus)
            .where('agency_id', agencyId)
            .whereRaw(`DATE(created_at) = ?`, [fecha])
            .orderBy('created_at', 'asc')
        ...
        for (const ev of eventos) {
            ...
            r.eventos++;
            if (ev.estado_cumplimiento === 'EN_TIEMPO')
                r.en_tiempo++;
            else if (ev.estado_cumplimiento === 'ATRASADO')
                r.atrasado++;
            else if (ev.estado_cumplimiento === 'ADELANTADO')
                r.adelantado++;
            else
                r.sin_horario++;
        ...
        Object.values(resumenPorLinea).forEach((r) => {
        ...
        logger_1.default.error('[cartones/coche]', { error: String(err) });
 ...
 */
router.get('/ajustes-sugeridos/:linea', async (req, res) => {
    try {
        ...
        const stats = await (0, database_1.default)('vehicle_events')
            .where('agency_id', agencyId)
            .where('linea', linea)
            .where('created_at', '>', database_1.default.raw(`NOW() - INTERVAL '${dias} days'`))
            .whereIn('estado_cumplimiento', ['EN_TIEMPO', 'ATRASADO', 'ADELANTADO'])
            .whereNotNull('proxima_parada')
            .select('proxima_parada', database_1.default.raw('COUNT(*) AS muestras'), database_1.default.raw('AVG(desviacion_min) AS desv_media'), database_1.default.raw('STDDEV(desviacion_min) AS desv_std'), database_1.default.raw("COUNT(*) FILTER (WHERE estado_cumplimiento = 'ATRASADO') AS atrasados"), database_1.default.raw("COUNT(*) FILTER (WHERE estado_cumplimiento = 'ADELANTADO') AS adelantados"), database_1.default.raw("COUNT(*) FILTER (WHERE estado_cumplimiento = 'EN_TIEMPO') AS en_tiempo"))
            .groupBy('proxima_parada')
            .havingRaw('COUNT(*) >= 20') // mínimo 20 muestras para significancia
            .orderByRaw('ABS(AVG(desviacion_min)) DESC')
            .limit(50);
        const paradas = stats.map((s) => {
        ...
        logger_1.default.error('[cartones/ajustes-sugeridos]', { error: String(err) });
 ...
 */
router.get('/comparativa-etapas/:idBus', async (req, res) => {
    try {
        const idBus = req.params.idBus;
        const fecha = req.query.fecha || new Date().toISOString().slice(0, 10);
        const agencyId = req.query.agency_id || '70';
        const tolerancia = Math.max(1, parseInt(req.query.tolerancia || '4', 10));
        ...
        const cartonData = (cartonRow.data_jsonb ?? {});
        const parsed = (cartonData.parsed ?? {});
        const paradas = parsed.paradas ?? [];
        const viajes = parsed.viajes ?? [];
        // 2. Eventos GPS del coche en esa fecha
        const eventos = await (0, database_1.default)('vehicle_events')
            .where('id_bus', idBus)
            .where('agency_id', agencyId)
        ...
        const viajesComparados = viajes.map((viaje, vIdx) => {
            const tiempos = viaje.filter((c) => typeof c === 'string' && hhmmRe.test(c));
            ...
            const etapas = tiempos.map((tCarton, eIdx) => {
                ...
                for (const e of eventos) {
                    ...
                    if (!matchParada && diff > 15)
                        continue; // sin match parada, ventana más chica
                    if (diff < mejorDiff) {
                        mejorDiff = diff;
                        mejorEvento = e;
                    }
        ...
        logger_1.default.error('[cartones/comparativa-etapas]', { error: String(err) });
 ...
 */
router.get('/coches-en-servicio-hoy', async (req, res) => {
    try {
        const agency = String(req.query.agency || '70');
        ...
        const rows = await (0, database_1.default)('bus_last_pos as b')
            .leftJoin('cartones_completados as c', function () {
            this.on('c.agency_id', '=', 'b.agency_id')
                .andOn(database_1.default.raw("c.vehiculo_id = regexp_replace(b.id_bus, '^[0-9]+_', '')"))
                .andOn(database_1.default.raw("COALESCE((c.data_jsonb ->> 'timestamp')::timestamptz::date, c.updated_at::date) = b.updated_at::date"));
        })
            .where('b.agency_id', agency)
            .where('b.updated_at', '>', database_1.default.raw("NOW() - INTERVAL '5 minutes'"))
            .select(database_1.default.raw("regexp_replace(b.id_bus, '^[0-9]+_', '') AS coche"), 'b.linea AS imm_linea', 'b.velocidad', 'b.estado_cumplimiento', 'b.lat', 'b.lon', 'b.updated_at', 'c.service_number', 'c.line AS ucot_linea', 'c.conductor_id');
        const items = rows.map((r) => {
            let estadoCruce;
            if (!r.ucot_linea)
                estadoCruce = 'sin_carton';
            else if (r.ucot_linea === r.imm_linea)
                estadoCruce = 'match_directo';
            else if ('3' + r.imm_linea === r.ucot_linea)
                estadoCruce = 'match_prefijo_ucot';
            else
                estadoCruce = 'no_match';
            return {
                coche: r.coche,
        ...
        const resumen = items.reduce((acc, it) => {
            ...
            acc[it.estadoCruce]++;
            return acc;
        }, { total: 0, match_directo: 0, match_prefijo_ucot: 0, no_match: 0, sin_carton: 0 });
        const payload = {
            ok: true,
            agency,
            ...
            items: items.sort((a, b) => {
                const order = { no_match: 0, sin_carton: 1, match_prefijo_ucot: 2, match_directo: 3 };
                return (order[a.estadoCruce] - order[b.estadoCruce]) || Number(a.coche) - Number(b.coche);
            }),
        };
        cm.cacheSet(cacheKey, payload, 15000);
        res.json(payload);
    }
    catch (err) {
        logger_1.default.error('[cartones/coches-en-servicio-hoy]', { error: String(err) });
 ...
 */
router.get('/comparacion/:coche', async (req, res) => {
    try {
        ...
        const out = await (0, comparacionServicioService_1.compararServicioCoche)(coche, fecha, agencyId);
        res.json(out);
    }
    catch (err) {
        logger_1.default.error('[cartones/comparacion]', { error: String(err) });
 ...
 *   OK
 */
router.get('/panel-cumplimiento', async (req, res) => {
    try {
        const agencyId = req.query.agency_id || '70';
        const fecha = req.query.fecha || new Date().toISOString().slice(0, 10);
        // GPS agregado del día por coche (MV rápida).
        const gps = (await (0, database_1.default)('mv_fleet_ranking_diario')
            .where('agency_id', agencyId)
            .whereRaw('fecha = ?::date', [fecha])
            .select('id_bus', 'total', 'en_tiempo', 'atrasado', 'adelantado', 'sin_horario', 'desv_media_sum', 'lineas', 'ultima'));
        // Cartón asignado del día por coche (rotación scrapeada).
        const cartones = (await (0, database_1.default)('cartones_completados')
            .where('agency_id', agencyId)
            .whereRaw("COALESCE((data_jsonb ->> 'timestamp')::timestamptz::date, updated_at::date) = ?::date", [fecha])
        ...
        const tipoDia = (0, comparacionServicioService_1.tipoDiaDeFecha)(fecha);
        const cartonPorCoche = new Map();
        for (const c of cartones) {
            const servicio = c.service_number && /^\d+$/.test(String(c.service_number))
            ...
            cartonPorCoche.set(String(c.vehiculo_id), {
                servicio,
                linea: lineaScrape ?? (0, comparacionServicioService_1.lineaDeServicio)(servicio, tipoDia),
            });
        }
        // Limpia el array de líneas observadas por GPS (IMM): saca vacíos,
        // '0', '?', duplicados y ordena — evita el "hasta 3 líneas erráticas".
        const limpiarLineas = (arr) => {
            if (!Array.isArray(arr))
                return [];
            const set = new Set();
            for (const v of arr) {
                const s = String(v ?? '').trim();
                if (s && s !== '0' && s !== '?' && s.toLowerCase() !== 'null')
                    set.add(s);
            }
            return [...set].sort();
        };
        const gpsPorCoche = new Map();
        for (const g of gps)
            gpsPorCoche.set(String(g.id_bus), g);
        const SEV = {
            NO_SALIO: 5,
            ATRASADO: 4,
            BAJA_COBERTURA: 3,
        ...
        const filas = [];
        const coches = new Set([...cartonPorCoche.keys(), ...gpsPorCoche.keys()]);
        for (const coche of coches) {
            ...
            if (c && (!g || total < 30))
                ...
                estado = 'SIN_CARTON';
            else if (desvMedio != null && desvMedio > 4)
                estado = 'ATRASADO';
            else if (desvMedio != null && desvMedio < -4)
                estado = 'ADELANTADO';
            else if (total > 0 && coberturaPct < 40)
                estado = 'BAJA_COBERTURA';
            if (estado === 'OK')
                continue; // el panel SOLO lista problemas
        ...
        logger_1.default.error('[cartones/panel-cumplimiento]', { error: String(err) });
 ...
 */
router.post('/historial/snapshot', async (req, res) => {
    try {
        const agencyId = req.query.agency_id || '70';
        const n = await (0, cartonesHistorialService_1.snapshotHistorial)(agencyId);
        res.json({ ok: true, insertadas: n });
    }
    catch (err) {
        logger_1.default.error('[cartones/historial/snapshot]', { error: String(err) });
        res.status(500).json({ ok: false, error: String(err) });
    }
});
/**
 ...
 */
router.get('/distribucion/:coche', async (req, res) => {
    try {
        const coche = String(req.params.coche).trim();
        const agencyId = req.query.agency_id || '70';
        const dist = await database_1.default.raw(`WITH h AS (
       ...
       SELECT h.service_number AS servicio,
        ...
        ORDER BY veces DESC, servicio`, [agencyId, coche, agencyId, coche, agencyId, coche]);
        const filas = (dist.rows ?? dist);
        res.json({
            ok: true,
            coche,
            agencyId,
        ...
        logger_1.default.error('[cartones/distribucion]', { error: String(err) });
        res.status(500).json({ ok: false, error: String(err) });
    }
});
/**
 ...
 */
router.get('/sustituciones', async (req, res) => {
    try {
        const agencyId = req.query.agency_id || '70';
        const fecha = req.query.fecha || new Date().toISOString().slice(0, 10);
        // Esperados: del historial del día (fallback: cartones_completados).
        let esperados = (await (0, database_1.default)('cartones_historial')
            .where({ agency_id: agencyId, fecha })
            .whereNotNull('service_number')
            .select('vehiculo_id', 'service_number', 'line'));
        if (esperados.length === 0) {
            esperados = (await (0, database_1.default)('cartones_completados')
                .where('agency_id', agencyId)
                .whereRaw("COALESCE((data_jsonb ->> 'timestamp')::timestamptz::date, updated_at::date) = ?::date", [fecha])
                .whereNotNull('vehiculo_id')
                .select('vehiculo_id', 'service_number', { line: 'line' }));
        }
        // Operación real del día: coche → líneas y nº de puntos GPS.
        const ops = (await (0, database_1.default)('vehicle_events')
            .where('agency_id', agencyId)
            .whereRaw("timestamp_gps >= ? AND timestamp_gps < (?::date + interval '1 day')", [fecha, fecha])
            .select('id_bus', 'linea')
            .count('* as pts')
            .groupBy('id_bus', 'linea'));
        const operoPorCoche = new Map();
        for (const o of ops) {
            const coche = String(o.id_bus).replace(new RegExp(`^${agencyId}_`), '');
            const e = operoPorCoche.get(coche) ?? { lineas: new Set(), pts: 0 };
            if (o.linea)
                e.lineas.add(String(o.linea));
            e.pts += Number(o.pts);
            operoPorCoche.set(coche, e);
        }
        const UMBRAL_PTS = 50; // actividad mínima para considerar "salió"
        const salio = (coche) => (operoPorCoche.get(coche)?.pts ?? 0) >= UMBRAL_PTS;
        const noSalieron = [];
        for (const e of esperados) {
            ...
            const esperadosEnLinea = new Set(esperados.filter((x) => (0, lineaUcot_1.mismaLinea)(String(x.line), linea)).map((x) => String(x.vehiculo_id)));
            const sustitutos = [];
            if (linea) {
                for (const [c, info] of operoPorCoche) {
                    const operoEsaLinea = [...info.lineas].some((l) => (0, lineaUcot_1.mismaLinea)(l, linea));
        ...
        logger_1.default.error('[cartones/sustituciones]', { error: String(err) });
        res.status(500).json({ ok: false, error: String(err) });
    }
});
