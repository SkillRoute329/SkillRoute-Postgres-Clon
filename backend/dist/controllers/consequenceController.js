...
exports.computeConsequencesForEvent = computeConsequencesForEvent;
exports.postConsequencePreview = postConsequencePreview;
...
let TARIFA_HORA_UYU = motorConfigService_1.DEFAULTS.tarifaHoraUyu;
let SUBSIDIO_POR_KM_UYU = motorConfigService_1.DEFAULTS.subsidioPorKmUyu;
let COSTO_RESERVA_EXTRA_UYU = motorConfigService_1.DEFAULTS.costoReservaExtraUyu;
function leerEvento(body) {
    const ev = body?.evento;
    return (ev && typeof ev === 'object') ? ev : {};
}
function severidadCombinada(efectos) {
    if (efectos.some((e) => e.severidad === 'critico'))
        return 'critico';
    if (efectos.some((e) => e.severidad === 'advertencia'))
        return 'advertencia';
    return 'info';
}
// ─── Reglas por tipo de evento ────────────────────────────────────────────
function reglasConductorAusente(ev) {
...
function reglasVehiculoFueraServicio(ev) {
...
function reglasRetrasoOperativo(ev) {
    const conductorId = String(ev.conductorId ?? 'conductor-?');
    const lineaId = String(ev.lineaId ?? '');
...
function reglasViajeCancelado(ev) {
    const lineaId = String(ev.lineaId ?? '');
    const kmEsperados = Number(ev.kmEsperados ?? 0);
...
function resumirEfectos(efectos) {
    const sumByDominio = (d) => efectos.filter((e) => e.dominio === d && typeof e.delta === 'number').reduce((s, e) => s + (e.delta ?? 0), 0);
    return {
...
async function computeConsequencesForEvent(ev) {
    ...
    switch (tipo) {
        case 'CONDUCTOR_AUSENTE':
            efectos = reglasConductorAusente(ev);
            break;
        case 'VEHICULO_FUERA_DE_SERVICIO':
            efectos = reglasVehiculoFueraServicio(ev);
            break;
        case 'RETRASO_OPERATIVO':
            efectos = reglasRetrasoOperativo(ev);
            break;
        case 'VIAJE_CANCELADO':
            efectos = reglasViajeCancelado(ev);
            break;
        default:
    ...
    let logId = null;
    try {
        const [inserted] = await (0, database_1.default)('logs_auditoria')
            .insert({
    ...
    const lineaIdEv = String(ev.lineaId ?? ev.linea ?? '');
    const agencyIdEv = String(ev.agencyId ?? ev.empresaId ?? '');
    if (resumen.severidadGlobal === 'critico' && lineaIdEv) {
        (0, socketBus_1.busEmit)('bus:driver:linea-critica', {
            lineaId: lineaIdEv,
            agencyId: agencyIdEv,
            tipo,
            severidad: resumen.severidadGlobal,
            causa: String(ev.causa ?? ev.causaViaje ?? ev.motivoVehiculo ?? tipo),
            feedId: logId,
        });
    }
    return { ok: true, tipo, efectos, resumen };
}
async function postConsequencePreview(req, res) {
    try {
        ...
        logger_1.default.error('[consequencePreview]', { error: String(err) });
