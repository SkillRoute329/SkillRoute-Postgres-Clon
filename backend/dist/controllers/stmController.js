...
exports.stmController = void 0;
const stmService_1 = require("../services/stmService");
...
class STMController {
    ...
    async getLineas(req, res) {
        try {
            const lineas = await stmService_1.stmService.obtenerLineasSTM();
            res.json({
                success: true,
                data: {
                    total: lineas.length,
            ...
            logger_1.logger.error(`Error obteniendo líneas STM: ${error}`);
            res.status(500).json({
    ...
    async getHorarios(req, res) {
        try {
            ...
            const horarios = await stmService_1.stmService.obtenerHorariosLinea(parseInt(numeroLinea));
            res.json({
                success: true,
                data: horarios
            });
            ...
            logger_1.logger.error(`Error obteniendo horarios: ${error}`);
            res.status(500).json({
     ...
     */
    async sincronizarHorarios(req, res) {
        try {
            ...
            const resultado = await stmService_1.stmService.sincronizarHorarios();
            res.json({
                success: true,
                data: {
                    id: resultado.id,
            ...
            logger_1.logger.error(`Error sincronizando horarios: ${error}`);
            res.status(500).json({
     ...
     */
    async detectarCambios(req, res) {
        try {
            const { numeroLinea } = req.params;
            const cambios = await stmService_1.stmService.detectarCambiosHorarios(parseInt(numeroLinea));
            res.json({
                success: true,
                data: {
            ...
            logger_1.logger.error(`Error detectando cambios: ${error}`);
            res.status(500).json({
     ...
     */
    async registrarBoletaje5G(req, res) {
        try {
            ...
            await stmService_1.stmService.registrarBoletaje5G(datos);
            res.json({
                success: true,
    ...
    async actualizarOcupacion(req, res) {
        try {
            ...
            await stmService_1.stmService.actualizarConteoPassajeros(datos);
            res.json({
                success: true,
            ...
            logger_1.logger.error(`Error actualizando ocupación: ${error}`);
            res.status(500).json({
     ...
     */
    async obtenerBusEnVivo(req, res) {
        try {
            const { busId } = req.params;
            const datos = await stmService_1.stmService.obtenerDatosEnVivoBus(busId);
            if (!datos) {
                res.status(404).json({
            ...
            logger_1.logger.error(`Error obteniendo datos en vivo: ${error}`);
            res.status(500).json({
    ...
    async obtenerEstadisticas(req, res) {
        try {
            ...
            const stats = await stmService_1.stmService.obtenerEstadisticasDiasBus(busId, new Date(fecha));
            if (!stats) {
                res.status(404).json({
     ...
     */
    async obtenerCalidadDatos(req, res) {
        try {
            const calidad = await stmService_1.stmService.calcularCalidadDatos();
            res.json({
                success: true,
                data: {
            ...
            logger_1.logger.error(`Error calculando calidad: ${error}`);
            res.status(500).json({
...
exports.stmController = new STMController();
