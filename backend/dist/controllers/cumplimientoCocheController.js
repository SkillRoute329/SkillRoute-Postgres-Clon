...
async function getCocheEventos(req, res) {
    try {
        ...
        let q = (0, database_1.default)('vehicle_events')
            .select('id', 'id_bus', 'linea', 'agency_id', 'estado_cumplimiento', 'desviacion_min', 'proxima_parada', 'sentido', 'destino', 'velocidad', 'timestamp_gps')
            .where('id_bus', idBus)
        ...
        const estadoCounter = new Map(); // linea → estado → count
        for (const ev of rows) {
            ...
            if (dv > agg.desviacionMaxMin)
                agg.desviacionMaxMin = dv;
            if (ev.proxima_parada)
                agg.ultimaParada = ev.proxima_parada;
            porLinea.set(linea, agg);
            const map = estadoCounter.get(linea) ?? new Map();
        ...
        logger_1.default.error('[cumplimiento/coche/eventos]', { error: String(err) });
