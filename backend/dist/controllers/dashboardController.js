...
class DashboardController {
    ...
    async getExecutiveDashboard(req, res) {
        try {
            ...
            const dashboard = await dashboardService_1.dashboardService.generarDashboardEjecutivo(operador);
            res.json({
                success: true,
                data: dashboard
            });
            ...
            logger_1.logger.error(`Error en getExecutiveDashboard: ${error}`);
            res.status(500).json({
    ...
    async getMetricas(req, res) {
        try {
            ...
            logger_1.logger.error(`Error en getMetricas: ${error}`);
            res.status(500).json({
    ...
    async getLineas(req, res) {
        try {
            ...
            logger_1.logger.error(`Error en getLineas: ${error}`);
            res.status(500).json({
    ...
    async getAlertas(req, res) {
        try {
            ...
            logger_1.logger.error(`Error en getAlertas: ${error}`);
            res.status(500).json({
    ...
    async getRecomendaciones(req, res) {
        try {
            ...
            const recomendacionesOrdenadas = dashboard.recomendaciones.sort((a, b) => {
                ...
                const scoreA = urgenciaScore[a.urgencia] * a.impacto;
                const scoreB = urgenciaScore[b.urgencia] * b.impacto;
            ...
            logger_1.logger.error(`Error en getRecomendaciones: ${error}`);
            res.status(500).json({
    ...
    async getSalud(req, res) {
        try {
            ...
            logger_1.logger.error(`Error en getSalud: ${error}`);
            res.status(500).json({
        ...
        try {
            ...
            res.json({
                ...
                data: {
                    ...
                    ingresos_promedio: Math.round(dashboard.proyecciones.reduce((sum, p) => sum + p.ingresosProyectados, 0) /
                        dashboard.proyecciones.length),
                    crecimiento_promedio: (dashboard.proyecciones.reduce((sum, p) => sum + p.cambioEsperado, 0) /
                        dashboard.proyecciones.length).toFixed(1)
                }
            });
        }
        catch (error) {
            logger_1.logger.error(`Error en getProyecciones: ${error}`);
            res.status(500).json({
    ...
    async getResumen(req, res) {
        try {
            ...
            logger_1.logger.error(`Error en getResumen: ${error}`);
            res.status(500).json({
