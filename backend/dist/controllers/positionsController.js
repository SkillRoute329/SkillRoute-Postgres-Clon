...
exports.getCutcsaPositions = getCutcsaPositions;
...
async function queryBuses(agencyFilter) {
    let q = (0, database_1.default)('bus_last_pos as b')
        .leftJoin('empresas as e', 'b.agency_id', 'e.agency_id')
        .select('b.id_bus', 'b.agency_id', 'b.linea', 'b.destino', 'b.sentido', 'b.lat', 'b.lon', 'b.velocidad', 'b.estado_cumplimiento', 'b.timestamp_gps', 'e.nombre as nombre_empresa')
        .whereNotNull('b.lat')
        .whereNotNull('b.lon')
        // Solo pings recientes (60 min) — descartamos buses muertos del feed.
        .where('b.timestamp_gps', '>=', database_1.default.raw("NOW() - INTERVAL '60 minutes'"));
    if (agencyFilter) {
 ...
 */
async function getAllPositions(_req, res) {
    try {
        ...
        logger_1.default.error('[positions] getAllPositions error', { error: msg });
 ...
 */
async function getCutcsaPositions(_req, res) {
    try {
        ...
        const msg = error instanceof Error ? error.message : String(error);
        logger_1.default.error('[positions] getCutcsaPositions error', { error: msg });
