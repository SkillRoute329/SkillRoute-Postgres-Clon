...
class AgentFactory {
    ...
    static async createLineEcosystem(lineConfig) {
        ...
        const ownAgents = lineConfig.destinos.map(dest => this.createOwnAnalyzer(lineId, dest, lineConfig.horarios_tipos));
        // 3. Crear sub-agentes de competencia
        const competitorAgents = lineConfig.competidores.map(comp => this.createCompetitorMonitor(lineId, comp, lineConfig.datos_publicos));
        return {
            lineId: lineId,
