...
const planningService_1 = require("./planningService");
...
async function testPlanningService() {
    ...
    const dist = planningService_1.planningService.haversineDistance(-34.9011, -56.1645, -34.8950, -56.1600);
    console.log(`   - Distancia calculada: ${dist.toFixed(4)} km`);
    assert_1.default.ok(dist > 0.5 && dist < 1.0, 'La distancia entre el centro y Tres Cruces debería estar entre 0.5 y 1.0 km');
    ...
    const isInsidePocitos = planningService_1.planningService.isPointInPolygon({ lat: -34.912, lng: -56.148 }, [
        [-34.9150, -56.1550],
        [-34.9050, -56.1500],
        [-34.9080, -56.1350],
        [-34.9180, -56.1420]
    ...
    assert_1.default.strictEqual(isInsidePocitos, true, 'El punto dentro del polígono de Pocitos debería retornar true');
    // Un punto claramente fuera de Pocitos: -34.850, -56.200
    const isOutsidePocitos = planningService_1.planningService.isPointInPolygon({ lat: -34.850, lng: -56.200 }, [
        [-34.9150, -56.1550],
        [-34.9050, -56.1500],
        [-34.9080, -56.1350],
        [-34.9180, -56.1420]
    ...
    assert_1.default.strictEqual(isOutsidePocitos, false, 'El punto fuera de Pocitos debería retornar false');
    ...
    const barrios = planningService_1.planningService.getCrossedBarrios(points, paradas);
    console.log(`   - Barrios cruzados detectados: ${barrios.map(b => b.nombre).join(', ')}`);
    assert_1.default.ok(barrios.some(b => b.nombre === 'Centro'), 'Debería detectar el barrio Centro');
    assert_1.default.ok(barrios.some(b => b.nombre === 'Tres Cruces'), 'Debería detectar el barrio Tres Cruces');
    // 4. Probar Análisis de Equidad (Latam Engine)
    console.log('4. Probando motor de análisis de equidad (Equity Analysis)...');
    const equityAnalysis = planningService_1.planningService.analyzeEquity(points, paradas, 80);
    ...
    assert_1.default.ok(typeof equityAnalysis.socialCoverageIndex === 'number', 'Social Coverage Index debería ser un número');
    assert_1.default.ok(equityAnalysis.equityScore >= 0 && equityAnalysis.equityScore <= 100, 'El Equity Score debería estar entre 0 y 100');
    // 5. Probar Impacto Financiero
    console.log('5. Probando simulación de impacto financiero...');
    const financial = planningService_1.planningService.calculateFinancialImpact(points, paradas, 40, 90, 56);
    ...
    assert_1.default.ok(financial.lengthKm > 0, 'La longitud de la ruta calculada debería ser mayor a cero');
    assert_1.default.ok(financial.monthlyCost > 0, 'El costo operativo mensual debería ser mayor a cero');
    assert_1.default.ok(financial.dailyPassengers > 0, 'Los pasajeros diarios estimados deberían ser mayores a cero');
    assert_1.default.ok(financial.roi !== undefined, 'El ROI debería calcularse');
