...
class ForecastService {
    ...
    async simuladorHorarios(lineaId, cambios) {
        try {
            ...
            if (multiplicadorImpacto < 0.95)
                riesgo = 'alto';
            else if (multiplicadorImpacto < 1.0)
                riesgo = 'medio';
            return {
     ...
     */
    async estimarPasajerosPorHorario(lineaId, horario) {
        try {
            const registros = await this.obtenerHistoricoBoletaje(lineaId, 30);
            ...
            if (registrosPorHora.length === 0) {
                ...
                return Math.round(registros.reduce((sum, r) => sum + r.boletosVendidos, 0) / registros.length);
            }
     ...
     */
    async calcularDemandaPorZona(zona) {
        try {
            // Obtener todas las líneas que operan en la zona
            const lineasSnapshot = await database_1.db.collection('lineas').where('zona', '==', zona).get();
     ...
     */
    async identificarHorariosAlta(lineaId) {
        try {
            const registros = await this.obtenerHistoricoBoletaje(lineaId, 30);
            // Agrupar por hora
     ...
     */
    async proyectarCrecimiento(lineaId, meses) {
        try {
    ...
    generarRecomendacionSimulacion(multiplicador, riesgo, cambios) {
        if (riesgo === 'alto') {
    ...
    generarRecomendacionesBenchmark(diferencia) {
