...
exports.gtfsService = {
    ...
    async getAllStops() {
        try {
            ...
            const stops = await (0, database_1.default)('gtfs.stops')
                .select('stop_id', 'stop_name', 'stop_lat', 'stop_lon', 'stop_code')
                .orderBy('stop_name', 'asc');
            return stops;
        }
        catch (err) {
            logger_1.logger.error('[GTFS Service] Error fetching stops', err);
            throw err;
        }
    },
    /**
    ...
    async getShape(shapeId) {
        try {
            const points = await (0, database_1.default)('gtfs.shapes')
            ...
            logger_1.logger.error(`[GTFS Service] Error fetching shape ${shapeId}`, err);
            throw err;
        }
    },
    /**
     ...
     */
    async getRouteDetails(routeShortName) {
        try {
            const route = await (0, database_1.default)('gtfs.routes')
                .where('route_short_name', routeShortName)
                .first();
            return route;
        }
        catch (err) {
            logger_1.logger.error(`[GTFS Service] Error fetching route ${routeShortName}`, err);
            throw err;
        }
    },
    /**
     ...
     */
    async getNextDepartures(stopId, limit = 10) {
        try {
            const now = new Date();
            // 1. Formatear hora actual compatible con HH:MM:SS del GTFS en timezone de Montevideo
            const timeOptions = { hour: '2-digit', minute: '2-digit', second: '2-digit', hour12: false, timeZone: 'America/Montevideo' };
            ...
            const dayOptions = { weekday: 'long', timeZone: 'America/Montevideo' };
            const currentDayName = new Intl.DateTimeFormat('en-US', dayOptions).format(now).toLowerCase();
            logger_1.logger.info(`[Oráculo] Consultando salidas para parada ${stopId} a las ${localTimeStr} (${currentDayName})`);
            const departures = await (0, database_1.default)('gtfs.stop_times as st')
                ...
                .select('st.arrival_time as arrivalTime', 'r.route_short_name as route', 'r.route_long_name as routeName', 't.trip_headsign as destination')
                .orderBy('st.arrival_time', 'asc')
                .limit(limit);
            return departures;
        }
        catch (err) {
            logger_1.logger.error(`[GTFS Service] Error calculating departures for ${stopId}`, err);
            throw err;
        }
    },
    /**
     ...
     */
    async getStats() {
        const stopsCount = await (0, database_1.default)('gtfs.stops').count('stop_id as total');
        const routesCount = await (0, database_1.default)('gtfs.routes').count('route_id as total');
        return {
     ...
     */
    async getCompetitiveOverlaps(routeShortName) {
        try {
            const rawQuery = `
        WITH TargetStops AS (
        ...
        LIMIT 15;
      `;
            const result = await database_1.default.raw(rawQuery, [routeShortName, routeShortName]);
            ...
            const data = (result.rows || result).map((r) => {
                ...
                if (!isNaN(routeNum)) {
                    if (routeNum < 100)
                        inferredCompany = 'CUTCSA / Varios';
                    else if (routeNum >= 100 && routeNum < 200)
                        inferredCompany = 'CUTCSA';
                    else if (routeNum >= 200 && routeNum < 300)
                        inferredCompany = 'COME';
                    else if (routeNum >= 300 && routeNum < 400)
                        inferredCompany = 'UCOT / COETC';
                    else if (routeNum >= 400 && routeNum < 500)
                        inferredCompany = 'COETC / UCOT';
                }
                ...
                const topZones = zonesArray.slice(0, 4).map((z) => z.toLowerCase().replace(/(^|\s)\S/g, (l) => l.toUpperCase()) // Capitalize nice
                );
                return {
                    ...r,
            ...
            logger_1.logger.error(`[GTFS Service] Error in getCompetitiveOverlaps`, err);
            throw err;
        }
    },
    /**
     ...
     */
    async getDailyTimetable(serviceType, agencyId) {
        try {
            let dayFilter = 'monday = 1';
            if (serviceType === 'SABADO')
                dayFilter = 'saturday = 1';
            if (serviceType === 'DOMINGO')
                dayFilter = 'sunday = 1';
            // Heurística de rangos de línea por empresa si el dataset GTFS es consolidado
            let routeCondition = '';
            if (agencyId === '70')
                routeCondition = "AND r.route_short_name ~ '^(3[0-9]{2}|L)'"; // UCOT (300s + Locales)
            else if (agencyId === '50')
                routeCondition = "AND r.route_short_name ~ '^(1[0-9]{2}|D10|G)'"; // CUTCSA
            else if (agencyId === '20')
                routeCondition = "AND r.route_short_name ~ '^2[0-9]{2}'"; // COME
            else if (agencyId === '60')
                routeCondition = "AND r.route_short_name ~ '^4[0-9]{2}'"; // COETC
            const query = `
        WITH Timestamps AS (
          SELECT 
            t.route_id,
      ...
      `;
            const raw = await database_1.default.raw(query);
            ...
            logger_1.logger.info(`[GTFS DIAGNOSTIC] Complete. Returning ${finalResult.length} mapped rows to UI.`);
            if (finalResult.length > 0) {
                logger_1.logger.info(`[GTFS DIAGNOSTIC] Row sample: ${JSON.stringify(finalResult[0]).substring(0, 300)}`);
            }
            else {
                logger_1.logger.warn(`[GTFS DIAGNOSTIC] CRITICAL: Returning 0 rows to frontend! Raw rows retrieved from DB was ${rows.length}`);
            }
            return finalResult;
        }
        catch (err) {
            logger_1.logger.error(`[GTFS Service] Error computing daily timetable`, err);
            throw err;
        }
    },
    /**
     ...
     */
    async listLinesForAgency(agencyId) {
        try {
            let routeCondition = "1=1";
            if (agencyId === '70')
                routeCondition = "route_short_name ~ '^(3[0-9]{2}|L)'";
            else if (agencyId === '50')
                routeCondition = "route_short_name ~ '^(1[0-9]{2}|D10|G)'";
            else if (agencyId === '20')
                routeCondition = "route_short_name ~ '^2[0-9]{2}'";
            else if (agencyId === '60')
                routeCondition = "route_short_name ~ '^4[0-9]{2}'";
            const query = `
        SELECT DISTINCT 
      ...
      `;
            ...
            logger_1.logger.error(`[GTFS Service] Error listing lines for agency`, err);
            throw err;
        }
    },
    /**
     ...
     */
    async getLineGeometry(agencyId, linea, directionId) {
        try {
      ...
      `;
            const shapeRaw = await database_1.default.raw(shapeQuery, [linea, directionId]);
            const shape = (shapeRaw.rows || shapeRaw).map((r) => ({
                lat: Number(r.lat),
                lng: Number(r.lng)
            }));
      ...
      `;
            const stopsRaw = await database_1.default.raw(stopsQuery, [linea, directionId]);
            const paradas = (stopsRaw.rows || stopsRaw).map((r) => ({
                id: String(r.id),
                nombre: r.nombre,
            ...
            logger_1.logger.info(`[GTFS GEOMETRY] Resolved ${linea} DIR ${directionId}: ${shape.length} points, ${paradas.length} stops.`);
            return {
                linea,
                directionId,
            ...
            logger_1.logger.error(`[GTFS Service] Error getting line geometry`, err);
            throw err;
        }
    },
    /**
     ...
     */
    async getSpecificTimetable(agencyId, linea, directionId, serviceType) {
        try {
            let dayFilter = 'monday = 1';
            if (serviceType === 'SABADO')
                dayFilter = 'saturday = 1';
      ...
      `;
            const raw = await database_1.default.raw(query, [linea, directionId]);
            const rows = raw.rows || raw;
            const timeToMin = (timeStr) => {
                const [h, m] = timeStr.split(':').map(Number);
                return (h * 60) + m;
            ...
            logger_1.logger.error(`[GTFS Service] Error loading specific timetable for ${linea}`, err);
            return null;
        }
    }
};
