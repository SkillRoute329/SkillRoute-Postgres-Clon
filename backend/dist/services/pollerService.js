...
exports.getPollerStats = getPollerStats;
exports.startPoller = startPoller;
exports.stopPoller = stopPoller;
const database_1 = __importDefault(require("../config/database"));
const logger_1 = __importDefault(require("../config/logger"));
const scheduleComplianceEngine_1 = require("./scheduleComplianceEngine");
const vehicleHistoryService_1 = require("./vehicleHistoryService");
function readConfig() {
...
async function updateBusLastPos(results) {
    if (results.length === 0)
        return 0;
    let updated = 0;
    for (const r of results) {
        try {
            ...
            await (0, database_1.default)('bus_last_pos')
                .insert(row)
                .onConflict('id_bus')
            ...
            logger_1.default.warn(`[Poller] error actualizando bus_last_pos para ${r.idBus}`, { err: String(e) });
        }
    }
    return updated;
}
// ─── Cálculo de ETAs por parada (sub-componente FASE 3.5.4) ────────────────
async function updateEtaPredictions(results) {
    let written = 0;
    for (const r of results) {
        if (!r.tripActivo || !r.proximaParadaControl || r.minutosParaProximaParada == null)
            continue;
        ...
        try {
            ...
            await (0, database_1.default)('bus_eta_predictions')
                .insert(row)
            ...
            logger_1.default.warn(`[Poller] error actualizando ETA para ${r.idBus}`, { err: String(e) });
        }
    }
    return written;
}
async function recordCycleHealth(s) {
    try {
        await (0, database_1.default)('poller_health').insert({
        ...
        logger_1.default.error('[Poller] no se pudo persistir poller_health', { err: String(e) });
    }
}
// ─── Un ciclo del poller ───────────────────────────────────────────────────
async function runCycle(agencyId) {
    ...
    try {
        ...
        if (results.length > 0) {
            ...
            await (0, vehicleHistoryService_1.saveComplianceSnapshot)(results);
        ...
        totalErrors++;
        logger_1.default.error(`[Poller] error en ciclo agency=${agencyId}`, { err: errorMessage });
    }
    ...
    if (totalCycles % 30 === 0) {
        ...
        logger_1.default.info(`[Poller] heartbeat — cycles=${totalCycles} events=${totalEventsPersisted} errors=${totalErrors} lastAgency=${agencyId}`);
    }
}
// ─── Ciclo cooperativo (todas las agencies configuradas, una vez) ──────────
async function runAllAgencies() {
    if (cycleInFlight) {
        logger_1.default.warn('[Poller] ciclo anterior aún en curso; salteando para evitar overlap');
        return;
    }
    cycleInFlight = true;
    try {
...
function startPoller() {
    ...
    if (!config.enabled) {
        logger_1.default.info('[Poller] desactivado por POLLER_ENABLED=false');
        return;
    }
    if (isRunning) {
        logger_1.default.warn('[Poller] ya está corriendo, ignoro start');
        return;
    }
    logger_1.default.info(`[Poller] arrancando — agencies=[${config.agencies.join(',')}] interval=${config.intervalMs}ms version=${config.version}`);
    isRunning = true;
    // Primer ciclo inmediato (no esperar el primer interval)
    runAllAgencies().catch((e) => logger_1.default.error('[Poller] error en primer ciclo', { err: String(e) }));
    timer = setInterval(() => {
        runAllAgencies().catch((e) => logger_1.default.error('[Poller] error en ciclo programado', { err: String(e) }));
    }, config.intervalMs);
}
function stopPoller() {
    if (!isRunning)
        return;
    logger_1.default.info(`[Poller] deteniendo — total cycles=${totalCycles} events=${totalEventsPersisted}`);
    if (timer)
        clearInterval(timer);
    timer = null;
    isRunning = false;
