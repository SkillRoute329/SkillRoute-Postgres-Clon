...
exports.TOOL_SPECS = exports.WRITE_TOOL_SPECS = exports.READ_TOOL_SPECS = void 0;
...
exports.READ_TOOL_SPECS = [
    {
        type: 'function',
        function: {
            name: 'get_fleet_status',
            description: 'Devuelve el estado agregado de la flota UCOT: cantidad total de vehículos y desglose por estado (available, maintenance, service, disabled). Usar cuando el inspector pregunta "cuántos coches tenemos", "cuántos en servicio", "estado general de flota".',
    ...
    },
    {
        ...
        function: {
            ...
            description: 'Busca un coche específico por su número interno (ej: "55", "300", "1042") y devuelve su estado actual, matrícula, modelo, último check y conductor asignado. Usar cuando el inspector menciona un interno específico.',
            parameters: {
                type: 'object',
                properties: {
                    internal_number: {
    ...
    },
    {
        ...
        function: {
            ...
            description: 'Lista los coches UCOT actualmente en servicio (status = service) con su número interno y conductor. Usar cuando el inspector quiere saber quiénes están en la calle ahora mismo.',
            parameters: { type: 'object', properties: {} },
        },
    },
    {
        ...
        function: {
            ...
            description: 'Devuelve la fecha y hora actual del servidor en zona horaria Uruguay. Usar antes de analizar horarios, frecuencias o cualquier cálculo que dependa del momento actual.',
            parameters: { type: 'object', properties: {} },
        },
    },
    {
        ...
        function: {
            ...
            description: 'Devuelve las posiciones GPS reales en tiempo real de los coches en la calle, obtenido directamente de la API de la Intendencia (STM). Permite filtrar por línea y empresa. Usar para saber qué coches específicos (número interno) están en el recorrido ahora mismo.',
            parameters: {
                type: 'object',
                properties: {
                    line_id: {
    ...
    },
    {
        ...
        function: {
            ...
            description: 'Consulta el Oráculo de horarios teóricos para una parada específica utilizando los calendarios oficiales SQL. Devuelve los próximos ómnibus que pasarán por allí hoy.',
            parameters: {
                type: 'object',
                properties: {
                    stop_id: {
...
exports.WRITE_TOOL_SPECS = [
    {
        type: 'function',
        function: {
            name: 'suggest_hold_vehicle',
            description: 'PROPONE (no ejecuta) retener un coche UCOT por X minutos en su próxima parada. La sugerencia queda pendiente de aprobación del inspector humano. Usar cuando la situación táctica justifique cerrar un gap (ej: el siguiente interno está muy cerca, o hay que dejar separación con la competencia).',
            parameters: {
                type: 'object',
                properties: {
    ...
    },
    {
        ...
        function: {
            ...
            description: 'PROPONE (no ejecuta) que un coche UCOT adelante el paso (minimice paradas, acelere) para cerrar un gap. Queda pendiente de aprobación humana.',
            parameters: {
                type: 'object',
                properties: {
...
exports.TOOL_SPECS = [...exports.READ_TOOL_SPECS, ...exports.WRITE_TOOL_SPECS];
// ─── Handlers (ejecutan la consulta real) ─────────────────────────────────────
const handlers = {
    async get_fleet_status() {
        const vehicles = await (0, fleetService_1.getAllVehicles)();
        const byStatus = {
            available: 0,
            maintenance: 0,
            service: 0,
    ...
    async find_vehicle_by_internal_number(args) {
        ...
        if (!internal)
            return { error: 'internal_number vacío' };
        const vehicles = await (0, fleetService_1.getAllVehicles)();
        const match = vehicles.find((v) => v.internalNumber === internal);
        if (!match)
            return { error: `No se encontró coche con interno ${internal}` };
        try {
            const full = await (0, fleetService_1.getVehicleById)(match.id);
            return {
    ...
    async get_active_positions(args) {
        ...
        try {
            const res = await axios_1.default.post('https://www.montevideo.gub.uy/buses/rest/stm-online', {}, {
                timeout: 10000,
                headers: {
            ...
            logger_1.default.error('[AI-TOOLS] Error fetching STM positions', { error: String(err) });
    ...
    async get_competition_report(args) {
        ...
        try {
            const report = await (0, stmPublicDataScraper_1.analizarCompetenciaLinea)(lineId);
            return {
                linea: report.linea,
    ...
    async suggest_hold_vehicle(args, ctx) {
        ...
        if (!internal || !reason)
            return { error: 'internal_number y reason son requeridos' };
        if (!Number.isFinite(minutes) || minutes < 1 || minutes > 15) {
            return { error: 'minutes debe estar entre 1 y 15' };
        }
        const order = await (0, aiOrdersService_1.createSuggestion)({
            type: 'hold_vehicle',
    ...
    async analyze_gtfs_route_overlaps(args) {
        ...
        try {
            ...
            return {
                ...
                rivales_detectados_top: analysis.map((r) => ({
...
async function executeTool(name, args, ctx) {
    ...
    if (!handler) {
        logger_1.default.warn(`[AITools] Tool desconocida: ${name}`);
        return { result: { error: `Tool '${name}' no existe` }, ok: false };
    }
    try {
        const result = await handler(args, ctx);
        logger_1.default.debug(`[AITools] ${name} ejecutada`, { args });
        return { result, ok: true };
    }
    catch (err) {
        logger_1.default.error(`[AITools] ${name} falló`, { err: String(err) });
