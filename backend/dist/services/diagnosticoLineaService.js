...
exports.generarDiagnosticoLineas = generarDiagnosticoLineas;
/**
...
const cartonOficial_1 = require("./cartonOficial");
/** Línea → servicios UCOT (FUENTE OFICIAL: cartón XLS validado). */
function serviciosDeLinea(linea) {
    return (0, cartonOficial_1.serviciosOficialesDeLinea)(linea).map((s) => ({
        servicio: s.servicio,
 ...
 */
async function auditarLineas(lineas, codEmpresa, op, mesAct, mesComp) {
    if (!mesAct || lineas.length === 0) {
        return {
            certificado: 'INFORME VERIFICADO — SIN DISCREPANCIAS',
            metodoVerificacion: 'Sin datos para auditar (no hay mes STM disponible para el operador).',
            totalChecks: 0,
            ok: 0,
            discrepancias: 0,
            lineasConDiscrepancia: [],
            selloVerificacion: (0, crypto_1.createHash)('sha256').update('vacio').digest('hex').slice(0, 16),
    ...
    const selloRowQ = await database_1.default.raw(`SELECT verificado_en, filas_mv, filas_raw, discrepancias, ok, sello
       FROM mv_stm_verificacion ORDER BY id DESC LIMIT 1`);
    const selloRow = ((selloRowQ.rows ?? selloRowQ)[0] ?? null);
    ...
    const dhSql = async (iso) => {
        const q = await database_1.default.raw(`SELECT count(*)::int AS n
         FROM generate_series(?::date,
                              (?::date + interval '1 month') - interval '1 day',
                              interval '1 day') g
        WHERE extract(isodow from g) BETWEEN 1 AND 5`, [iso, iso]);
    ...
    const reValQ = await (0, database_1.default)('mv_stm_linea_mes')
        .where('cod_empresa', codEmpresa)
        .whereIn('mes', [mesAct, mesComp].filter(Boolean))
        .select('dsc_linea', 'mes', 'habil', 'total');
    const reHabil = new Map();
    const reTotal = new Map();
    for (const r of reValQ) {
    ...
    const rePicoQ = await (0, database_1.default)('mv_stm_linea_mes_hora')
        .where('cod_empresa', codEmpresa)
        .where('mes', mesAct)
        .select('dsc_linea', 'hora', 'val_habil');
    const rePicoMap = new Map();
    for (const r of rePicoQ) {
        const cur = rePicoMap.get(r.dsc_linea);
        const v = Number(r.val_habil);
        if (!cur || v > cur.val)
            rePicoMap.set(r.dsc_linea, { hora: Number(r.hora), val: v });
    }
    let totalChecks = 0;
    let okCount = 0;
    const lineasMal = [];
    for (const l of lineas) {
        ...
        checks.push({
            ...
            fuente: fuenteMV,
            metodoRecomputo: 're-lectura independiente del agregado + días hábiles por 2º método (Postgres isodow vs cálculo JS del informe)',
        ...
        const svMal = svShown.filter((sv) => ((0, cartonOficial_1.lineaOficialDeServicio)(sv) ?? '') !== l.linea);
        checks.push({
            campo: 'serviciosUcot↔línea',
            fuente: `cartón OFICIAL UCOT (${(0, cartonOficial_1.cartonMeta)().archivo.split(/[\\/]/).pop()}), ${(0, cartonOficial_1.cartonMeta)().totalServicios} servicios`,
            metodoRecomputo: 're-lectura del XLS oficial: línea oficial de cada servicio mostrado debe ser = línea del informe',
    ...
    const sello = (0, crypto_1.createHash)('sha256')
    ...
    return {
        certificado: disc === 0 && docOk
            ? 'INFORME VERIFICADO — SIN DISCREPANCIAS'
            : 'INFORME CON DISCREPANCIAS — REVISAR',
        metodoVerificacion: 'Dos niveles: (1) DOCUMENTO↔CRUDO OFICIAL verificado OFFLINE al ' +
 ...
 */
async function generarDiagnosticoLineas(op = '70') {
    ...
    const metodologiaTxt = 'Método: comparación INTERANUAL (mismo mes del año anterior) sobre ' +
    ...
    const etiquetaEstacional = (iso) => {
        const m = Number(iso.split('-')[1]);
        if (m === 1 || m === 2)
            return 'verano + receso escolar (temporada baja)';
        if (m === 7)
            return 'receso invernal escolar';
        if (m === 12)
            return 'cierre de clases a mediados de mes (parcial)';
        return null;
    };
    ...
    const mesesRows = (await (0, database_1.default)('mv_stm_linea_mes')
        .where('cod_empresa', codEmpresa)
        .distinct('mes')
        .orderBy('mes', 'asc'));
    ...
    const valRows = mesAct
        ? (await (0, database_1.default)('mv_stm_linea_mes')
            .where('cod_empresa', codEmpresa)
            .whereIn('mes', [mesAct, mesComp].filter(Boolean))
            .select('dsc_linea', 'mes', 'habil', 'total'))
        : [];
    const habilPorLineaMes = new Map();
    const totalPorLineaMes = new Map();
    ...
    const picoRows = mesAct
        ? (await (0, database_1.default)('mv_stm_linea_mes_hora')
            .where('cod_empresa', codEmpresa)
            .where('mes', mesAct)
            .select('dsc_linea', 'hora')
            .select(database_1.default.raw('val_habil AS val')))
        : [];
    const picoPorLinea = new Map();
    for (const r of picoRows) {
        const cur = picoPorLinea.get(r.dsc_linea);
        const v = Number(r.val);
        if (!cur || v > cur.val)
            picoPorLinea.set(r.dsc_linea, { hora: Number(r.hora), val: v });
    }
    ...
    const gtfsMeta = (await database_1.default.raw(`WITH nm AS (
    ...
    for (const r of gtfsMeta.rows) {
        ...
        let frec = null;
        if (trips > 1 && p && u) {
            const span = (Number(u.slice(0, 2)) * 60 + Number(u.slice(3))) -
                (Number(p.slice(0, 2)) * 60 + Number(p.slice(3)));
            if (span > 0)
                frec = Math.round((span / (trips - 1)) * 10) / 10;
        }
    ...
    const overlaps = (await (0, database_1.default)('corridor_overlap')
        .where('same_empresa', false)
        .andWhere((b) => b.where('agency_a', op).orWhere('agency_b', op))
        .select('linea_a', 'linea_b', 'agency_a', 'agency_b', 'pct_a_in_b', 'pct_b_in_a', 'shared_km'));
    const compPorLinea = new Map();
    ...
    for (const r of valRows)
        lineasSet.add(String(r.dsc_linea));
    const lineas = [];
    for (const linea of [...lineasSet].sort((a, b) => a.localeCompare(b, 'es', { numeric: true }))) {
        ...
        const baseComparacion = hayYoY ? 'INTERANUAL' : 'NINGUNA';
        let variacionPct = null;
        let dAbs = 0;
        let tendencia = 'NO_CONCLUYENTE';
        ...
        let diagnostico = `Línea ${linea} (${origen} → ${destino}). En ${mesAct} promedió ` +
        ...
        if (baseComparacion === 'INTERANUAL' && promComp != null) {
            const palabra = tendencia === 'BAJA'
                ? `BAJA ${Math.abs(variacionPct)}%`
                : tendencia === 'SUBE'
                    ? `SUBE ${variacionPct}%`
        ...
        if (top) {
            ...
            if (tendencia === 'BAJA') {
                accion =
                    `La caída es INTERANUAL real (${Math.abs(variacionPct)}% vs ` +
                ...
                baseComparacion === 'NINGUNA'
                    ...
                    : tendencia === 'BAJA'
                        ? `Sin competidor cross-operador: la baja interanual ` +
                            `(${Math.abs(variacionPct)}%) es de demanda propia; ` +
        ...
        const fuentes = [
            ...
            servicios.length
                ? `Servicio/cartón: CARTÓN OFICIAL UCOT "${(0, cartonOficial_1.cartonMeta)().archivo
                    .split(/[\\/]/)
        ...
        const fundamento = baseComparacion === 'NINGUNA'
            ...
            : tendencia === 'BAJA'
                ? `La caída es real: ${Math.abs(variacionPct)}% interanual ` +
                    `(${fmt(promComp)}->${fmt(promAct)} val/día hábil, ` +
    ...
    logger_1.default.info(`[diagnosticoLinea] op ${op}: ${lineas.length} líneas, mes ${mesAct}` +
