...
class AIService {
     ...
     */
    static async generate(task, prompt, opts = {}) {
    ...
    static async embed(text) {
        const res = await (0, node_fetch_1.default)(`${OLLAMA_BASE}/api/embeddings`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ model: MODEL_MAP.EMBED, prompt: text }),
        });
        if (!res.ok)
            throw new Error(`[AIService] Embed error: ${res.status}`);
     ...
     */
    static async callClaude(prompt, maxTokens = 500) {
     ...
     */
    static async chat(task, messages, opts = {}) {
        ...
        try {
            ...
            if (tools && tools.length > 0)
                body.tools = tools;
            const res = await (0, node_fetch_1.default)(`${OLLAMA_BASE}/api/chat`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
     ...
     */
    static prewarm(task = 'HEAVY') {
        const model = MODEL_MAP[task];
        (0, node_fetch_1.default)(`${OLLAMA_BASE}/api/chat`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
                model,
    ...
    // ─── Privado ────────────────────────────────────────────────────────────────
    static async callOllama(model, prompt, timeoutMs, temperature, maxTokens) {
        const t0 = Date.now();
        const controller = new AbortController();
        const timer = setTimeout(() => controller.abort(), timeoutMs);
        try {
            const res = await (0, node_fetch_1.default)(`${OLLAMA_BASE}/api/generate`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({
                    model,
