...
const aiToolsService_1 = require("./aiToolsService");
...
async function routeIntent(userMessage) {
    ...
    if (normalized.includes('competencia') ||
        ...
        logger_1.default.info('[Router] Forzando INTENT=READ vía Heurística (Palabras Clave detectadas)');
        return { intent: 'READ', latencyMs: Date.now() - t0 };
    }
    try {
        const res = await aiService_1.AIService.chat('FAST', [
        ...
        const intent = match ? match[1] : 'TACTICAL';
        return { intent, latencyMs: Date.now() - t0 };
    }
    catch (err) {
        logger_1.default.warn('[Router] Falló, fallback a TACTICAL', { err: String(err) });
...
function configForIntent(intent) {
    switch (intent) {
        case 'TACTICAL':
            return { task: 'HEAVY', systemPrompt: TACTICAL_SYSTEM, tools: aiToolsService_1.TOOL_SPECS, temperature: 0.3, maxTokens: 800 };
        case 'READ':
            ...
            return { task: 'HEAVY', systemPrompt: READ_SYSTEM, tools: aiToolsService_1.READ_TOOL_SPECS, temperature: 0.2, maxTokens: 500 };
        case 'CHAT':
            // Le equipamos también las tools de lectura en CHAT como contingencia máxima
            return { task: 'HEAVY', systemPrompt: CHAT_SYSTEM, tools: aiToolsService_1.READ_TOOL_SPECS, temperature: 0.4, maxTokens: 500 };
    }
}
async function runCopilot(history, userMessage, userId) {
    const t0 = Date.now();
    const traces = [];
    const { intent, latencyMs: routerMs } = await routeIntent(userMessage);
    const cfg = configForIntent(intent);
    logger_1.default.info(`[Orchestrator] Intent=${intent} (router ${routerMs}ms)`);
    const messages = [
    ...
    while (rounds < MAX_TOOL_ROUNDS) {
        rounds++;
        const res = await aiService_1.AIService.chat(cfg.task, messages, {
            tools: cfg.tools,
            timeoutMs: 120000,
        ...
        for (const call of toolCalls) {
            ...
            const { result, ok } = await (0, aiToolsService_1.executeTool)(name, args, { userId, model });
    ...
    logger_1.default.warn(`[Orchestrator] Alcanzó MAX_TOOL_ROUNDS=${MAX_TOOL_ROUNDS} (intent=${intent})`);
    return {
