...
exports.startCascadeAutoTrigger = startCascadeAutoTrigger;
exports.stopCascadeAutoTrigger = stopCascadeAutoTrigger;
...
async function lastFiredCooldown(entityType, entityId, eventoTipo) {
    try {
        const row = await (0, database_1.default)('cascade_cooldowns')
...
async function setFiredCooldown(entityType, entityId, eventoTipo, meta) {
    try {
        await (0, database_1.default)('cascade_cooldowns')
            .insert({
            entity_type: entityType,
            entity_id: entityId,
            evento_tipo: eventoTipo,
            fired_at: database_1.default.fn.now(),
            data_jsonb: meta ?? {},
        })
            .onConflict(['entity_type', 'entity_id', 'evento_tipo'])
            .merge({ fired_at: database_1.default.fn.now(), data_jsonb: meta ?? {} });
    }
    catch (e) {
        logger_1.default.warn('[cascadeCooldown] error UPSERT', { err: String(e).slice(0, 100) });
    }
}
let _timer = null;
async function detectarRetrasoPorLinea() {
    ...
    const rows = await (0, database_1.default)('bus_last_pos')
        ...
        .count('* as total')
        .select(database_1.default.raw("COUNT(*) FILTER (WHERE estado_cumplimiento = 'ATRASADO') AS atrasados"))
        .whereNotNull('linea')
        .andWhere('timestamp_gps', '>=', database_1.default.raw("NOW() - INTERVAL '10 minutes'"))
        .groupBy('linea', 'agency_id');
    const ahora = Date.now();
    for (const r of rows) {
        const total = Number(r.total);
        const atrasados = Number(r.atrasados);
        if (total < cfg.retrasoMinBuses)
            continue;
        const pct = (atrasados / total) * 100;
        if (pct < cfg.retrasoThresholdPct)
            continue;
        const key = `${r.agency_id ?? 'NA'}:${r.linea}`;
        const elapsed = await lastFiredCooldown('linea', key, 'RETRASO_OPERATIVO');
        if (elapsed < cfg.cooldownLineaMs)
            continue;
        await setFiredCooldown('linea', key, 'RETRASO_OPERATIVO', { atrasados, total, pct });
        try {
            const res = await (0, consequenceController_1.computeConsequencesForEvent)({
            ...
            if (res.ok) {
                logger_1.default.info(`[AUTO-CASCADE] línea ${r.linea} (agency ${r.agency_id}) → ${atrasados}/${total} ATRASADOS (${pct.toFixed(0)}%) — efectos=${res.efectos.length}`);
            }
        }
        catch (e) {
            logger_1.default.warn('[AUTO-CASCADE] error en línea ' + r.linea, { err: String(e).slice(0, 100) });
        }
    }
}
async function detectarFueraDeServicio() {
    const cfg = await (0, motorConfigService_1.getMotorConfig)();
    const rows = await (0, database_1.default)('bus_last_pos')
        .select('id_bus', 'agency_id', 'linea', 'timestamp_gps')
        .where('estado_cumplimiento', 'FUERA_DE_SERVICIO')
        .andWhere('timestamp_gps', '<=', database_1.default.raw(`NOW() - INTERVAL '${cfg.cocheFdsMinMin} minutes'`))
        .andWhere('timestamp_gps', '>=', database_1.default.raw("NOW() - INTERVAL '30 minutes'"));
    for (const r of rows) {
        const elapsed = await lastFiredCooldown('coche', r.id_bus, 'VEHICULO_FUERA_DE_SERVICIO');
        if (elapsed < cfg.cooldownCocheMs)
            continue;
        ...
        try {
            ...
            if (res.ok) {
                logger_1.default.info(`[AUTO-CASCADE] coche ${r.id_bus} FUERA_DE_SERVICIO ${cfg.cocheFdsMinMin}min+ — efectos=${res.efectos.length}`);
            }
        }
        catch (e) {
            logger_1.default.warn('[AUTO-CASCADE] error en coche ' + r.id_bus, { err: String(e).slice(0, 100) });
        }
    }
}
...
async function detectarBunching() {
    const cfg = await (0, motorConfigService_1.getMotorConfig)();
    const distMetros = cfg.bunchingDistanciaMetros;
    let rows = [];
    try {
        rows = (await database_1.default.raw(`SELECT a.linea, a.agency_id, a.id_bus AS id_bus_a, b.id_bus AS id_bus_b,
        ...
        LIMIT 50`, [distMetros])).rows;
    }
    catch (e) {
        logger_1.default.warn('[AUTO-CASCADE] bunching SQL error', { err: String(e).slice(0, 120) });
        return;
    }
    for (const r of rows) {
        ...
        try {
            ...
            if (res.ok) {
                logger_1.default.info(`[AUTO-CASCADE] BUNCHING línea ${r.linea} ${r.id_bus_a}↔${r.id_bus_b} ${Math.round(r.metros)}m — efectos=${res.efectos.length}`);
            }
        }
        catch (e) {
            logger_1.default.warn('[AUTO-CASCADE] error bunching', { err: String(e).slice(0, 100) });
        }
    }
}
...
async function detectarBajaCobertura() {
    const cfg = await (0, motorConfigService_1.getMotorConfig)();
    if (cfg.coberturaMinBusesPorLinea <= 0)
        return;
    ...
    let rows = [];
    try {
        rows = (await database_1.default.raw(`SELECT l.numero AS linea, l.agency_id,
        ...
        HAVING COUNT(b.id_bus) FILTER (WHERE b.timestamp_gps >= NOW() - INTERVAL '10 minutes') > 0
           AND COUNT(b.id_bus) FILTER (WHERE b.timestamp_gps >= NOW() - INTERVAL '10 minutes') < ?`, [cfg.coberturaMinBusesPorLinea])).rows;
    }
    catch (e) {
        logger_1.default.warn('[AUTO-CASCADE] baja cobertura SQL error', { err: String(e).slice(0, 120) });
        return;
    }
    for (const r of rows) {
        const buses = Number(r.buses_vivos);
        if (buses <= 0)
            continue; // 0 buses puede ser madrugada legítima
        const key = `${r.agency_id ?? 'NA'}:${r.linea}`;
        ...
        try {
            ...
            if (res.ok) {
                logger_1.default.info(`[AUTO-CASCADE] BAJA COBERTURA línea ${r.linea} (agency ${r.agency_id}) buses=${buses} < mín=${cfg.coberturaMinBusesPorLinea} — efectos=${res.efectos.length}`);
            }
        }
        catch (e) {
            logger_1.default.warn('[AUTO-CASCADE] error cobertura', { err: String(e).slice(0, 100) });
        }
    }
}
...
async function detectarHeadwayIrregular() {
    const cfg = await (0, motorConfigService_1.getMotorConfig)();
    ...
    let rows = [];
    try {
        rows = (await database_1.default.raw(`WITH activos AS (
       ...
       SELECT lb.linea, lb.agency_id,
        ...
        GROUP BY lb.linea, lb.agency_id, lb.n`, [cfg.headwayMinBuses, cfg.headwayPegadoMetros, cfg.headwayLejosMetros])).rows;
    }
    catch (e) {
        logger_1.default.warn('[AUTO-CASCADE] headway SQL error', { err: String(e).slice(0, 120) });
        return;
    }
    for (const r of rows) {
        ...
        try {
            ...
            if (res.ok) {
                logger_1.default.info(`[AUTO-CASCADE] HEADWAY_IRREGULAR línea ${r.linea} (agency ${r.agency_id}) pegados=${pegados} lejos=${lejos} buses=${r.total_buses} — efectos=${res.efectos.length}`);
            }
        }
        catch (e) {
            logger_1.default.warn('[AUTO-CASCADE] error headway', { err: String(e).slice(0, 100) });
        }
    }
}
...
// ruta.
async function detectarVelocidadAnomala() {
    const cfg = await (0, motorConfigService_1.getMotorConfig)();
    let rows = [];
    try {
        rows = (await database_1.default.raw(`SELECT id_bus,
              agency_id,
              MAX(linea) AS linea,
              COUNT(*) AS muestras,
       ...
       HAVING COUNT(*) >= ?
          AND MAX(velocidad) < ?
          AND AVG(velocidad) < ?`, [cfg.velocidadAnomalaMuestraMin, cfg.velocidadAnomalaKmhMin, cfg.velocidadAnomalaKmhMin])).rows;
    }
    catch (e) {
        // Si vehicle_events no tiene la columna velocidad (algunos esquemas no la tienen),
        // probamos con bus_last_pos como fallback.
        try {
            rows = (await (0, database_1.default)('bus_last_pos')
                .select('id_bus', 'agency_id', 'linea')
                .where('timestamp_gps', '>=', database_1.default.raw("NOW() - INTERVAL '8 minutes'"))
                .andWhere('velocidad', '<', cfg.velocidadAnomalaKmhMin)
                .andWhere('velocidad', '>=', 0)
                .limit(50));
            // Sin contar muestras en este fallback — saltamos.
            logger_1.default.warn('[AUTO-CASCADE] velocidad: usando fallback bus_last_pos (sin agregación temporal)', { err: String(e).slice(0, 80) });
            return;
        }
        catch (e2) {
            logger_1.default.warn('[AUTO-CASCADE] velocidad SQL error', { err: String(e2).slice(0, 100) });
            return;
        }
    }
    for (const r of rows) {
        ...
        try {
            ...
            if (res.ok) {
                logger_1.default.info(`[AUTO-CASCADE] VELOCIDAD_ANOMALA coche ${r.id_bus} (línea ${r.linea}) avg=${Number(r.vel_avg).toFixed(1)}km/h muestras=${r.muestras} — efectos=${res.efectos.length}`);
            }
        }
        catch (e) {
            logger_1.default.warn('[AUTO-CASCADE] error velocidad', { err: String(e).slice(0, 100) });
        }
    }
}
...
async function detectarInspeccionesAusentes() {
    const cfg = await (0, motorConfigService_1.getMotorConfig)();
    let rows = [];
    try {
        rows = (await database_1.default.raw(`WITH lineas_activas AS (
       ...
       SELECT la.linea, la.agency_id,
        ...
        LIMIT 50`, [cfg.inspeccionAusenteDias])).rows;
    }
    catch (e) {
        logger_1.default.warn('[AUTO-CASCADE] inspecciones-ausentes SQL error', { err: String(e).slice(0, 120) });
        return;
    }
    for (const r of rows) {
        const dias = Number(r.dias_sin_inspeccion);
        if (!Number.isFinite(dias) || dias < cfg.inspeccionAusenteDias)
            continue;
        const key = `${r.agency_id ?? 'NA'}:${r.linea}`;
        ...
        try {
            ...
            if (res.ok) {
                logger_1.default.info(`[AUTO-CASCADE] INSPECCIONES_AUSENTES línea ${r.linea} (agency ${r.agency_id}) días=${Math.round(dias)} — efectos=${res.efectos.length}`);
            }
        }
        catch (e) {
            logger_1.default.warn('[AUTO-CASCADE] error inspecciones-ausentes', { err: String(e).slice(0, 100) });
        }
    }
}
async function tick() {
    try {
        ...
        logger_1.default.warn('[cascadeAutoTrigger] tick error', { err: String(e).slice(0, 200) });
    }
}
function startCascadeAutoTrigger() {
    if (_timer)
        return;
    logger_1.default.info(`[cascadeAutoTrigger] ACTIVO · cada ${TICK_MS / 1000}s (retraso, FDS, bunching, baja-cobertura, headway, velocidad-anomala, inspecciones-ausentes — params en system_config, cooldowns en cascade_cooldowns)`);
    // Primer tick demorado 30s para no competir con el arranque.
    setTimeout(() => { void tick(); }, 30000);
    _timer = setInterval(() => { void tick(); }, TICK_MS);
}
function stopCascadeAutoTrigger() {
    if (_timer) {
