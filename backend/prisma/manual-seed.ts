...
async function seed() {
    ...
    try {
        ...
        for (const cat of categories) {
            await pool.query(
                `INSERT INTO "ShiftCategory" (name, "baseValue", "extraHourValue", "updatedAt") 
         VALUES ($1, $2, $3, NOW())
         ON CONFLICT (name) DO UPDATE SET "baseValue" = $2, "extraHourValue" = $3`,
                [cat.name, cat.baseValue, cat.extraHourValue]
            );
        ...
        for (const user of users) {
            ...
            await pool.query(
                `INSERT INTO "User" ("internalNumber", "firstName", "lastName", "fullName", "passwordHash", "role") 
         VALUES ($1, $2, $3, $4, $5, $6)
         ON CONFLICT ("internalNumber") DO UPDATE SET "passwordHash" = $5`,
                [user.internalNumber, user.firstName, user.lastName, user.fullName, passwordHash, user.role]
            );
