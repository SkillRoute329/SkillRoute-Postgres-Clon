...
class MasterOrchestrator {
   ...
   */
  async initialize() {
    console.log('[MasterOrchestrator] Inicializando ecosistemas...');
    ...
    for (const lineaConfig of this.lineasConfig.lineas) {
      try {
        ...
        console.log(`✅ Línea ${lineaConfig.id}: ${ecosystem.totalAgents} agentes creados`);
      } catch (error) {
        console.error(`❌ Error inicializando línea ${lineaConfig.id}:`, error.message);
      }
    }

    console.log(`[MasterOrchestrator] Inicialización completada. ${this.activeEcosystems.size} líneas activas.`);
  }
  ...
  async alertFromCompetitor(lineId, competitorData) {
    ...
    const monitor = ecosystem.competitorAgents.find(
      a => a.competitorId === competitorData.competitorId
    );
