...
const URL = 'http://localhost:3000'; // Default dev backend port
...
for (let i = 0; i < TOTAL_DRIVERS; i++) {
  ...
  socket.on('connect', () => {
    ...
    if (connections === TOTAL_DRIVERS) {
      console.log(`[Socket] ¡Todos los choferes conectados con éxito! Iniciando ráfaga de polling...`);
...
function startIntensivePolling() {
  ...
  const testDurationMs = 10000; // 10 segundos de test
  
  const timer = setInterval(() => {
    // Agrupamos en bloques para no asfixiar el loop de Node local
    let batch = 100;
  ...
  }, 1000); // 100 requests / sec from Node script directly

  setTimeout(() => {
    clearInterval(timer);
    console.log(`\n==============================================`);
    console.log(`[RESULTADOS DE AUDITORÍA - ESTRÉS DE 10 SEC]`);
    console.log(`==============================================`);
...
setTimeout(() => {
  if (connections < TOTAL_DRIVERS) {
    console.log(`[Timeout] Solo conectaron ${connections} sockets. Test abortado.`);
    process.exit(1);
  }
}, 10000);
