...
const stdoutIsTTY = process.stdout && (process.stdout as any).isTTY === true;
...
function silenceConsoleOnEpipe(stream: NodeJS.WriteStream, name: string) {
...
silenceConsoleOnEpipe(process.stdout as NodeJS.WriteStream, 'stdout');
silenceConsoleOnEpipe(process.stderr as NodeJS.WriteStream, 'stderr');
