...
interface HoraBucket {
  hora: string;          // ISO truncado a hora
  total: number;
  criticos: number;
}

export async function getMotorHealth(_req: Request, res: Response): Promise<void> {
  try {
    // 1. Eventos por hora en últimas 24h
    const horasRows: Array<{ hora: Date; total: string; criticos: string }> = (await sqlDb.raw(
    ...
    const porHora: HoraBucket[] = horasRows.map((r) => ({
    ...
    const tipoRows: Array<{ tipo: string; total: string }> = (await sqlDb.raw(
      `SELECT recurso AS tipo, COUNT(*) AS total
         FROM logs_auditoria
        WHERE accion = 'consequencePreview'
          AND timestamp >= NOW() - INTERVAL '24 hours'
        GROUP BY recurso
    ...
    for (const r of tipoRows) porTipo[r.tipo] = Number(r.total);

    // 3. Distribución por fuente (manual vs auto-trigger)
    const fuenteRows: Array<{ fuente: string; total: string }> = (await sqlDb.raw(
      `SELECT COALESCE(detalles_jsonb #>> '{evento,fuente}', 'manual') AS fuente,
              COUNT(*) AS total
         FROM logs_auditoria
        WHERE accion = 'consequencePreview'
          AND timestamp >= NOW() - INTERVAL '24 hours'
    ...
    for (const r of fuenteRows) porFuente[r.fuente] = Number(r.total);

    // 4. Cooldowns activos
    const cooldownRows: Array<{ entity_type: string; evento_tipo: string; total: string; oldest: Date }> = (await sqlDb.raw(
      `SELECT entity_type, evento_tipo, COUNT(*) AS total, MIN(fired_at) AS oldest
    ...
    const atendidosRow = await sqlDb('logs_auditoria')
      .where('accion', 'consequencePreview')
    ...
    const noAtendidosRow = await sqlDb('logs_auditoria')
      .where('accion', 'consequencePreview')
      ...
      .andWhere('timestamp', '>=', sqlDb.raw("NOW() - INTERVAL '24 hours'"))
      .count<{ count: string }>({ count: '*' })
      .first();
    ...
    logger.error('[motor/health]', { error: String(err) });
