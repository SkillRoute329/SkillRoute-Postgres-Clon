...
interface OperatorMetricSummary {
  value: number | null;
  n: number;
  ic95?: [number, number];
  badge: 'OK' | 'IC_VISIBLE' | 'INSUFFICIENT' | 'NO_COVERAGE';
  applicable: boolean;
}

interface OperatorSummaryOut {
  agencyId: string;
  name: string;
  totalEvents: number;
  totalLines: number;
  lineCount: number;
  coverageGps: number;
  services: { value: number; type: 'medido' | 'estimado' };
...
// ─── Helpers ──────────────────────────────────────────────────────────────
function badgeFromN(n: number, coverageGps: number): OperatorMetricSummary['badge'] {
  if (coverageGps < 30) return 'NO_COVERAGE';
  if (n < MIN_N_FOR_BADGE_OK) return 'INSUFFICIENT';
  if (n < 200) return 'IC_VISIBLE';
  return 'OK';
}

/** Intervalo de confianza 95% para una proporción Bernoulli (Wilson aprox). */
function ic95Proportion(p: number, n: number): [number, number] {
  if (n === 0) return [0, 0];
  const z = 1.96;
  const denom = 1 + (z * z) / n;
...
async function computeOperatorMetrics(
  ...
  const eventsAggrPromise = sqlDb('mv_fleet_ranking_diario')
    .where('agency_id', agencyId)
    .whereBetween('fecha', [from, to])
    .select(
      sqlDb.raw('COALESCE(SUM(total),0) AS total_events'),
      sqlDb.raw('COUNT(DISTINCT id_bus) AS active_buses'),
      sqlDb.raw('COALESCE(SUM(en_tiempo + atrasado + adelantado),0) AS total_with_schedule'),
      sqlDb.raw('COALESCE(SUM(en_tiempo),0) AS en_tiempo'),
      sqlDb.raw(
        '(SELECT COUNT(DISTINCT l) FROM mv_fleet_ranking_diario f2, ' +
          'LATERAL unnest(f2.lineas) AS l WHERE f2.agency_id = ? ' +
          'AND f2.fecha BETWEEN ?::date AND ?::date) AS total_lines',
        [agencyId, from, to],
      ),
    )
    .first<{
      ...
      total_lines: string;
      active_buses: string;
      total_with_schedule: string;
      en_tiempo: string;
    }>();

  // Cobertura GPS — query separada (otra tabla)
  const coveragePromise = sqlDb('v_poller_coverage_diario')
    .where('agency_id', agencyId)
    .whereBetween('fecha', [from, to])
    .avg<Array<{ avg: string | null }>>({ avg: 'pct_cobertura_estimado' })
    .first();

  // Flota total — query separada
  const fleetPromise = sqlDb('vehiculos')
    .where('agency_id', agencyId)
  ...
  const otp: OperatorMetricSummary = {
  ...
  const ewt: OperatorMetricSummary = {
    value: null,
    n: 0,
    badge: 'INSUFFICIENT',
    applicable: false,
  ...
  const serviceDelivered: OperatorMetricSummary = {
  ...
  const srsValue =
    otpValue !== null && coverageGps > 0
      ? Number(((otpValue * 100 * 0.7 + coverageGps * 0.3) / 1).toFixed(2))
      : null;
  const srs: OperatorMetricSummary = {
    value: srsValue,
    n: totalSched,
    badge: badgeFromN(totalSched, coverageGps),
    applicable: true,
 ...
 */
export async function getRegulatoryData(req: Request, res: Response): Promise<void> {
  try {
    ...
    const empresa = ((req.query.empresa as string) || 'all').trim();
    const desde = ((req.query.desde as string) || today).trim();
    const hasta = ((req.query.hasta as string) || today).trim();
    const granularidad = ((req.query.granularidad as string) || 'diaria').trim();
    ...
    const operators: OperatorSummaryOut[] = await Promise.all(
      operatorsToCompute.map((ag) => computeOperatorMetrics(ag, desde, hasta)),
    );
    ...
    logger.error('[Compliance] regulador error', { error: msg });
 ...
 */
export async function getOperatorData(req: Request, res: Response): Promise<void> {
  try {
    const today = new Date().toISOString().slice(0, 10);
    const sevenDaysAgo = new Date(Date.now() - 7 * 86_400_000).toISOString().slice(0, 10);

    const agencyId = ((req.query.agencyId as string) || '70').trim();
    const desde = ((req.query.desde as string) || sevenDaysAgo).trim();
    const hasta = ((req.query.hasta as string) || today).trim();
    const granularidad = ((req.query.granularidad as string) || 'diaria').trim();

    // Cobertura del operador
    const coverageRow = await sqlDb('v_poller_coverage_diario')
      .where('agency_id', agencyId)
    ...
    const totalEventsRow = await sqlDb('mv_cumplimiento_linea_diario')
      .where('agency_id', agencyId)
      .whereBetween('fecha', [desde, hasta])
      .sum<Array<{ sum: string | null }>>({ sum: 'total' })
      .first();
    const totalEvents = parseInt(totalEventsRow?.sum ?? '0', 10);

    // Por línea: OTP y service_delivered desde la MV diaria agrupada.
    const linesRaw = await sqlDb('mv_cumplimiento_linea_diario')
      .where('agency_id', agencyId)
      .whereBetween('fecha', [desde, hasta])
      .whereNotNull('linea')
      .groupBy('linea')
      .select('linea')
      .select(sqlDb.raw('COALESCE(SUM(total),0) AS total_events'))
      .select(sqlDb.raw('COALESCE(SUM(con_horario),0) AS total_with_schedule'))
      .select(sqlDb.raw('COALESCE(SUM(en_tiempo),0) AS en_tiempo'))
      .orderBy('linea');

    const lines = linesRaw.map((r: any) => {
      ...
      const otpValue = totalSched > 0 ? enTiempo / totalSched : null;
      const otpBadge: OperatorMetricSummary['badge'] =
        operatorGps < 30
          ? 'NO_COVERAGE'
      ...
      return {
        ...
        metrics: {
          ...
          ewt: { value: null, n: 0, badge: 'INSUFFICIENT' as const },
          serviceDelivered: { value: null, n: 0, badge: 'INSUFFICIENT' as const },
          srs: {
            value: otpValue !== null ? Number((otpValue * 100).toFixed(2)) : null,
            n: totalSched,
            badge: otpBadge,
    ...
    const msg = error instanceof Error ? error.message : String(error);
    logger.error('[Compliance] operador error', { error: msg });
    res.status(500).json({ ok: false, error: msg });
  }
}
