...
async function refreshOnce(): Promise<void> {
  if (running) {
    logger.warn('[fleetRankingMv] refresh anterior aún en curso (este proceso), salteando');
    return;
  }
  ...
  try {
    ...
    if (n > 0) {
      logger.warn(`[fleetRankingMv] ${n} REFRESH de mantenimiento aún activo en Postgres, salteando ciclo (anti-apilado)`);
      return;
    }
  } catch (e) {
    logger.warn('[fleetRankingMv] no se pudo chequear pg_stat_activity, continúo', { err: String(e) });
  }
  running = true;
  const t0 = Date.now();
  try {
    ...
    await sqlDb.raw(
      'SET statement_timeout = 0; REFRESH MATERIALIZED VIEW CONCURRENTLY mv_fleet_ranking_diario',
    );
    logger.info(`[fleetRankingMv] refrescada en ${Date.now() - t0}ms`);
  } catch (e) {
    logger.error('[fleetRankingMv] error en refresh', { err: String(e) });
  } finally {
    running = false;
  }
  ...
  try {
    const t2 = Date.now();
    await sqlDb.raw(
      'SET statement_timeout = 0; REFRESH MATERIALIZED VIEW CONCURRENTLY mv_cumplimiento_linea_diario',
    );
    logger.info(`[cumplLineaMv] refrescada en ${Date.now() - t2}ms`);
  } catch (e) {
    logger.error('[cumplLineaMv] error en refresh', { err: String(e) });
  }
  ...
  if (ticks % TICKS_POR_DIA === 1) {
    try {
      const t1 = Date.now();
      await sqlDb.raw(
        'SET statement_timeout = 0; REFRESH MATERIALIZED VIEW mv_oferta_linea_hora',
      );
      logger.info(`[mvOferta] mv_oferta_linea_hora refrescada en ${Date.now() - t1}ms`);
    } catch (e) {
      logger.error('[mvOferta] error en refresh', { err: String(e) });
    }
  }
}

export function startFleetRankingMvRefresh(): void {
  ...
  setTimeout(() => { void refreshOnce(); }, 3 * 60_000);
  timer = setInterval(() => { void refreshOnce(); }, 10 * 60_000);
}

export function stopFleetRankingMvRefresh(): void {
