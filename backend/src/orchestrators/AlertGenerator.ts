...
interface AnalysisData {
  tiempo_desviacion?: number;
  frecuencia_real?: number | null;
  frecuencia_teorica?: number | null;
...
interface Monitor {
  competitorId: string;
  competitorNombre: string;
  competitorEmpresa: string;
  codigoStm: number | null;
  corredoresCompartidos: string[];
}
...
interface CompetitorData {
  ...
  recorrido?: string;
  sentido?: string;
  tiempo_ventaja?: number | null;
  distancia_metros?: number | null;
  frecuencia_rival?: number | null;
  unidades_detectadas?: number | null;
}
...
class AlertGenerator {
   ...
   */
  static generate(ecosystem: Ecosystem, alertData: AlertData): any {
   ...
   */
  static generateFromAnalysis(ecosystem: Ecosystem, analyzer: Analyzer, analysisData: AnalysisData): any {
   ...
   */
  static generateFromCompetitor(ecosystem: Ecosystem, monitor: Monitor, competitorData: CompetitorData): any {
   ...
   */
  private static _classifyCompetitorAlert(competitorData: CompetitorData): string {
   ...
   */
  private static _generateCompetitorActions(monitor: Monitor, competitorData: CompetitorData): string[] {
   ...
   */
  private static _getCompetitorMessage(competitorData: CompetitorData): string {
   ...
   */
  private static _calculateSeverity(alertData: AlertData): string {
    if (!alertData.tiempo_minutos) return 'MEDIA';

    const tiempo = Math.abs(alertData.tiempo_minutos);
    if (tiempo > 15) return 'CRÍTICA';
    if (tiempo > 8) return 'ALTA';
    if (tiempo > 3) return 'MEDIA';
    return 'BAJA';
  }
   ...
   */
  private static _calculateSeverityFromMetrics(analysisData: AnalysisData): string {
    const desv = Math.abs(analysisData.tiempo_desviacion || 0);
    const tasaPuntualidad = analysisData.tasa_puntualidad || 100;

    if (desv > 15 || tasaPuntualidad < 50) return 'CRÍTICA';
    if (desv > 8 || tasaPuntualidad < 70) return 'ALTA';
    if (desv > 3 || tasaPuntualidad < 85) return 'MEDIA';
    return 'BAJA';
  }
   ...
   */
  private static _calculateCompetitorSeverity(competitorData: CompetitorData): string {
    const tiempo = competitorData.tiempo_ventaja || 0;
    const unidades = competitorData.unidades_detectadas || 0;
    const evento = competitorData.tipo_evento?.toLowerCase() || '';

    if (evento.includes('roto')) return 'CRÍTICA'; // Oportunidad crítica
    if (tiempo > 10 || unidades > 3) return 'ALTA';
    if (tiempo > 5 || unidades > 1) return 'MEDIA';
    return 'BAJA';
  }
}
