  ...
  try {
    ...
    if (!snapNum.empty) {
      ...
      const snapLeg = await db
        ...
        .limit(1)
        .get();
      if (!snapLeg.empty) userDoc = snapLeg.docs[0];
    }
...
app.get('/api/doctor', async (_req, res) => {
  try {
    ...
    res.json({
      ...
      database: {
        connected: true,
        vehicleCount: (await db.collection('vehicles').get()).size,
        cartonCount: (await db.collection('cartones_completados').get()).size,
      },
...
app.get('/api/cartones', async (_req, res) => {
  try {
    const snap = await db.collection('cartones_completados').get();
...
app.get('/api/cartones/:id', async (req, res) => {
  try {
    const doc = await db.collection('cartones_completados').doc(req.params.id).get();
    if (!doc.exists) return res.status(404).json({ error: 'Cartón no encontrado' });
...
app.post('/api/fleet/check', verifyAuth, async (req: AuthRequest, res) => {
  try {
    ...
    const checkRef = db.collection('fleet_checks').doc();
    const payload = {
...
app.post('/api/simulation/reset', verifyAuth, async (req: AuthRequest, res) => {
...
app.post('/api/cartones', verifyAuth, async (req: AuthRequest, res) => {
  try {
    const data = req.body;
    if (!data.serviceNumber) return res.status(400).json({ error: 'serviceNumber requerido' });
