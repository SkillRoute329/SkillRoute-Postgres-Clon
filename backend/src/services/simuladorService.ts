...
const DIAS_TIPO: Record<string, number> = { habil: 22, sabado: 4, festivo: 4 };

export interface Escenario {
  deltaVehiculosPct?: number; // +20 = 20% más coches
  headwayObjetivoMin?: number; // fija un headway objetivo
  capacidadBus?: number; // supuesto, default 90
}
...
export interface SimulacionResultado {
  ...
  // FASE 5.20 — EFECTO RED: cuántos pasajeros de esta línea están
  // conectados al resto de la red por transbordo (matriz OD real STM,
  // id_viaje). Es el "modelado" acotado con DATO MEDIDO: un cambio en
  // esta línea NO es aislado, propaga a las líneas conectadas.
  efectoRed: {
    mes: string | null;
...
export async function simularEscenarioLinea(
  linea: string,
  ...
  const mesRow = (await sqlDb('mv_stm_demanda_linea_hora').max('mes as m').first()) as
    | { m: string }
    | undefined;
  ...
  const demRows = mes
    ? ((await sqlDb('mv_stm_demanda_linea_hora')
        ...
        .select('hora', 'pasajeros', 'cod_empresa')) as Array<{
        hora: number;
        pasajeros: string;
        cod_empresa: number;
      }>)
    : [];
  const paxMes = new Map<number, number>();
  let codEmp: number | null = null;
  ...
  const dows =
    tipoDia === 'habil' ? [1, 2, 3, 4, 5] : tipoDia === 'sabado' ? [6] : [0];
  const lineasGps =
    agencyId === '70' && /^\d{1,2}$/.test(String(linea))
      ? [String(linea), `3${linea}`]
      : [String(linea)];
  let vehRows: Array<{ hora: number; veh_hora_prom: string }> = [];
  try {
    vehRows = (await sqlDb('mv_oferta_linea_hora')
      .whereIn('linea', lineasGps)
      .whereIn('dow', dows)
      .select('hora')
      .avg({ veh_hora_prom: 'veh_hora_prom' })
      .groupBy('hora')) as Array<{ hora: number; veh_hora_prom: string }>;
  } catch (e) {
    logger.warn('[simulador] oferta MV no disponible', { linea, err: String(e) });
  }
  const vehHora = new Map<number, number>();
  for (const r of vehRows) vehHora.set(Number(r.hora), Number(r.veh_hora_prom) || 0);

  const filas: FilaHora[] = [];
  for (let h = 5; h <= 23; h++) {
    const paxDia = Math.round((paxMes.get(h) ?? 0) / diasTipo);
    const vBase = Math.round((vehHora.get(h) ?? 0) * 10) / 10;
    if (paxDia === 0 && vBase === 0) continue;
  ...
  const sum = (f: (x: FilaHora) => number) => filas.reduce((a, x) => a + f(x), 0);
  const avg = (f: (x: FilaHora) => number) =>
    filas.length ? Math.round((sum(f) / filas.length) * 100) / 100 : 0;
  ...
  const odMesRow = (await sqlDb('stm_transbordos_mensual').max('mes as m').first()) as
    | { m: string }
    | undefined;
  ...
  const efectoRed = {
    ...
    transfTotalMes: 0,
    alimentanEstaLinea: [] as Array<{ linea: string; transbordos: number }>,
    estaLineaAlimenta: [] as Array<{ linea: string; transbordos: number }>,
    nota:
      'Transbordos reales (cadena de id_viaje del documento STM). Un cambio en esta línea NO es aislado: propaga a estas líneas conectadas.',
  };
  if (odMes) {
    try {
      // Pares OD del mes; se cruza la línea con mismaLinea (prefijo UCOT).
      const pares = (await sqlDb('stm_transbordos_mensual')
        .where('mes', odMes)
      ...
      for (const p of pares) {
        ...
        const n = Number(p.tb);
        if (mismaLinea(d, String(linea)) && !mismaLinea(o, String(linea))) {
          entran.set(o, (entran.get(o) ?? 0) + n);
          tot += n;
        }
        if (mismaLinea(o, String(linea)) && !mismaLinea(d, String(linea))) {
      ...
      logger.warn('[simulador] efectoRed OD no disponible', { linea, err: String(e) });
    }
  }
  ...
  return {
    ...
    operador:
      codEmp != null
        ? ({ 10: 'COETC', 20: 'COME', 50: 'CUTCSA', 70: 'UCOT' } as Record<number, string>)[codEmp] ??
          String(codEmp)
        : null,
    tipoDia,
