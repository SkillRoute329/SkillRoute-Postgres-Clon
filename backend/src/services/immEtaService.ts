...
export async function getImmToken(): Promise<string> {
  ...
  if (!id || !secret) throw new Error('Faltan IMM_CLIENT_ID/SECRET');
 ...
 */
export async function snapshotEtaParada(codigoParada: string): Promise<number> {
  if (process.env.IMM_ETA_ENABLED !== 'true' || !process.env.IMM_ETA_PATH) {
    logger.warn(
      '[immEta] deshabilitado: setear IMM_ETA_ENABLED=true e IMM_ETA_PATH ' +
  ...
  const resp = await new Promise<{ status: number; body: string }>((resolve) => {
    const r = https.request(url, { headers: { Authorization: `Bearer ${token}` } }, (x) => {
      let d = '';
      x.on('data', (c) => (d += c));
  ...
  if (resp.status !== 200) {
    logger.error(`[immEta] HTTP ${resp.status} en ${url}`, { body: resp.body.slice(0, 200) });
    return 0;
  }
  ...
  let n = 0;
  for (const a of arr) {
    await sqlDb('stm_eta_snapshots').insert({
  ...
  logger.info(`[immEta] parada ${codigoParada}: ${n} arribos snapshot`);
  return n;
}
