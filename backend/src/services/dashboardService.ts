...
class DashboardService {
   ...
   */
  async generarDashboardEjecutivo(operador: string): Promise<DashboardEjecutivo> {
    try {
      ...
      const [lineas, metricas, competencia, cartonesMarginales, lineasEnRiesgo, pronósticos] = await Promise.all([
        ...
        analyticsService.detectarCartonesMarginales(operador),
        analyticsService.identificarLineasEnRiesgo(operador),
      ...
      const resumen_texto = this.generarResumenEjecutivo(
        metricas,
        salud,
        competencia,
        recomendaciones
   ...
   */
  private async obtenerEstadoLineas(operador: string): Promise<EstadoLinea[]> {
    try {
      ...
      const estados: EstadoLinea[] = [];

      for (const lineaDoc of lineasSnapshot.docs) {
        const lineaData = lineaDoc.data() as any;

        // Obtener análisis de competencia
        const analisisCompetencia = await competitionService.analizarCompetitividad(lineaDoc.id);
        ...
        for (const cartoonDoc of cartonesSnapshot.docs) {
          const viabilidad = await analyticsService.validarCartoon(cartoonDoc.id);
        ...
        const estado: EstadoLinea = {
          ...
          ocupacion: 75,
          competencia: [],
          alertas: this.generarAlertasLinea(
            lineaData.numero,
            analisisCompetencia,
            cartonesMarginal
          ),
          recomendacion: this.generarRecomendacionLinea(
            analisisCompetencia,
            cartonesMarginal
          )
        };
      ...
      logger.error(`Error obteniendo estado de líneas: ${error}`);
      throw error;
    }
  }
   ...
   */
  private async calcularMetricas(operador: string): Promise<DashboardMetricas> {
    try {
      const lineasSnapshot = await db
        .collection('lineas')
        .where('operador', '==', operador)
   ...
   */
  private async obtenerResumenCompetitivo(operador: string): Promise<any> {
    try {
      const lineasSnapshot = await db
        .collection('lineas')
        .where('operador', '==', operador)
      ...
      let amenazas = 0;
      let oportunidades = 0;
      const lineasCompetidas = new Set<string>();

      for (const lineaDoc of lineasSnapshot.docs) {
        const analisis = await competitionService.analizarCompetitividad(lineaDoc.id);
      ...
      logger.error(`Error obteniendo resumen competitivo: ${error}`);
      throw error;
    }
  }
   ...
   */
  private async calcularProyecciones(operador: string): Promise<ProyeccionIngresos[]> {
    return [
      {
        periodo: 'Este mes',
   ...
   */
  private calcularSaludOperacional(
    lineas: EstadoLinea[],
    cartonesNoViables: number
  ): SaludOperacional {
    ...
    const indiceGeneral = Math.round(
      porcentajeOperativas * 0.5 - porcentajeEnRiesgo * 0.3 - porcentajeNoViables * 0.2
    );

    let estado: 'excelente' | 'bueno' | 'regular' | 'critico';
    if (indiceGeneral >= 80) estado = 'excelente';
    else if (indiceGeneral >= 60) estado = 'bueno';
    else if (indiceGeneral >= 40) estado = 'regular';
    else estado = 'critico';
   ...
   */
  private generarAlertasLinea(
    numeroLinea: number,
    analisisCompetencia: any,
   ...
   */
  private determinarEstadoLinea(pasajerosEnRiesgo: number, cartonesMarginales: number): string {
    if (pasajerosEnRiesgo > 200 || cartonesMarginales > 2) return 'critica';
    if (pasajerosEnRiesgo > 100 || cartonesMarginales > 0) return 'riesgo';
    if (cartonesMarginales > 0) return 'marginal';
    return 'operativa';
  }
   ...
   */
  private generarRecomendacionLinea(analisisCompetencia: any, cartonesMarginales: number): string {
    if (cartonesMarginales > 0) {
