...
function tipoDia(dow: number): 'habil' | 'sabado' | 'festivo' {
...
export interface FilaProyeccion {
  ...
  linea: string;
  hora: number;
  demandaEsperada: number; // validaciones proyectadas
  promedioHistorico: number;
  tendenciaPctMes: number; // % de cambio mes a mes (señal corto/mediano plazo)
 ...
 */
export async function proyectarDemanda(
  fechaObjetivo: string,
  agencyId?: string,
): Promise<ProyeccionResultado> {
  ...
  const mesesRows = (await sqlDb('mv_stm_linea_resumen')
    .distinct('mes')
    .orderBy('mes', 'asc')) as Array<{ mes: Date | string }>;
  ...
  const params: unknown[] = [...dows];
  const filtroOp = agencyId ? 'AND cod_empresa = ?' : '';
  if (agencyId) params.push(Number(agencyId));
  ...
  const rows = (await sqlDb.raw(
    `WITH base AS (
     ...
     HAVING avg(v) >= 50
      ...
      LIMIT 300`,
    [...params, ultimoMes, primerMes] as unknown[] as string[],
  )).rows as Array<{
    cod_empresa: number;
    dsc_linea: string;
    hora: number;
    prom_hist: string;
    prom_ultimo: string | null;
    prom_primero: string | null;
  }>;
  ...
  for (const r of rows) {
    ...
    const tendPctMes =
      pri > 0 && nMeses > 1 ? Math.round(((ult - pri) / pri / (nMeses - 1)) * 1000) / 10 : 0;
  ...
  logger.info(
    `[proyeccion] ${fechaObjetivo} (${td}): ${proyeccion.length} líneas, ${alertas} alertas anticipadas`,
  );
  return {
    ok: true,
    fechaObjetivo,
