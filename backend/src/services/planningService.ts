...
export interface BarrioDemography {
  nombre: string;
  population: number;
  avgIncome: number; // en UYU
  avgAge: number;
  povertyRate: number; // en %
  polygon: [number, number][]; // [lat, lng] array
}

// 11 barrios clave de Montevideo con límites aproximados y datos demográficos realistas
export const MONTEVIDEO_BARRIOS: BarrioDemography[] = [
  {
    nombre: 'Ciudad Vieja',
    population: 14000,
    avgIncome: 45000,
...
export class PlanningService {
   ...
   */
  isPointInPolygon(point: LatLng, polygon: [number, number][]): boolean {
    const x = point.lng;
    const y = point.lat;
    let inside = false;
   ...
   */
  calculateRouteLength(points: LatLng[]): number {
    let totalLength = 0;
   ...
   */
  getCrossedBarrios(points: LatLng[], paradas: Stop[]): BarrioDemography[] {
    const crossed = new Set<BarrioDemography>();

    // Verificar las paradas
    for (const stop of paradas) {
      if (stop.lat === 0 || stop.lng === 0) continue;
      const found = MONTEVIDEO_BARRIOS.find((barrio) =>
        this.isPointInPolygon({ lat: stop.lat, lng: stop.lng }, barrio.polygon)
      );
      if (found) {
        crossed.add(found);
      }
    ...
    for (let i = 0; i < points.length; i += Math.max(1, Math.floor(points.length / 20))) {
      const pt = points[i];
      const found = MONTEVIDEO_BARRIOS.find((barrio) =>
        this.isPointInPolygon(pt, barrio.polygon)
      );
      if (found) {
        crossed.add(found);
      }
    ...
    if (crossed.size === 0 && paradas.length > 0) {
      ...
      let closestBarrio: BarrioDemography | null = null;

      for (const barrio of MONTEVIDEO_BARRIOS) {
   ...
   */
  analyzeEquity(points: LatLng[], paradas: Stop[], frecuenciaDiaria: number) {
  ...
  calculateFinancialImpact(
    ...
    costoKmOperativo = 90, // UYU por km
