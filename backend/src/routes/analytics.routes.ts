...
import { getEtapasPrincipales, norm } from './etapaStats.routes';
 ...
 */
router.get('/run-times/:agencyId/:linea', async (req: Request, res: Response) => {
  try {
    const { agencyId, linea } = req.params;
    const days = Math.min(30, Math.max(1, parseInt((req.query.days as string) || '3', 10)));
    const sentidoRaw = String((req.query.sentido as string) || '').toUpperCase();
    const sentido: 'IDA' | 'VUELTA' | null =
      sentidoRaw === 'IDA' ? 'IDA' : sentidoRaw === 'VUELTA' ? 'VUELTA' : 'IDA'; // default a IDA

    // 1. Obtener etapas principales de la línea
    const { etapas } = getEtapasPrincipales(agencyId, linea, 0.3);
    if (etapas.size === 0) {
    ...
    const tripsEvents: Record<string, Record<string, number>> = {};
    for (const r of rows) {
      const tId = r.trip_id;
      const stopNameNorm = norm(r.proxima_parada);
      const ts = new Date(r.timestamp_gps).getTime();

      // Match tolerante por tokens con etapas principales
      let matchedEtapaKey: string | null = null;
      ...
      if (matchedEtapaKey) {
        if (!tripsEvents[tId]) tripsEvents[tId] = {};
    ...
    const segmentsAccumulator: Record<string, number[]> = {};
    ...
    for (const tId of Object.keys(tripsEvents)) {
      ...
      for (let i = 0; i < etapasOrdenadas.length - 1; i++) {
        ...
        const keyA = norm(stopA.nombre);
        const keyB = norm(stopB.nombre);
        ...
        if (timeA !== undefined && timeB !== undefined && timeB > timeA) {
          ...
          if (durationMinutes > 0 && durationMinutes < 60) {
            ...
            if (!segmentsAccumulator[segKey]) segmentsAccumulator[segKey] = [];
    ...
    for (let i = 0; i < etapasOrdenadas.length - 1; i++) {
      ...
      const parseArrivalMins = (arrStr: string): number => {
      ...
      if (scheduledMins < 0) scheduledMins += 1440; // Cruce medianoche
    ...
    logger.error('[analytics/run-times]', { error: String(err) });
 ...
 */
router.get('/stop-dwell/:agencyId/:linea', async (req: Request, res: Response) => {
  try {
    const { agencyId, linea } = req.params;
    ...
    const flushSequence = (key: string, seq: number[]) => {
      if (seq.length > 0) {
        const start = seq[0];
    ...
    const dwellTimes = Object.entries(dwellMap).map(([stopName, durations]) => {
      ...
      let congestionLevel: 'BAJO' | 'MEDIO' | 'ALTO' = 'BAJO';
      if (avgDwellSeconds > 90) congestionLevel = 'ALTO';
      else if (avgDwellSeconds > 35) congestionLevel = 'MEDIO';
    ...
    logger.error('[analytics/stop-dwell]', { error: String(err) });
