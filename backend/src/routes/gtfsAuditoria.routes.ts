...
let stopsGeo: StopsGeo | null = null;
...
let stopsCombined: Record<string, { name: string; lat: number; lon: number }> | null = null;
let lastLoad = 0;
...
function loadIfStale(): void {
  if (scheduleIndex && Date.now() - lastLoad < RELOAD_MS) return;
  const base = path.resolve(__dirname, '..', 'data', 'gtfs');
  try {
    scheduleIndex = JSON.parse(fs.readFileSync(path.join(base, 'schedule_index.json'), 'utf8')) as ScheduleIndex;
    stopsGeo = JSON.parse(fs.readFileSync(path.join(base, 'stops_geo.json'), 'utf8')) as StopsGeo;
    ...
    const combined: Record<string, { name: string; lat: number; lon: number }> = {};
 ...
 */
router.get('/db/gtfs_timetable/:id', (req: Request, res: Response) => {
  loadIfStale();
  if (!scheduleIndex) {
  ...
  if (!ag) return res.json({ ok: true, data: null });
  const route = ag.routes[linea];
  if (!route) return res.json({ ok: true, data: null });

  const day = svcType === 'HABIL' ? route.habiles : svcType === 'SABADO' ? route.sabados : route.domingos;
  if (!day || day.length === 0) return res.json({ ok: true, data: null });
  ...
  const viajes = trips.map((t) => {
    const tArr: number[] = new Array(stopIds.length).fill(-1);
    for (const cs of t.control_stops) {
 ...
 */
router.get('/db/gtfs_stops/:stopId', (req: Request, res: Response) => {
  loadIfStale();
  if (!stopsCombined) {
