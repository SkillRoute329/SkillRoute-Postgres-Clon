 ...
 */
router.get('/eta', verifyAuth, async (req: Request, res: Response) => {
  ...
  try {
    ...
    const prediction = await sqlDb('bus_eta_predictions as p')
    ...
    if (prediction.data_jsonb && typeof prediction.data_jsonb === 'object') {
      scheduledDeviationMin = (prediction.data_jsonb as any).desviacionMin ?? 0;
    ...
    const t0 = Date.now();
    const etaMl = PredictionEngine.getInstance().predict(features);
    ...
    logger.error('[Predictions Route] Error calculando ETA:', error);
    res.status(500).json({
      status: 500,
 ...
 */
router.get('/metrics', verifyAuth, async (req: Request, res: Response) => {
  try {
    ...
    const activeRaw = await sqlDb('bus_eta_predictions as p')
      .leftJoin('bus_last_pos as l', 'p.id_bus', 'l.id_bus')
      .select(
        'p.id_bus',
        'p.stop_id',
    ...
    logger.error('[Predictions Route] Error obteniendo métricas:', error);
    res.status(500).json({
      status: 500,
 ...
 */
router.post('/retrain', verifyAuth, async (req: Request, res: Response) => {
  try {
    logger.info('[Predictions API] Solicitado re-entrenamiento manual de ML.');
    ...
    logger.error('[Predictions Route] Error en re-entrenamiento manual:', error);
    res.status(500).json({
      status: 500,
