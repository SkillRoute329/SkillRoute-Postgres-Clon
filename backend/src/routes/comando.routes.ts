...
import { generarRecomendaciones } from '../services/recomendacionesService';
import { proyectarDemanda } from '../services/proyeccionService';
import { simularEscenarioLinea } from '../services/simuladorService';
import { generarDiagnosticoLineas } from '../services/diagnosticoLineaService';
import { generarFugaParadas } from '../services/fugaParadasService';
 ...
 */
let _frecCache: { at: number; data: Record<string, number> } | null = null;
router.get('/frecuencias-gtfs', async (_req: Request, res: Response) => {
  try {
    if (_frecCache && Date.now() - _frecCache.at < 6 * 3600_000) {
    ...
    const rows = (q.rows ?? q) as Array<{ linea: string; headway_min: string | null }>;
    const data: Record<string, number> = {};
    for (const r of rows) {
      const h = Number(r.headway_min);
      if (r.linea && h > 0 && h <= 180) data[String(r.linea)] = h;
    }
    ...
    logger.error('[comando/frecuencias-gtfs]', { error: String(err) });
 ...
 */
router.get('/demanda-real', async (req: Request, res: Response) => {
  try {
    ...
    const mesRow = (await sqlDb('mv_stm_demanda_linea').max('mes as m').first()) as
      | { m: string }
      | undefined;
    if (!mesRow?.m) {
    ...
    const mes = (req.query.mes as string) || mesRow.m;
    let q = sqlDb('mv_stm_demanda_linea')
      .where('mes', mes)
      .select('cod_empresa', 'dsc_linea', 'pasajeros', 'validaciones', 'variantes')
      .orderBy('pasajeros', 'desc')
      .limit(limit);
    if (req.query.op) q = q.where('cod_empresa', Number(req.query.op));
    const rows = await q;
    res.json({ ok: true, mes, fuente: 'STM oficial (pasajeros reales)', total: rows.length, lineas: rows });
  } catch (err) {
    logger.error('[comando/demanda-real]', { error: String(err) });
    res.status(500).json({ ok: false, error: String(err) });
  }
});
 ...
 */
router.get('/od', async (req: Request, res: Response) => {
  try {
    ...
    const mes = (req.query.mes as string) || mesRow.m;
    let q = sqlDb('stm_transbordos_mensual')
      .where('mes', mes)
    ...
    const rows = await q;
    const crossOp = rows.filter(
      (r: { cod_empresa_o: number; cod_empresa_d: number }) =>
        r.cod_empresa_o && r.cod_empresa_d && r.cod_empresa_o !== r.cod_empresa_d,
    ).length;
    res.json({
      ok: true,
      mes,
    ...
    logger.error('[comando/od]', { error: String(err) });
    res.status(500).json({ ok: false, error: String(err) });
  }
});
 ...
 */
router.get('/recomendaciones', async (req: Request, res: Response) => {
  try {
    const fecha = (req.query.fecha as string) || new Date().toISOString().slice(0, 10);
    const ops = ((req.query.operadores as string) || '70,50,20,10')
      .split(',')
      .map((s) => s.trim())
      .filter(Boolean);
    const out = await generarRecomendaciones(fecha, ops);
    res.json(out);
  } catch (err) {
    logger.error('[comando/recomendaciones]', { error: String(err) });
    res.status(500).json({ ok: false, error: String(err) });
  }
});
 ...
 */
router.get('/shape-linea/:linea', async (req: Request, res: Response) => {
  try {
    ...
    const rows = (q.rows ?? q) as Array<{ lat: number; lon: number }>;
    const puntos = rows
    ...
    logger.error('[comando/shape-linea]', { error: String(err) });
    res.status(500).json({ ok: false, error: String(err) });
  }
});
 ...
 */
router.get('/fuga-paradas', async (req: Request, res: Response) => {
  try {
    const op = String(req.query.op || '70').trim();
    res.json(await generarFugaParadas(op));
  } catch (err) {
    logger.error('[comando/fuga-paradas]', { error: String(err) });
    res.status(500).json({ ok: false, error: String(err) });
  }
});
 ...
 */
router.get('/diagnostico-linea', async (req: Request, res: Response) => {
  try {
    const op = String(req.query.op || '70').trim();
    const out = await generarDiagnosticoLineas(op);
    res.json(out);
  } catch (err) {
    logger.error('[comando/diagnostico-linea]', { error: String(err) });
    res.status(500).json({ ok: false, error: String(err) });
  }
});
...
router.get('/proyeccion', async (req: Request, res: Response) => {
  try {
    ...
    const fecha = (req.query.fecha as string) || manana;
    const agencyId = (req.query.agency_id as string) || undefined;
    const out = await proyectarDemanda(fecha, agencyId);
    res.json(out);
  } catch (err) {
    logger.error('[comando/proyeccion]', { error: String(err) });
    res.status(500).json({ ok: false, error: String(err) });
  }
});
 ...
 */
router.get('/simulador', async (req: Request, res: Response) => {
  try {
    ...
    const tipoDia = (['habil', 'sabado', 'festivo'] as const).includes(
      req.query.tipo_dia as 'habil' | 'sabado' | 'festivo',
    )
      ? (req.query.tipo_dia as 'habil' | 'sabado' | 'festivo')
      : 'habil';
    const esc = {
      deltaVehiculosPct:
        req.query.deltaVehiculosPct != null ? Number(req.query.deltaVehiculosPct) : undefined,
      headwayObjetivoMin:
        req.query.headwayObjetivoMin != null ? Number(req.query.headwayObjetivoMin) : undefined,
      capacidadBus: req.query.capacidadBus != null ? Number(req.query.capacidadBus) : undefined,
    };
    const out = await simularEscenarioLinea(
      linea,
      tipoDia,
      esc,
      req.query.op ? String(req.query.op) : undefined,
    );
    res.json(out);
  } catch (err) {
    logger.error('[comando/simulador]', { error: String(err) });
    res.status(500).json({ ok: false, error: String(err) });
  }
});
 ...
 * operación").
 */
router.get('/trafico-linea/:linea', async (req: Request, res: Response) => {
  try {
    const linea = String(req.params.linea).trim();
    const sensores = (await sqlDb('sensor_linea_prox')
      .where('linea', linea)
      .select('cod_detector')) as Array<{ cod_detector: number }>;
    ...
    const filas = (await sqlDb('velocidad_vehicular')
    ...
    const horas = filas.map((f) => {
      ...
      const estado =
        ratio < 0.6 ? 'CONGESTION_SEVERA' : ratio < 0.8 ? 'CONGESTION' : 'FLUIDO';
      return {
        hora: f.hora,
        velPromKmh: vp,
    ...
    const veredicto =
      congest.length === 0
        ...
        : `Línea ${linea}: ${congest.length} franja(s) con congestión real (velocidad comercial cae a ~${Math.min(
            ...congest.map((h) => h.velPromKmh),
          )} km/h vs típica ${Math.round(tipica * 10) / 10}). El atraso en esas franjas es de origen EXÓGENO (tráfico), no operativo — defendible ante IMM.`;
    res.json({
      ok: true,
      linea,
    ...
    logger.error('[comando/trafico-linea]', { error: String(err) });
    res.status(500).json({ ok: false, error: String(err) });
  }
});
