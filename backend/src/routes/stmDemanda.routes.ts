...
router.get('/meses', async (_req: Request, res: Response) => {
  try {
    const payload = await cached('stm-demanda:meses', 60_000, async () => {
      const rows = await sqlDb('stm_validaciones_ingestados')
    ...
    logger.error('[stm-demanda/meses]', { error: String(err) });
 ...
 */
router.get('/operadores', async (req: Request, res: Response) => {
  try {
    ...
    const payload = await cached(cacheKey, 60_000, async () => {
      ...
      let q = sqlDb('mv_stm_operador_mes')
        .select('mes', 'cod_empresa')
        .sum<{ mes: string; cod_empresa: number; total: string }[]>({ total: 'validaciones' })
        .groupBy('mes', 'cod_empresa')
        .orderBy('mes');
      if (mesParam) q = q.where('mes', mesParam + '-01');
      const rows = await q;
    ...
    logger.error('[stm-demanda/operadores]', { error: String(err) });
    res.status(500).json({ ok: false, error: String(err) });
  }
});
 ...
 */
router.get('/lineas/:op', async (req: Request, res: Response) => {
  try {
    const op = String(req.params.op);
    const mes = String(req.query.mes || '');
    const limit = Math.min(200, Math.max(5, parseInt((req.query.limit as string) || '20', 10)));
    const cacheKey = `stm-demanda:lineas:${op}:${mes}:${limit}`;
    const payload = await cached(cacheKey, 60_000, async () => {
      // FASE 5.15: MV mv_stm_linea_resumen (109k filas) → rápido.
      let q = sqlDb('mv_stm_linea_resumen')
        .select('dsc_linea')
        .sum<{ dsc_linea: string; total: string }[]>({ total: 'validaciones' })
        .where('cod_empresa', op)
        .groupBy('dsc_linea')
        .orderBy('total', 'desc')
        .limit(limit);
      if (mes) q = q.where('mes', mes + '-01');
      const rows = await q;
      return {
        ok: true,
    ...
    logger.error('[stm-demanda/lineas]', { error: String(err) });
    res.status(500).json({ ok: false, error: String(err) });
  }
});
 ...
 */
router.get('/linea/:op/:linea', async (req: Request, res: Response) => {
  try {
    const op = String(req.params.op);
    ...
    const payload = await cached(cacheKey, 60_000, async () => {
      ...
      const evol = await sqlDb('mv_stm_linea_resumen')
        .select('mes')
        .sum<{ mes: string; total: string }[]>({ total: 'validaciones' })
        .where({ cod_empresa: op, dsc_linea: linea })
        .groupBy('mes')
        .orderBy('mes');
      const horaDow = await sqlDb('mv_stm_linea_resumen')
        .select('hora', 'dow')
        .sum<{ hora: number; dow: number; total: string }[]>({ total: 'validaciones' })
        .where({ cod_empresa: op, dsc_linea: linea })
        .groupBy('hora', 'dow');
      return {
        ok: true,
    ...
    logger.error('[stm-demanda/linea]', { error: String(err) });
    res.status(500).json({ ok: false, error: String(err) });
  }
});
 ...
 */
router.get('/parada/:codParada', async (req: Request, res: Response) => {
  try {
    const codParada = String(req.params.codParada);
    const cacheKey = `stm-demanda:parada:${codParada}`;
    const payload = await cached(cacheKey, 60_000, async () => {
      const rows = await sqlDb('stm_validaciones_mensual')
        .select('mes', 'cod_empresa')
        .sum<{ mes: string; cod_empresa: number; total: string }[]>({ total: 'validaciones' })
    ...
    logger.error('[stm-demanda/parada]', { error: String(err) });
    res.status(500).json({ ok: false, error: String(err) });
  }
});
 ...
 */
router.get('/hora-dow/:op', async (req: Request, res: Response) => {
  try {
    const op = String(req.params.op);
    const mes = String(req.query.mes || '');
    ...
    logger.error('[stm-demanda/hora-dow]', { error: String(err) });
    res.status(500).json({ ok: false, error: String(err) });
  }
});
 ...
 */
router.get('/load-factor/:op/:linea', async (req: Request, res: Response) => {
  try {
    const op = String(req.params.op);
    const linea = String(req.params.linea);
    const mes = String(req.query.mes || '');
    ...
    const payload = await cached(cacheKey, 60_000, async () => {
      // STM por parada+hora
      let q = sqlDb('stm_validaciones_mensual')
        .select('codigo_parada', 'hora')
        .sum<{ codigo_parada: string; hora: number; total: string }[]>({ total: 'validaciones' })
        .where({ cod_empresa: op, dsc_linea: linea })
      ...
      const lineasGps =
        String(op) === '70' && /^\d{1,2}$/.test(String(linea))
          ? [String(linea), `3${linea}`]
          : [String(linea)];
      const gpsRows = await sqlDb('vehicle_events')
        .select(
          'proxima_parada',
          sqlDb.raw('EXTRACT(HOUR FROM created_at)::int AS hora'),
          sqlDb.raw('COUNT(*) AS pasadas'),
        )
        .where('agency_id', op)
      ...
      return {
        ...
        pasadasGpsPorParadaHora: (gpsRows as Array<{ proxima_parada: string; hora: number; pasadas: string }>)
          .slice(0, limit)
          .map((r) => ({
    ...
    logger.error('[stm-demanda/load-factor]', { error: String(err) });
    res.status(500).json({ ok: false, error: String(err) });
  }
});
 ...
 */
router.get('/mapa-global', async (req: Request, res: Response) => {
  try {
    const mes = String(req.query.mes || '');
    ...
    const payload = await cached(cacheKey, 120_000, async () => {
      ...
      let mesFiltro = mes;
      if (!mesFiltro) {
        const r = await sqlDb('stm_validaciones_ingestados')
          .max<{ max: Date | string | null }[]>({ max: 'mes' })
          .first();
        if (r?.max) {
      ...
      let q = sqlDb('stm_validaciones_mensual')
        ...
        .sum<{ codigo_parada: string; cod_empresa: number; total: string }[]>({ total: 'validaciones' })
        .whereNotNull('codigo_parada')
        .groupBy('codigo_parada', 'cod_empresa');
      if (mesFiltro) q = q.where('mes', mesFiltro + '-01');
      const rows = await q;

      // Agrupar por parada
      interface Comp { ucot: number; cutcsa: number; coetc: number; come: number; }
      const porParada = new Map<string, Comp>();
      for (const r of rows) {
        ...
        if (r.cod_empresa === 70) c.ucot += v;
        else if (r.cod_empresa === 50) c.cutcsa += v;
        else if (r.cod_empresa === 10) c.coetc += v;
        else if (r.cod_empresa === 20) c.come += v;
      }
      ...
      type Item = {
        ...
        lat: number | null;
        lon: number | null;
        ucot: number; cutcsa: number; coetc: number; come: number;
        total: number;
        dominante: 'UCOT' | 'CUTCSA' | 'COETC' | 'COME';
        cuotaDominante: number;
        // Indice Herfindahl-Hirschman normalizado (0=competencia perfecta, 1=monopolio)
        hhi: number;
        nOperadores: number;
      };
      const items: Item[] = [];
      for (const [cod, c] of porParada) {
        ...
        if (total < minViajes) continue;
        if (conUcot && c.ucot === 0) continue;
        // Operador dominante
        const partes: Array<['UCOT' | 'CUTCSA' | 'COETC' | 'COME', number]> = [
    ...
    logger.error('[stm-demanda/mapa-global]', { error: String(err) });
    res.status(500).json({ ok: false, error: String(err) });
  }
});
...
interface SchedStop { stop_id: string; name: string; lat: number; lon: number; arrival?: string; }
interface SchedTrip { control_stops: SchedStop[]; }
interface SchedRoute { habiles?: SchedTrip[]; sabados?: SchedTrip[]; domingos?: SchedTrip[]; }
let stopsIndex: Map<string, { name: string; lat: number; lon: number }> | null = null;
let stopsLoadedAt = 0;
function getStopsIndex(): Map<string, { name: string; lat: number; lon: number }> {
  if (stopsIndex && Date.now() - stopsLoadedAt < 60 * 60 * 1000) return stopsIndex;
  const idx = new Map<string, { name: string; lat: number; lon: number }>();
  try {
    const baseDir = path.resolve(__dirname, '..', 'data', 'gtfs');
    // stops_geo prefija con "2"; mantenemos ambas claves
    const stopsGeo = JSON.parse(fs.readFileSync(path.join(baseDir, 'stops_geo.json'), 'utf8')) as Record<string, { name?: string; lat: number; lon: number }>;
    for (const [id, info] of Object.entries(stopsGeo)) {
      ...
      if (id.startsWith('2')) {
        ...
        if (!idx.has(sinPref)) idx.set(sinPref, { name: info.name ?? sinPref, lat: info.lat, lon: info.lon });
      }
    }
    const sched = JSON.parse(fs.readFileSync(path.join(baseDir, 'schedule_index.json'), 'utf8')) as Record<string, { routes: Record<string, SchedRoute> }>;
    ...
    logger.warn('[stm-demanda] no se pudo cargar stops index', { err: String(e) });
  }
  return idx;
}

router.get('/paradas-linea/:op/:linea', async (req: Request, res: Response) => {
  try {
    const op = String(req.params.op);
    const linea = String(req.params.linea);
    const mes = String(req.query.mes || '');
    ...
    const payload = await cached(cacheKey, 60_000, async () => {
      let q = sqlDb('stm_validaciones_mensual')
        ...
        .sum<{ codigo_parada: string; mes: string; total: string }[]>({ total: 'validaciones' })
        .where({ cod_empresa: op, dsc_linea: linea })
        .whereNotNull('codigo_parada')
      ...
      const stops = getStopsIndex();
      const byCode = new Map<string, { actual: number; previo: number }>();
      // Normalizar mes (puede venir como Date o string ISO)
      function mesYM(v: unknown): string {
      ...
      for (const r of rows) {
        ...
        if (!byCode.has(cod)) byCode.set(cod, { actual: 0, previo: 0 });
    ...
    logger.error('[stm-demanda/paradas-linea]', { error: String(err) });
    res.status(500).json({ ok: false, error: String(err) });
  }
});
 ...
 */
router.get('/forecast/:op/:linea', async (req: Request, res: Response) => {
  try {
    const op = String(req.params.op);
    const linea = String(req.params.linea);
    const dowParam = req.query.dow != null ? parseInt(req.query.dow as string, 10) : null;
    const horaParam = req.query.hora != null ? parseInt(req.query.hora as string, 10) : null;

    const cacheKey = `stm-demanda:forecast:${op}:${linea}:${dowParam ?? 'X'}:${horaParam ?? 'X'}`;
    const payload = await cached(cacheKey, 300_000, async () => {
      // FASE 5.15: MV mv_stm_linea_resumen.
      let q = sqlDb('mv_stm_linea_resumen')
        .select('mes', 'hora', 'dow')
        .sum<{ mes: string; hora: number; dow: number; total: string }[]>({ total: 'validaciones' })
        .where({ cod_empresa: op, dsc_linea: linea })
        .groupBy('mes', 'hora', 'dow')
        .orderBy('mes', 'desc');
      if (dowParam != null) q = q.where('dow', dowParam);
      if (horaParam != null) q = q.where('hora', horaParam);
      const rows = await q;

      // Normalizar mes (Date | string) → "YYYY-MM"
      const toYM = (v: unknown): string =>
        v instanceof Date ? v.toISOString().slice(0, 7) : String(v).slice(0, 7);

      // Agrupar por (hora, dow) y tomar las series mensuales
      const series = new Map<string, Array<{ mes: string; total: number }>>();
      for (const r of rows) {
      ...
      for (const [key, items] of series) {
        ...
        if (ult3.length >= 2) {
          ...
          if (cambio > 0.05) tendencia = 'subiendo';
          else if (cambio < -0.05) tendencia = 'bajando';
        }
    ...
    logger.error('[stm-demanda/forecast]', { error: String(err) });
    res.status(500).json({ ok: false, error: String(err) });
  }
});
 ...
 */
router.get('/competidores/:codParada', async (req: Request, res: Response) => {
  try {
    const codParada = String(req.params.codParada);
    ...
    const payload = await cached(cacheKey, 300_000, async () => {
      const rows = await sqlDb('stm_validaciones_mensual')
        ...
        .sum<{ mes: string; cod_empresa: number; dsc_linea: string; total: string }[]>({ total: 'validaciones' })
        .where('codigo_parada', codParada)
        .groupBy('mes', 'cod_empresa', 'dsc_linea')
        .orderBy('mes');
      const porLinea = new Map<string, { operador: string; linea: string; series: Array<{ mes: string; total: number }> }>();
      for (const r of rows) {
    ...
    logger.error('[stm-demanda/competidores]', { error: String(err) });
    res.status(500).json({ ok: false, error: String(err) });
  }
});
