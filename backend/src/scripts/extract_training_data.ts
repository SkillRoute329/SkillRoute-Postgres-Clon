...
export async function runExtraction(exitOnComplete = false) {
  logger.info('📊 Iniciando extracción de datos para pipeline ML...');
  ...
  try {
    ...
    const rows = await sqlDb.raw(query);
    const data = rows.rows || [];

    if (data.length === 0) {
      logger.warn('⚠️ No se encontraron registros de vehicle_events para extraer. Generando datos sintéticos de calibración basados en historiales...');
    ...
    logger.info(`✅ Pipeline exitoso: ${data.length} registros guardados en: ${csvPath}`);

  } catch (error) {
    logger.error('❌ Error en la extracción del dataset ML:', error);
  } finally {
    if (exitOnComplete) {
      process.exit(0);
    }
...
function generateSyntheticData() {
  ...
  fs.writeFileSync(csvPath, csvLines.join('\n'));
  logger.info(`✅ Pipeline exitoso (Datos Sintéticos): 1500 registros guardados en: ${csvPath}`);
}
