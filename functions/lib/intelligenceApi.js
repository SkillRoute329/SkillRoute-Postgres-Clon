...
app.get('/api/ucot/fleet-intel', async (_req, res) => {
    try {
        const geojson = await fetchSTM();
        const todos = (geojson.features || []).map(parseBus).filter((b) => b.gpsValido);
        const ucotAll = todos.filter((b) => b.empresaId === EMPRESA_UCOT_ID);
        ...
        const lineas = UCOT_LINEAS.map((meta) => {
            const busesUcot = ucotAll.filter((b) => b.linea === meta.id);
            ...
            else if (pctFlotaEnDisputa >= 60 || bunchingPares >= 2)
                nivelAlerta = 'ALTA';
            else if (pctFlotaEnDisputa >= 30 || bunchingPares >= 1)
                nivelAlerta = 'MEDIA';
            ...
            else if (pctFlotaEnDisputa >= 60)
                posicionCompetitiva = 'CRITICA';
            else if (pctFlotaEnDisputa >= 30)
...
app.post('/api/listero/ausencia', async (req, res) => {
    ...
    const fechaHoy = fecha || fechaHoyMVD();
    try {
        if (conductorId) {
...
app.post('/api/listero/generar-programacion', async (req, res) => {
    var _a;
    const fecha = String(((_a = req.body) === null || _a === void 0 ? void 0 : _a.fecha) || fechaHoyMVD());
    try {
...
app.get('/api/cartones/oficiales', async (req, res) => {
    ...
    try {
        ...
        let query = db.collection('servicios_ucot').limit(limit);
        if (linea)
