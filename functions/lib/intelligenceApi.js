...
exports.intelligenceApi = exports.UCOT_LINEAS_LIST = void 0;
...
const autostats_1 = require("./api/autostats");
const ucotPortal_1 = require("./api/ucotPortal");
...
(0, autostats_1.registerAutostatsRoutes)(app);
(0, ucotPortal_1.registerUcotPortalRoutes)(app);
...
exports.UCOT_LINEAS_LIST = UCOT_LINEAS;
    ...
    try {
        ...
        for (const ucot of ucotBuses) {
            for (const rival of todos) {
                if (rival.empresaId === EMPRESA_UCOT_ID)
                    continue;
        ...
        const competenciaFinal = Array.from(competenciaMap.values())
            ...
            return Object.assign(Object.assign({}, c), { frecuenciaRealMinutos: frecEstimada, frecuenciaProgramadaMinutos: null, ventajaCompetitiva: frecEstimada < 15 ? "ALTA (Headway Corto)" : "MEDIA" });
        });
        ...
        let puntualidad = 85;
        if (bunchingPares > 0)
            puntualidad = Math.max(30, 85 - (bunchingPares * 15));
        res.json({
            ok: true,
            linea: lineaUcot,
            timestamp: new Date().toISOString(),
            ...
            resumenEjecutivo: `Línea ${lineaUcot}: ${ucotBuses.length} coches activos. ${bunchingPares > 0 ? 'Agrupamiento detectado.' : 'Separación ideal.'} Enemigo táctico: ${((_a = competenciaFinal[0]) === null || _a === void 0 ? void 0 : _a.empresa) || 'Ninguno'} con frec. est. de ${((_b = competenciaFinal[0]) === null || _b === void 0 ? void 0 : _b.frecuenciaRealMinutos) || '-'} min.`,
...
async function posicionesHandler(_req, res) {
    try {
        const geojson = await fetchSTM();
        const buses = (geojson.features || [])
            .map(parseBus)
            .filter((b) => b.gpsValido)
            .map((b) => ({
...
app.get('/api/analysis/:linea', async (req, res) => {
    ...
    try {
        ...
        const empresasDetectadas = new Set();
        const alertas = [];
        for (const ucot of ucotBuses) {
            const cercanos = [];
            for (const rival of todos) {
        ...
        const nivelAlerta = pctFlota >= 60 ? 'ALTA' : pctFlota >= 30 ? 'MEDIA' : total === 0 ? 'SIN DATOS' : 'BAJA';
        res.json({
            ok: true,
            linea,
            resumen: {
        ...
        res.status(502).json({ ok: false, linea, error: (err === null || err === void 0 ? void 0 : err.message) || String(err) });
    }
});
/**
 ...
 */
function frecuenciaProgramadaDesdeDoc(doc, tipoDia, hhmm) {
    var _a, _b, _c, _d, _e, _f;
    const data = doc.data();
    const dia = (_a = data === null || data === void 0 ? void 0 : data.dias) === null || _a === void 0 ? void 0 : _a[tipoDia];
    if (!dia)
        return { frecuenciaProgramadaMin: null, horaInicio: null, horaFin: null, totalSalidas: 0 };
    const salidas = (dia.salidasDominante || []);
    // horaInicio/horaFin del día = primera y última salida de CUALQUIER variante
    const horasOrdenadas = salidas
        .map((s) => s.desde)
        .filter((h) => typeof h === 'string')
        .sort();
    const horaInicio = (_b = horasOrdenadas[0]) !== null && _b !== void 0 ? _b : null;
    const horaFin = (_c = horasOrdenadas[horasOrdenadas.length - 1]) !== null && _c !== void 0 ? _c : null;
    const target = hhmmToMin(hhmm);
    if (target === null) {
        return {
            frecuenciaProgramadaMin: (_d = dia.frecuenciaDominanteMin) !== null && _d !== void 0 ? _d : null,
            horaInicio,
            horaFin,
            totalSalidas: (_e = dia.totalSalidas) !== null && _e !== void 0 ? _e : 0,
        };
    }
    let enVentana = 0;
    ...
    return {
        ...
        horaInicio,
        horaFin,
        totalSalidas: (_f = dia.totalSalidas) !== null && _f !== void 0 ? _f : 0,
    };
}
...
app.get('/api/agency-lines/:agencyId', async (req, res) => {
    ...
    try {
        ...
        if (!empresaId)
            return res.json({ ok: true, agencyId, source: 'empty', lines: [] });
        const geojson = await fetchSTM();
        const todos = (geojson.features || []).map(parseBus).filter((b) => b.gpsValido);
        const companyBuses = todos.filter((b) => b.empresaId === empresaId);
        const lineMap = new Map();
        companyBuses.forEach((b) => { var _a; if (b.linea)
            lineMap.set(b.linea, ((_a = lineMap.get(b.linea)) !== null && _a !== void 0 ? _a : 0) + 1); });
...
app.get('/api/ucot/fleet-intel', async (req, res) => {
    var _a, _b, _c;
    try {
        const agencyId = String((_a = req.query.agencyId) !== null && _a !== void 0 ? _a : '70').trim();
        const lineIdsParam = String((_b = req.query.lineIds) !== null && _b !== void 0 ? _b : '').trim();
        const requestedLineIds = lineIdsParam ? lineIdsParam.split(',').map((s) => s.trim()).filter(Boolean) : null;
        const empresaId = (_c = EMPRESA_IDS[agencyId]) !== null && _c !== void 0 ? _c : EMPRESA_UCOT_ID;
        const geojson = await fetchSTM();
        const todos = (geojson.features || []).map(parseBus).filter((b) => b.gpsValido);
        const agencyBuses = todos.filter((b) => b.empresaId === empresaId);
        let lineasMeta;
        if (requestedLineIds && requestedLineIds.length > 0) {
            lineasMeta = requestedLineIds.map((id) => {
                var _a, _b;
                const ucotMeta = UCOT_LINEAS.find((l) => l.id === id);
                return { id, nombre: (_a = ucotMeta === null || ucotMeta === void 0 ? void 0 : ucotMeta.nombre) !== null && _a !== void 0 ? _a : `Línea ${id}`, categoria: (_b = ucotMeta === null || ucotMeta === void 0 ? void 0 : ucotMeta.categoria) !== null && _b !== void 0 ? _b : 'urbana' };
            });
        }
        else if (agencyId === '70') {
            ...
            const lineMap = new Map();
            agencyBuses.forEach((b) => { var _a; if (b.linea)
                lineMap.set(b.linea, ((_a = lineMap.get(b.linea)) !== null && _a !== void 0 ? _a : 0) + 1); });
        ...
        if (lineasMeta.length > 0) {
            const horarioDocs = await getDb().getAll(...lineasMeta.map((l) => getDb().collection('horarios_oficiales').doc(l.id)));
        ...
        evSnap.docs.forEach((d) => {
            const e = d.data();
            if (e.linea) {
                if (!linesBuses[e.linea])
                    linesBuses[e.linea] = new Set();
                linesBuses[e.linea].add(e.idBus);
            }
        });
        const frecuenciaRealByLine = {};
        ...
        const lineas = lineasMeta.map((meta) => {
            var _a;
            const busesLinea = agencyBuses.filter((b) => b.linea === meta.id);
            ...
            let frecuenciaProgramadaMin = null;
            let horaInicioProgramada = null;
            let horaFinProgramada = null;
            ...
            const frecReal = (_a = frecuenciaRealByLine[meta.id]) !== null && _a !== void 0 ? _a : null;
            ...
            if (busesActivos === 0)
                estadoOperativo = 'SIN_SERVICIO';
            else if (nivelAlerta === 'ALTA')
                estadoOperativo = 'ALERTA';
            let posicionCompetitiva = 'CON_RIVALES';
            if (busesActivos === 0)
                posicionCompetitiva = 'SIN_SERVICIO';
            else if (empresasDetectadas.size === 0)
                posicionCompetitiva = 'SIN_RIVALES_VISIBLES';
            else if (pctFlotaEnDisputa >= 70)
                posicionCompetitiva = 'CRITICA';
            else if (pctFlotaEnDisputa >= 40)
                posicionCompetitiva = 'DISPUTADA';
            ...
            if (brechaPct !== null && brechaPct > 0)
                saludServicio -= Math.min(40, Math.round(brechaPct * 0.7));
            if (bunchingPares > 0)
                saludServicio -= Math.min(30, bunchingPares * 12);
            if (busesActivos === 0)
                saludServicio = 0;
...
const recomputeSentidoHandler = async (req, res) => {
    var _a, _b, _c, _d, _e, _f, _g, _h, _j, _k, _l, _m;
    try {
        const hours = Math.max(1, Math.min(72, Number((_a = req.query.hours) !== null && _a !== void 0 ? _a : 6)));
        const limit = Math.max(1, Math.min(20000, Number((_b = req.query.limit) !== null && _b !== void 0 ? _b : 5000)));
        ...
        async function getCtx(agencyId, linea) {
            const key = `${agencyId}_${linea}`;
            let ctx = ctxCache.get(key);
            if (!ctx) {
                ctx = await (0, autoStatsCollector_1.loadSentidoContext)(agencyId, linea, fsdb);
        ...
        let sinCambio = 0;
        let sinDestino = 0;
        const conteoConfianza = { HIGH: 0, MEDIUM: 0, LOW: 0, ZERO: 0 };
        ...
        for (const docSnap of snap.docs) {
            ...
            if (!d.destinoDesc)
                sinDestino++;
            const linea = String((_c = d.linea) !== null && _c !== void 0 ? _c : '');
            const agencyId = String((_d = d.agencyId) !== null && _d !== void 0 ? _d : '');
            if (!linea) {
                sinCambio++;
                continue;
            }
            const ctx = await getCtx(agencyId, linea);
            const result = (0, autoStatsCollector_1.detectarSentidoConContexto)((_e = d.destinoDesc) !== null && _e !== void 0 ? _e : null, (_f = d.variante) !== null && _f !== void 0 ? _f : null, typeof d.bearing === 'number' ? d.bearing : null, ctx);
            conteoConfianza[result.confianza] = ((_g = conteoConfianza[result.confianza]) !== null && _g !== void 0 ? _g : 0) + 1;
            const cambio = ((_h = result.sentido) !== null && _h !== void 0 ? _h : null) !== ((_j = d.sentido) !== null && _j !== void 0 ? _j : null)
                || ((_k = result.confianza) !== null && _k !== void 0 ? _k : null) !== ((_l = d.confianzaSentido) !== null && _l !== void 0 ? _l : null);
            if (cambio) {
        ...
        res.status(500).json({ ok: false, error: (_m = err === null || err === void 0 ? void 0 : err.message) !== null && _m !== void 0 ? _m : String(err) });
    }
};
...
exports.intelligenceApi = functions
