...
class PayrollService {
     ...
     */
    static async calculateShiftSalary(userId, isExtra = false, manualRole) {
        // 1. Fetch User Data
        ...
        const roleName = manualRole || (user === null || user === void 0 ? void 0 : user.roleName) || 'Conductor';
     ...
     */
    static async accrueSalary(shiftId, userId, extraHours) {
        if (!shiftId || !userId)
            return;
        // Calculate
        const financials = await this.calculateShiftSalary(userId, extraHours > 0);
        // Record Transaction (Credit to Employee)
        const transactionRef = firebase_1.db.collection('transactions').doc();
        ...
        await firebase_1.db.collection('shifts').doc(shiftId).update({
...
PayrollService.VALUES = {
    BPC: 6864,
    // Retention Rates
    BPS_RATE: 0.15,
    FONASA_RATE: 0.045,
    FRL_RATE: 0.001,
    MIN_INTANGIBLE_PERCENT: 0.35,
