...
const aplicarPoliticaMinimos_1 = require("./aplicarPoliticaMinimos");
...
function computeHeadways(events) {
...
// ── Función principal ─────────────────────────────────────────────────────────
function computeMetrics(events, scheduledTrips, coverageGps, // % cobertura GPS global de la línea-sentido-día
totalTripsScheduled) {
    const n = events.length;
    // ── gps_coverage ────────────────────────────────────────────────────────────
    const validSnap = events.filter(e => e.confianzaV2 !== 'ZERO' && e.snapDistanceMV2 !== null && e.snapDistanceMV2 <= SNAP_MAX_M);
    const gpsCovPct = n > 0 ? (validSnap.length / n) * 100 : 0;
    const gps_coverage = (0, aplicarPoliticaMinimos_1.aplicarPoliticaMinimos)({
        valorRaw: gpsCovPct,
        n,
        cobertura: 100,
        nMinimo: 1,
    ...
    // ── otp_low_freq ────────────────────────────────────────────────────────────
    const evConHorario = events.filter(e => e.estadoCumplimiento &&
        !['SIN_HORARIO', 'FUERA_DE_SERVICIO', 'SIN_MATCH'].includes(e.estadoCumplimiento));
    const evATiempo = evConHorario.filter(e => e.desviacionMin !== null && Math.abs(e.desviacionMin) <= 4);
    const nOtp = evConHorario.length;
    const otpPct = nOtp > 0 ? (evATiempo.length / nOtp) * 100 : 0;
    const otp_low_freq = (0, aplicarPoliticaMinimos_1.aplicarPoliticaMinimos)({
    ...
    if (headwaysObs.length >= 5) {
        ...
        headway_cv = (0, aplicarPoliticaMinimos_1.aplicarPoliticaMinimos)({
            valorRaw: cv,
    ...
    if (headwaysObs.length >= 5) {
        ...
        bunching_index = (0, aplicarPoliticaMinimos_1.aplicarPoliticaMinimos)({
            valorRaw: bunchPct,
            n: headwaysObs.length,
            cobertura: coverageGps,
            nMinimo: 5,
    ...
    if (isHighFreq && headwaysObs.length >= 5 && scheduledTrips.length >= 3) {
        ...
        if (headwaysProg.length >= 3) {
            ...
            ewt_high_freq = (0, aplicarPoliticaMinimos_1.aplicarPoliticaMinimos)({
                valorRaw: ewt,
                n: headwaysObs.length,
                cobertura: coverageGps,
                nMinimo: 5,
    ...
    let service_reliability_score = null;
    const otpVal = otp_low_freq === null || otp_low_freq === void 0 ? void 0 : otp_low_freq.value;
    const cvVal = headway_cv === null || headway_cv === void 0 ? void 0 : headway_cv.value;
    const gpsVal = gps_coverage === null || gps_coverage === void 0 ? void 0 : gps_coverage.value;
    if (otpVal !== null && otpVal !== undefined &&
        ...
        service_reliability_score = (0, aplicarPoliticaMinimos_1.aplicarPoliticaMinimos)({
            valorRaw: score,
            n: nComp,
            cobertura: coverageGps,
            nMinimo: 5,
    ...
    let service_delivered = null;
    if (totalTripsScheduled && totalTripsScheduled > 0) {
        const evOperados = events.filter(e => e.estadoCumplimiento && e.estadoCumplimiento !== 'FUERA_DE_SERVICIO');
        ...
        service_delivered = (0, aplicarPoliticaMinimos_1.aplicarPoliticaMinimos)({
            valorRaw: pctDel,
            n: uniqueBuses,
            cobertura: coverageGps,
            nMinimo: 1,
