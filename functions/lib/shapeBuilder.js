...
exports.shapeBuilderRun = exports.shapeBuilderTick = void 0;
...
function distPerpendicularM(p, a, b) {
 ...
 */
function ordenarTrazado(pings) {
    var _a, _b;
    if (pings.length === 0)
        return [];
    // Agrupar por bus
    const porBus = new Map();
    for (const p of pings) {
        const key = String((_a = p.busId) !== null && _a !== void 0 ? _a : '_unknown');
        if (!porBus.has(key))
            porBus.set(key, []);
    ...
    const base = (_b = trayectorias[0]) !== null && _b !== void 0 ? _b : [];
    return base;
}
async function leerPings(desdeMs) {
    ...
    while (true) {
        ...
        for (const d of snap.docs) {
            const data = d.data();
            if (!data.empresa || !data.linea)
                continue;
            if (data.variante == null)
                continue;
            const key = `${data.empresa}|${data.linea}|${data.variante}`;
            if (!grupos.has(key))
                grupos.set(key, { pings: [] });
...
// ─── Materialización ─────────────────────────────────────────────────────────
async function materializarShapes(grupos) {
    var _a;
    ...
    for (const [key, val] of grupos.entries()) {
        ...
        const sentido = variante % 2 === 0 ? 'IDA' : 'VUELTA';
        ...
        await db
            ...
            empresa: (_a = AGENCY_NAMES[empresa]) !== null && _a !== void 0 ? _a : `EMP_${empresa}`,
            linea,
            variante,
            sentido,
...
// ─── Cron ────────────────────────────────────────────────────────────────────
exports.shapeBuilderTick = functions
    ...
    console.log(`[shapeBuilder] OK grupos=${grupos.size} shapes=${shapesEscritos} ignorados=${shapesIgnorados} ${elapsed}ms`);
    await db
 ...
 */
exports.shapeBuilderRun = functions.https.onRequest(async (_req, res) => {
    const start = Date.now();
    const desde = Date.now() - VENTANA_HORAS * 60 * 60 * 1000;
    try {
        const grupos = await leerPings(desde);
