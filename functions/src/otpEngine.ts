...
function getCurrentServiceType(): 'HABIL' | 'SABADO' | 'DOMINGO' {
...
async function loadTimetable(
  agencyId: string, linea: string, directionId: number, svcType: string,
...
function computeBusDelay(
  busId: string, agencyId: string, linea: string,
  ...
  if (bestIdx === -1 || bestDist > 400) return base; // bus demasiado lejos de la ruta
  ...
  let bestViaje: { s: string; t: number[] } | null = null;
  let bestDiff = Infinity;
  for (const v of timetable.viajes) {
    const t = v.t[bestIdx];
    if (t < 0) continue;
    const diff = Math.abs(t - currentMin);
    if (diff < bestDiff) { bestDiff = diff; bestViaje = v; }
  }
  if (!bestViaje || bestDiff > 45) return base; // ningún viaje dentro de ±45 min
...
interface OtpResult {
  busesVivos: number;
  busesEvaluados: number;
  busesATiempo: number;
  busesTardando: number;
  busesAdelantados: number;
  busesSinDatos: number;
  lineasActivas: number;
  durationMs: number;
}

async function runOtpTick(): Promise<OtpResult> {
  const t0 = Date.now();
  ...
  const buses = await getBusesEnriquecidosInternal('all');
  ...
  const lineasSet = new Map<string, { agencyId: string; linea: string }>();
  ...
  await Promise.all(
    [...lineasSet.values()].flatMap(({ agencyId, linea }) =>
      [0, 1].map(async (dir) => {
  ...
  // 5. Evaluar cada bus
  const delays: BusDelay[] = [];
  for (const b of buses) {
    if (!b.linea || !b.empresaId) continue;
    const agencyId = String(b.empresaId);
    let result: BusDelay | null = null;
    for (const dir of [0, 1]) {
      ...
      if (candidate.estado !== 'SIN_DATOS') { result = candidate; break; }
    }
    if (!result) result = {
  ...
  for (let i = 0; i < delays.length; i += BATCH_SIZE) {
    ...
    for (const d of delays.slice(i, i + BATCH_SIZE)) {
      batch.set(db.collection('bus_delays').doc(`${d.agencyId}_${d.busId}`), {
        ...d, svcType, updatedAt: now,
      });
    }
    await batch.commit();
  }

  // 7. Agregar y escribir otp_summary por línea
  type SummaryEntry = { agencyId: string; linea: string; total: number; aTiempo: number; retrasado: number; adelantado: number; sinDatos: number; delays: number[] };
  const summaryMap = new Map<string, SummaryEntry>();
  for (const d of delays) {
    const key = `${d.agencyId}_${d.linea}`;
    if (!summaryMap.has(key)) summaryMap.set(key, { agencyId: d.agencyId, linea: d.linea, total: 0, aTiempo: 0, retrasado: 0, adelantado: 0, sinDatos: 0, delays: [] });
    const s = summaryMap.get(key)!;
    s.total++;
    if (d.estado === 'A_TIEMPO') s.aTiempo++;
    else if (d.estado === 'RETRASADO') { s.retrasado++; s.delays.push(d.delayMin); }
    else if (d.estado === 'ADELANTADO') s.adelantado++;
    else s.sinDatos++;
  }
...
export const otpTick = onSchedule(
  { schedule: 'every 10 minutes', region: 'us-central1', timeoutSeconds: 300, memory: '1GiB' },
  async () => {
    try { await runOtpTick(); }
    catch (err) { logger.error('[OTP] Tick falló:', err instanceof Error ? err.message : String(err)); }
  },
);

export const computeOtpNow = onRequest(
