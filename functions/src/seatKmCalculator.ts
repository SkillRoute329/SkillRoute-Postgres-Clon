...
const CAPACITY_BY_AGENCY: Record<string, number> = {
  '10': 60,   // COETC
  '20': 60,   // COME
...
interface SeatKmSnapshot {
  ...
  generadoEn: admin.firestore.FieldValue;
}

// ─── Helpers ──────────────────────────────────────────────────────────────────

function getCurrentServiceType(date: Date): string {
...
function dirToSentido(directionId: number): 'IDA' | 'VUELTA' {
  return directionId === 0 ? 'IDA' : 'VUELTA';
}

// ─── Lógica principal ─────────────────────────────────────────────────────────

async function computeSeatKm(date: Date): Promise<SeatKmSnapshot> {
  const t0    = Date.now();
  ...
  const shapesMap = new Map<string, number>(); // agencyId|linea|directionId → lengthMeters
  for (const doc of shapesSnap.docs) {
    const d = doc.data() as ShapeDoc;
    if (!d.lengthMeters || d.lengthMeters <= 0) continue;
  ...
  for (const doc of ttSnap.docs) {
    const d = doc.data() as TimetableDoc;
    if (!d.agencyId || !d.linea || !Array.isArray(d.viajes)) continue;

    const shapeKey = `${d.agencyId}|${d.linea}|${d.directionId}`;
    const lengthM  = shapesMap.get(shapeKey) ?? 0;
    if (lengthM === 0) continue; // sin shape disponible → excluir

    const longKm         = lengthM / 1000;
    const viajesEstimados = d.viajes.length;
    if (viajesEstimados === 0) continue;

    const capacity = CAPACITY_BY_AGENCY[d.agencyId] ?? DEFAULT_CAPACITY;
    ...
    corredores.push({
      ...
      linea:            d.linea,
      agencyId:         d.agencyId,
      empresa:          AGENCY_NAMES[d.agencyId] ?? d.agencyId,
  ...
  const empresaMap = new Map<string, { seatKm: number; viajes: number; lineas: Set<string> }>();
  for (const r of corredores) {
  ...
  for (const [aid, e] of empresaMap) {
    empresas[aid] = {
      ...
      capacidadPromedio: CAPACITY_BY_AGENCY[aid] ?? DEFAULT_CAPACITY,
    };
  }

  const snapshot: SeatKmSnapshot = {
    date:             dateStr,
    svcType,
    empresas,
    total:            totalSeatKm,
  ...
  logger.info('[SeatKm] Corredores calculados:', corredores.length,
    '| Total seat-km:', totalSeatKm.toLocaleString(),
    '| ms:', Date.now() - t0);
...
export const seatKmCalculatorCron = onSchedule(
  { schedule: '0 6 * * *', timeZone: 'America/Montevideo', region: 'us-central1', timeoutSeconds: 300, memory: '512MiB' },
  async () => {
    try {
...
export const seatKmCalculatorNow = onRequest(
...
export const seatKmSnapshotQuery = onRequest(
  { region: 'us-central1', cors: true, timeoutSeconds: 30, memory: '256MiB' },
  async (req, res) => {
    try {
      const dateParam = req.query.date as string | undefined;
      const dateStr = dateParam ?? toDateString(new Date());
