...
interface VehicleEvent {
  ...
  timestampGPS?: string; // viene como ISO string desde el ingestor
}
...
interface AggBucket {
  ...
  desviacionAcumMin: number;
  desviacionMuestras: number;
}

function emptyAgg(): AggBucket {
  return {
...
interface AdherenceReport {
  ...
  ventanaFinMs: number;
  totalBuses: number;
  resultsByAgency: Record<string, AggBucket & { otp: number; lineasObservadas: string[] }>;
}
 ...
 */
async function computeAdherenceForRange(
  startMs: number,
  endMs: number,
  agencyFilter?: string,
  ...
  const resultsByAgency: Record<string, AggBucket & { otp: number; lineasObservadas: string[] }> = {};
  let totalBuses = 0;

  for (const agencyId of agencias) {
    const agg = emptyAgg();
    const lineasSeen = new Set<string>();
    ...
    snap.forEach((doc) => {
      const ev = doc.data() as VehicleEvent;
      const busId = String(ev.idBus ?? doc.id);
      if (seenBuses.has(busId)) return;
      seenBuses.add(busId);
      totalBuses += 1;

      const linea = String(ev.linea ?? '').trim();
      if (linea) lineasSeen.add(linea);

      const estado = ev.estadoCumplimiento ?? 'SIN_HORARIO';
      switch (estado) {
        case 'EN_TIEMPO':         agg.aTiempo += 1; agg.serviciosTotales += 1; break;
        case 'ADELANTADO':        agg.adelantados += 1; agg.serviciosTotales += 1; break;
        case 'ATRASADO':          agg.atrasados += 1; agg.serviciosTotales += 1; break;
        case 'SIN_HORARIO':       agg.sinHorario += 1; break;
        case 'FUERA_DE_SERVICIO': agg.fueraDeServicio += 1; break;
        default:                  agg.sinHorario += 1; break;
      }
      ...
      if (linea) {
        ...
        void db.collection(RT_COLLECTION).doc(rtKey).set({
          ...
          estado,
          desviacionMin: ev.desviacionMin ?? null,
          proximaParada: ev.proximaParada ?? null,
          sentido: ev.sentido ?? null,
          ymd,
    ...
    resultsByAgency[agencyId] = {
      ...agg,
      otp,
      lineasObservadas: [...lineasSeen].sort(),
    };
  }
...
export const computeAdherenceNow = functions
  .runWith({ timeoutSeconds: 540, memory: '1GB' })
  .https.onRequest(async (req, res) => {
    res.set('Access-Control-Allow-Origin', '*');
    try {
      const dateParam = (req.query.date as string) ?? null;
      const agencyParam = (req.query.agencyId as string) ?? undefined;
      const hoursParam = parseInt((req.query.hours as string) ?? '24', 10);
...
export const computeAdherenceCron = functions
  .runWith({ timeoutSeconds: 540, memory: '1GB' })
  .pubsub.schedule('15 * * * *')
  ...
  .onRun(async () => {
    try {
      ...
      console.log('[ScheduleAdherence] Cron OK:', JSON.stringify({
        ...
        otpByAgency: Object.fromEntries(
          Object.entries(r.resultsByAgency).map(([k, v]) => [k, +v.otp.toFixed(3)]),
        ),
      }));
    } catch (err: unknown) {
      const msg = err instanceof Error ? err.message : 'Error desconocido';
