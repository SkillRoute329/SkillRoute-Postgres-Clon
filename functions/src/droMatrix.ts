...
interface Sample {
  lat: number;
  lon: number;
  bearing: number; // grados [0,360)
}

/** Segmento de B con bearing precomputado — elimina trig del inner loop crítico. */
interface SegB { a: Point; b: Point; bearing: number; }

interface OverlapDoc {
  key: string; // keyA__keyB
  shapeAKey: string;
  shapeBKey: string;
  agencyA: string;
  empresaA: string;
...
function perpDistM(pLat: number, pLon: number, a: Point, b: Point): number {
...
function makeSegsB(pts: Point[]): SegB[] {
  const segs: SegB[] = [];
 ...
 */
function resamplePolyline(points: Point[], intervalM: number): Sample[] {
...
function countCoveredSamples(samplesA: Sample[], segsB: SegB[]): number {
  if (samplesA.length === 0 || segsB.length === 0) return 0;
  let covered = 0;
  ...
  for (const sa of samplesA) {
    ...
    if (bestDist > MAX_LATERAL_M) continue;
    if (bearingDiff(sa.bearing, bestBearing) > MAX_BEARING_DIFF_DEG) continue;
    covered++;
  }
  return covered;
}
...
export async function computeDroMatrix(
  ...
  pairsWritten: number;
  durationMs: number;
  topOverlaps: Array<{ keyA: string; keyB: string; pctAInB: number; sharedKm: number }>;
}> {
  const t0 = Date.now();
  ...
  const shapeDedup = new Map<string, ShapeDoc>();
  ...
  const resampledPts = new Map<string, Point[]>();
  ...
  const segsMap = new Map<string, SegB[]>();
  for (const s of dedupedShapes) {
    segsMap.set(s.key, makeSegsB(resampledPts.get(s.key) ?? []));
  }
  ...
  for (const a of dedupedShapes) {
    ...
    for (const b of dedupedShapes) {
      ...
      pairsEvaluated++;
      const covered = countCoveredSamples(samplesA, segsMap.get(b.key) ?? []);
      if (covered === 0) continue;
      const pctAInB = (covered / samplesA.length) * 100;
      if (pctAInB < minOverlapPct) continue;
...
export const droMatrixTick = functions
  ...
  .onRun(async () => {
    try {
      ...
      console.error('[droMatrixTick] Error:', err?.message || err);
    }
    return null;
  });

/** HTTP manual para testing. `?minOverlapPct=N` opcional. */
export const recomputeDroMatrixNow = functions
  .runWith({ timeoutSeconds: 540, memory: '1GB' })
  .https.onRequest(async (req, res) => {
    try {
      const minPct = Number(req.query.minOverlapPct ?? MIN_OVERLAP_PCT);
      const result = await computeDroMatrix(Number.isFinite(minPct) ? minPct : MIN_OVERLAP_PCT);
      res.json({ ok: true, ...result });
    } catch (err: any) {
      console.error('[recomputeDroMatrixNow] Error:', err?.message || err);
