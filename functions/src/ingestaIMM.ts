...
const EMPRESA_MAP: Record<number, string> = {
  10: 'COETC',
  20: 'COME',
  50: 'CUTCSA',
  70: 'UCOT',
 ...
 */
const CORREDORES_UCOT: Record<string, string[]> = {
...
export const ingestaIMMTick = functions
  ...
  .onRun(async () => {
    ...
    // ─────────────────────────────────────────────────────────────────────────
    let stmData: STMResponse;
    try {
      const response = await axios.post<STMResponse>(
        STM_ONLINE_URL,
        { empresa: '-1' },
        {
          headers: STM_HEADERS,
    ...
    if (!stmData?.features || !Array.isArray(stmData.features)) {
    ...
    // ─────────────────────────────────────────────────────────────────────────
    const busesValidos: BusProcesado[] = [];
    let descartados = 0;
    ...
    for (const bus of busesUCOT) {
      ...
      let lat_prev: number | null = null;
      let lng_prev: number | null = null;
      try {
        const prevDoc = await ref.get();
        if (prevDoc.exists) {
          const prevData = prevDoc.data();
          if (prevData?.posicion) {
            lat_prev = prevData.posicion.latitude ?? prevData.posicion._latitude ?? null;
            lng_prev = prevData.posicion.longitude ?? prevData.posicion._longitude ?? null;
          }
        }
      } catch {
    ...
    console.log(
      `[ingestaIMM] ✅ Ciclo completo en ${elapsed}ms | ` +
...
export const testIngestaIMM = functions
  .runWith({ timeoutSeconds: 30 })
  .https.onRequest(async (req, res) => {
    const corsHandler = (await import('cors')).default({ origin: true });
