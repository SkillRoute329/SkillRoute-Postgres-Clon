  ...
  return {
    NeTExDiscovery: {
      ...
      onDemandFullNeTExXml: {
        available: false,
        reason:
          'NeTEx completo XML requiere ~5-10 días de transformación. Disponible bajo acuerdo comercial específico.',
        requestContact: PUBLISHER.contact,
      },
    },
    _comment:
      'Este endpoint es un punto de entrada para procurement EU. El dataset completo NeTEx se genera on-demand bajo acuerdo. Para uso típico MaaS, consumir GTFS-static + GTFS-Realtime / SIRI-Lite ya disponibles.',
  };
}

function buildDiscoveryXml(): string {
  const now = new Date().toISOString();
...
export const netexEndpoint = functions
