...
import { fetchLineSchedule, TipoDia, HorarioLinea } from './stmHorariosScraper';

const db = admin.firestore();

/** Las 29 líneas UCOT (id en el STM, nombre comercial, categoría). */
const LINEAS_UCOT: Array<{ id: string; nombre: string; categoria: string }> = [
...
const TIPOS_DIA: TipoDia[] = ['Hábiles', 'Sábados', 'Domingos'];

function sleep(ms: number) {
  return new Promise((resolve) => setTimeout(resolve, ms));
}

async function scrapeLineaTodosLosDias(
  linea: { id: string; nombre: string; categoria: string },
): Promise<{ ok: boolean; error?: string; horarios?: Record<string, HorarioLinea> }> {
  const horarios: Record<string, HorarioLinea> = {};
  for (const tipoDia of TIPOS_DIA) {
    try {
      const h = await fetchLineSchedule(linea.id, tipoDia);
      horarios[tipoDia] = h;
      await sleep(300); // rate-limit básico
    } catch (err: any) {
      // Si la línea no existe en el catálogo STM, abortar completo
      if (String(err?.message || '').includes('no está en el catálogo')) {
...
async function persistirLinea(
  linea: { id: string; nombre: string; categoria: string },
  horarios: Record<string, HorarioLinea>,
): Promise<void> {
  ...
  for (const [tipoDia, h] of Object.entries(horarios)) {
    ...
    const variantesOrdenadas = [...h.variantes].sort(
...
async function refrescarTodas(): Promise<{
  ok: boolean;
  totalLineas: number;
  exitosas: number;
  falladas: number;
  detalle: Record<string, { ok: boolean; error?: string; tiposDia?: string[]; totalSalidas?: number }>;
  latenciaMs: number;
}> {
  const startMs = Date.now();
  const detalle: Record<string, { ok: boolean; error?: string; tiposDia?: string[]; totalSalidas?: number }> = {};
  let exitosas = 0;
  let falladas = 0;
  ...
  for (const linea of LINEAS_UCOT) {
    ...
    try {
      ...
      detalle[linea.id] = { ok: false, error: `persist: ${err?.message || err}` };
      falladas++;
    }
  }
...
// ─── CRON diario 04:00 America/Montevideo ────────────────────────────────────
export const refreshHorariosUcotTick = functions
  ...
  .onRun(async () => {
    ...
    console.log(
      `[refreshHorariosUcot] ✅ ${res.exitosas}/${res.totalLineas} líneas OK, ${res.falladas} falladas, ${res.latenciaMs}ms`,
    );
    return null;
  });

// ─── HTTP trigger manual: refresca todas las líneas o una sola ───────────────
export const refreshHorariosUcotNow = functions
  .runWith({ timeoutSeconds: 540, memory: '512MB' })
  .https.onRequest(async (req, res) => {
    const lineaFiltro = (req.query.linea as string | undefined)?.trim();
    try {
      if (lineaFiltro) {
