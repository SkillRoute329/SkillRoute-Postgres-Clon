...
import { UserService, type User as UserType } from '../services/api';
import { Search, User, X, CheckCircle, Loader2 } from 'lucide-react';
import clsx from 'clsx';

interface AssignShiftModalProps {
  onClose: () => void;
  onAssign: (userId: number | string) => void;
  currentAssigneeId?: number | string;
}
      ...
      <div className="bg-slate-900 border border-slate-700 rounded-2xl w-full max-w-md shadow-2xl overflow-hidden flex flex-col max-h-[80vh]">
          ...
          {isLoading ? (
            ...
            filteredUsers.map((user) => (
              ...
              >
                <div className="flex items-center gap-3">
                  ...
                  <div className="text-left">
                    ...
                    <div className="text-xs opacity-70">
                      Interno: {user.internalNumber || 'N/A'}
                    </div>
                  </div>
                </div>
                ...
                {selectedUserId === user.id && <CheckCircle className="w-5 h-5 text-primary-500" />}
              </button>
            ))
          ) : (
        ...
        <div className="p-4 border-t border-slate-700 bg-slate-800/50">
          ...
          >
            ...
            <span>
              {currentAssigneeId && selectedUserId !== currentAssigneeId
                ? 'Reasignar Conductor'
                : 'Asignar Conductor'}
            </span>
          </button>
