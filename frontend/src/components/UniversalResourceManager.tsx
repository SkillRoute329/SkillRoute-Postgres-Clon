...
const UniversalResourceManager = ({ entityKey }: Props) => {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      // Safety guard: If API returns unexpected structure or 404 handled by interceptor as null/generic error
      if (res && res.data) {
        setData(res.data || []);
        setTotal((res as { data: unknown[]; meta?: { total?: number } }).meta?.total ?? 0);
      } else {
        console.warn('⚠️ [UniversalResourceManager] Received invalid data structure:', res);
        setData([]);
        setTotal(0);
      }
    } catch (error: any) {
      console.error('⛔ [UniversalResourceManager] Critical Load Error:', error);

      // Check for 404 or specific error to stop retrying if necessary
      // The useEffect logic [entityKey, page] doesn't natively loop unless something resets page/entityKey
      // but if the parent component unmounts/remounts due to error boundary, it could loop.

      if (String(error).includes('404')) {
        console.error('🛑 Backend route missing. Stopping.');
      }
      setData([]); // Ensure no undefined data crashes filter logic
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDelete = async (item: any) => {
    ...
    if (isMasterRoute) {
      if (
        !window.confirm(
          `⚠️ ESTA ACCIÓN ES IRREVERSIBLE. ¿Realmente desea ELIMINAR LA LÍNEA ${itemName}?`,
        )
      )
        return;
      if (
        !window.confirm(
          `¿CONFIRMA borrar todos los trazados, paradas y alertas asociadas a la LÍNEA ${itemName}?`,
        )
      )
        return;
    } else {
  ...
  if (!config) {
    return (
      <div className="p-10 text-center text-red-500">
        Configuración no encontrada para la entidad: {entityKey}
      </div>
    );
  }
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-slate-800/50 p-6 rounded-2xl border border-slate-700">
        <div>
          ...
          <p className="text-slate-400 text-sm">
            Administración centralizada de {config.labels.plural.toLowerCase()}
          </p>
        </div>
