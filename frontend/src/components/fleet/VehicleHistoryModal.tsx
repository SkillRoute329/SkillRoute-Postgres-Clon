...
const VehicleHistoryModal: React.FC<VehicleHistoryModalProps> = ({ vehicleId, vehicleNumber, onClose }) => {
            ...
            <div className="bg-slate-900 border border-slate-700 rounded-2xl w-full max-w-4xl max-h-[90vh] shadow-2xl overflow-hidden flex flex-col animate-scale-in">
                ...
                <div className="p-6 border-b border-slate-800 flex justify-between items-center bg-slate-950/50">
                    <div>
                        ...
                        <p className="text-slate-400 text-sm">Registro de inspecciones y personal que operó la unidad.</p>
                    </div>
                    ...
                    {loading ? (
                        ...
                        <div className="space-y-6">
                            {history.map((inspection) => (
                                <div key={inspection.id} className="bg-slate-800/50 border border-slate-700 rounded-xl p-5 hover:border-slate-500 transition-all">
                                    <div className="flex flex-col md:flex-row justify-between gap-4 mb-4">
                                        <div className="flex items-center gap-4">
                                            ...
                                            <div>
                                                <div className="text-white font-bold text-lg">{inspection.user?.fullName}</div>
                                                <div className="text-sm text-slate-400">Interno: {inspection.user?.internalNumber}</div>
                                            </div>
                                        </div>
                                        <div className="text-right">
                                            <div className="text-primary-400 font-bold">
                                                {format(new Date(inspection.createdAt), "EEEE d 'de' MMMM, HH:mm", { locale: es })}
                                            </div>
                                            <div className={inspection.status === 'OK' ? 'text-emerald-400 text-sm font-bold' : 'text-orange-400 text-sm font-bold'}>
                                    ...
                                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-4 bg-slate-900/50 p-3 rounded-lg border border-slate-800">
                                        <div>
                                            <div className="text-[10px] text-slate-500 uppercase font-bold">Kilometraje</div>
                                            <div className="text-white font-mono">{inspection.odometer?.toLocaleString() || 'N/A'} Km</div>
                                        </div>
                                        <div>
                                            <div className="text-[10px] text-slate-500 uppercase font-bold">Combustible</div>
                                            <div className="text-white">{inspection.fuelLevel || 'N/A'}</div>
                                        </div>
                                        <div>
                                            <div className="text-[10px] text-slate-500 uppercase font-bold">Tipo</div>
                                            <div className="text-slate-300 text-xs">{inspection.type === 'StartShift' ? 'Inicio de Turno' : 'Fin de Turno'}</div>
                                        </div>
                                        <div>
                                            <div className="text-[10px] text-slate-500 uppercase font-bold">ID Registro</div>
                                        ...
                                        <div className="mt-4 space-y-3">
                                            ...
                                            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                                                {inspection.damages.map((d: any) => (
                                                    <div key={d.id} className="flex gap-3 bg-red-500/5 border border-red-500/20 p-3 rounded-lg">
                                                        {d.photoUrl && (
                                                            <img src={d.photoUrl} alt="Daño" className="w-20 h-20 object-cover rounded-lg border border-red-500/30 shrink-0" />
                                                        )}
                                                        <div>
                                                            <div className="text-white font-bold text-sm">[{d.zone}]</div>
                                                            <p className="text-xs text-slate-400 leading-relaxed">{d.description}</p>
                                                            <span className="text-[10px] bg-red-500/20 text-red-300 px-1.5 py-0.5 rounded mt-1 inline-block">Severidad: {d.severity}</span>
                                                        </div>
                                                    </div>
                                                ))}
                                            </div>
