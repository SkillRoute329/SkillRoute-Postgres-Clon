...
const DigitalCarton = ({ data: initialData, isEditable = false, onSave }: DigitalCartonProps) => {
    ...
    return (
        <div className="relative group p-6 select-none">
            ...
            <div className={`p-4 border-2 border-slate-900 bg-white text-black font-mono shadow-2xl max-w-full overflow-x-auto mx-auto transition-all ${isEditing ? 'ring-4 ring-blue-500/20 scale-[1.005]' : ''}`}>

                {/* Header Section */}
                <div className="flex justify-between items-end border-b-2 border-black pb-2 mb-2 min-w-[800px]">
                    <div className="text-center w-32">
                        <p className="text-xs font-bold uppercase tracking-wider text-slate-600">Linea</p>
                        {isEditing ? (
                            <input
                                value={data.line}
                                onChange={e => handleChange('line', e.target.value)}
                                className="text-4xl font-black text-blue-900 w-full text-center border-b border-blue-300 focus:outline-none bg-blue-50"
                            />
                        ) : (
                            <p className="text-4xl font-black text-blue-900">{data.line}</p>
                        )}
                    </div>

                    <div className="flex-1 flex flex-col items-center justify-end px-4">
                        <p className="text-[10px] font-bold mb-1 text-slate-500">U.C.O.T. MOBILE</p>
                        <div className="w-full bg-[#8ce655] border-2 border-black py-1.5 px-6 font-black text-xl shadow-[3px_3px_0px_0px_rgba(0,0,0,1)] text-center uppercase tracking-tight">
                            {isEditing ? (
                                <input
                                    value={data.title}
                                    onChange={e => handleChange('title', e.target.value)}
                                    className="w-full bg-transparent text-center focus:outline-none border-b border-black/20 placeholder-black/30 font-black text-xl"
                                />
                            ) : data.title}
                        </div>
                    </div>
                    ...
                    <div className="text-center w-32">
                        <p className="text-xs font-bold uppercase tracking-wider text-slate-600">Servicio N°</p>
                        {isEditing ? (
                            <input
                                value={data.serviceNumber}
                                onChange={e => handleChange('serviceNumber', e.target.value)}
                                className="text-4xl font-black text-black w-full text-center border-b border-blue-300 focus:outline-none bg-blue-50"
                            />
                        ) : (
                            <p className="text-4xl font-black">{data.serviceNumber}</p>
                        )}
                    </div>
                </div>

                {/* Grid Section */}
                <div className="border-2 border-black mb-1 overflow-x-auto relative min-w-[800px]">
                    <table className="w-full text-center text-xs border-collapse table-fixed">
                        <thead>
                            <tr>
                                {data.headers.map((point, idx) => (
                                    <th key={point.id} className="border-r border-black/50 p-1 bg-white align-bottom h-36 w-12 relative group/col overflow-visible">
                                        {isEditing && (
                                            <button
                                        ...
                                        <div className="h-full w-full flex items-end justify-center pb-2">
                                            <div className="[writing-mode:vertical-rl] transform rotate-180 whitespace-nowrap text-left font-bold text-[11px] h-32 w-4 flex items-center">
                                                {isEditing ? (
                                                    <input
                        ...
                        <tbody>
                            {data.rows.map((row, rowIdx) => (
                                <tr key={row.id} className="h-8 group/row">
                                    {data.headers.map((point) => {
                                        ...
                                        return (
                                            <td
                                                ...
                                                // Drag & Drop Hooks
                                                draggable={isEditing && isRelief} // Only drag existing reliefs
                                                ...
                                                className={clsx(
                                                    ...
                                                    isRelief && "border-[3px] border-red-600 z-10 cursor-move" // Cursor move for draggables
                                                )}
                                            >
                                                {isEditing && !isReliefMode ? (
                                                    <input
                                                        ...
                                                        className="w-full h-full text-center font-bold focus:outline-none bg-transparent text-[11px] min-w-[40px]"
                                                        placeholder="--:--"
                                                    />
                                                ) : (
                                                    <span className={clsx(
                                                        "block w-full h-full py-1.5 font-bold text-[11px]",
                ...
                {isEditing && (
                    <div className="flex justify-center mb-4">
                        <button
                            ...
                            className="flex items-center gap-2 text-xs font-bold text-blue-600 hover:bg-blue-50 px-3 py-1 rounded border border-blue-200 transition-colors"
                        >
                            <Plus className="w-4 h-4" /> Agregar Horario (Fila)
                        </button>
                    </div>
                )}

                {/* Timings Bar */}
                <div className="border-2 border-black bg-white p-1.5 mb-2 text-center font-bold text-sm flex items-center justify-center gap-1 min-w-[800px]">
                    ...
                    {isEditing ? (
                        <div className="flex gap-2 items-center bg-blue-50 px-2 rounded ml-2 flex-1">
                            <input
                                ...
                                className="w-16 text-center font-bold text-red-600 bg-transparent focus:outline-none text-xl"
                                placeholder="00:00"
                            />
                            <input
                            ...
                            />
                            {/* Explicit END TIME Input */}
                            <span className="pl-4 border-l border-blue-200">GUARDA:</span>
                            <input
                                value={data.endTime || ''}
                                onChange={e => handleChange('endTime', e.target.value)}
                                className="w-16 text-center font-bold text-red-600 bg-transparent focus:outline-none text-xl ml-1"
                                placeholder="00:00"
                            />
                        </div>
                    ) : (
                        <>
                            <span className="text-red-600 text-lg mx-1">{data.startTime}</span>
                            ...
                            <span className="text-red-600 text-lg mx-1">{data.endTime || '??:??'}</span>
                        </>
                    )}
                </div>

                {/* SHIFT SUMMARY CALCULATION */}
                <div className="border-2 border-black border-t-0 p-1 bg-slate-50 min-w-[800px]">
                    <h4 className="font-bold text-xs uppercase mb-1 border-b border-black pb-0.5">Cálculo de Turnos (Automático)</h4>
                    <div className="grid grid-cols-1 gap-1">
                        {shifts.map((shift, i) => (
                            <div key={i} className="flex items-center text-[10px] uppercase font-bold border-b border-black/10 last:border-0 pb-0.5">
                                <div className="w-20 bg-blue-100 text-center mr-2 rounded text-blue-800">
                                    TURNO {shift.index}
                                </div>
                                <div className="w-32 text-center border-r border-black/20 pr-2">
                                    <span className="text-red-700">{shift.start}</span> a <span className="text-red-700">{shift.end}</span>
                                </div>
                                <div className="w-24 text-center border-r border-black/20 px-2 flex flex-col justify-center">
                                    <span>{shift.duration} hs</span>
                                    {shift.extra && (
                                        <span className="text-[9px] text-red-600 bg-red-100 rounded px-1 font-bold whitespace-nowrap mt-0.5">
                                            + {shift.extra} EXTRA
                                        </span>
                                    )}
                                </div>
                                <div className="flex-1 pl-2">
                                    {isEditing ? (
                                        <input
                                            ...
                                            className="w-full bg-transparent border-b border-dashed border-slate-300 focus:outline-none focus:border-blue-500 placeholder-slate-400"
                ...
                {/* Footer Info */}
                <div className="border-t-2 border-black pt-2 mt-2 font-bold grid grid-cols-1 gap-1 min-w-[800px]">
                    <div className="bg-[#fff9c4] border border-black p-1 text-[10px] text-center w-full uppercase">
                        Es obligacion salir con expendedora encendida y cerrar ventanillas del coche al guardar en el terreno
                    </div>

                    <div className="flex justify-between items-center bg-slate-50 border border-slate-300 p-2 text-xs uppercase mt-1">
                        <div className="flex gap-2 items-center">
                            <span>Total de Horas:</span>
                            {isEditing ? <input value={data.totalHours} onChange={e => handleChange('totalHours', e.target.value)} className="w-16 bg-white border px-1" /> : <span>{data.totalHours}</span>}
                        </div>
                        <div className="flex gap-2 items-center">
                            <span>Esperas:</span>
                            {isEditing ? <input value={data.waitingTime} onChange={e => handleChange('waitingTime', e.target.value)} className="w-16 bg-white border px-1" /> : <span>{data.waitingTime}</span>}
                        </div>
                        <div className="flex gap-2 items-center">
                            <span>Horas Líquidas:</span>
                            {isEditing ? <input value={data.liquidHours} onChange={e => handleChange('liquidHours', e.target.value)} className="w-16 bg-white border px-1" /> : <span>{data.liquidHours}</span>}
                        </div>
                        <div className="flex gap-2 items-center">
                            <span>Kilómetros:</span>
                            {isEditing ? <input value={data.kilometers} onChange={e => handleChange('kilometers', e.target.value)} className="w-32 bg-white border px-1" /> : <span>{data.kilometers}</span>}
                        </div>
                    </div>
                </div>
