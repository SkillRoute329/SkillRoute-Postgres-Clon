...
const DigitalCarton = ({ data: initialData, isEditable = false, onSave }: DigitalCartonProps) => {
  ...
  return (
    <div className="relative group p-1 md:p-6 select-none">
          ...
          {isEditing ? (
            <>
              ...
              <button
                ...
                className="bg-slate-700 text-white px-3 py-1.5 rounded-lg flex items-center gap-2 text-sm hover:bg-slate-600 transition-colors"
              >
                <RotateCcw className="w-4 h-4" /> Restaurar
              </button>
              <button
                onClick={handleSave}
                className="bg-green-600 text-white px-3 py-1.5 rounded-lg flex items-center gap-2 text-sm hover:bg-green-500 shadow-lg shadow-green-900/20 font-bold transition-transform active:scale-95"
              >
            ...
            <button
              ...
              className="bg-blue-600 text-white px-3 py-1.5 rounded-lg flex items-center gap-2 text-sm hover:bg-blue-500 shadow-lg shadow-blue-900/20 font-bold transition-transform active:scale-95"
            >
      ...
      <div
        className={`p-1 md:p-4 border-2 border-slate-900 bg-white text-black font-mono shadow-2xl w-full mx-auto transition-all ${isEditing ? 'ring-4 ring-blue-500/20 scale-[1.005]' : ''}`}
      >
        {/* Header Section */}
        <div className="w-full overflow-x-auto touch-pan-x mb-2">
          <div className="flex justify-between items-end border-b-2 border-black pb-2 min-w-[600px] md:min-w-[800px]">
            <div className="text-center w-32">
              <p className="text-[10px] md:text-xs font-bold uppercase tracking-wider text-slate-600">
                Linea
              </p>
              {isEditing ? (
                <input
            ...
            <div className="flex-1 flex flex-col items-center justify-end px-4">
              <p className="text-[9px] md:text-[10px] font-bold mb-1 text-slate-500 text-center">
                U.C.O.T. MOBILE
              </p>
        ...
        <div className="w-full overflow-x-auto touch-pan-x border-2 border-black mb-1 relative">
            ...
            <thead>
              <tr>
                {data.headers.map((point, idx) => (
                  <th
                    key={point.id}
                    className="border-r border-black/50 p-1 bg-white align-bottom h-28 md:h-36 w-10 md:w-12 relative group/col overflow-visible"
                  >
                    {isEditing && (
                      <button
                        onClick={(e) => {
                          e.stopPropagation();
                          deleteColumn(point.id);
                        }}
          ...
          {isEditing && (
            <div
              className="absolute top-0 right-0 h-full w-8 bg-slate-100 border-l border-black flex flex-col items-center justify-center gap-2 opacity-0 hover:opacity-100 transition-opacity z-10 cursor-pointer"
              onClick={() => {
          ...
          <div className="border-2 border-black bg-white p-1 md:p-1.5 text-center font-bold text-[10px] md:text-sm flex items-center justify-center gap-1 min-w-[600px] md:min-w-[800px]">
            ...
            {isEditing ? (
              ...
              <>
                ...
                <span className="text-red-600 text-base md:text-lg mx-1">
                  {data.endTime || '??:??'}
                </span>
              </>
            )}
        ...
        <div className="w-full overflow-x-auto touch-pan-x mb-2">
          <div className="border-2 border-black border-t-0 p-1 bg-slate-50 min-w-[600px] md:min-w-[800px]">
            <h4 className="font-bold text-[9px] md:text-xs uppercase mb-1 border-b border-black pb-0.5">
              Cálculo de Turnos (Automático)
            </h4>
            <div className="grid grid-cols-1 gap-1">
              {shifts.map((shift, i) => (
                <div
                  key={i}
                  className="flex items-center text-[9px] md:text-[10px] uppercase font-bold border-b border-black/10 last:border-0 pb-0.5"
                >
                  ...
                  <div className="w-24 md:w-32 text-center border-r border-black/20 pr-2">
                    <span className="text-red-700">{shift.start}</span> a{' '}
                    <span className="text-red-700">{shift.end}</span>
                  </div>
                  ...
                  <div className="flex-1 pl-2">
                    {isEditing ? (
                      ...
                      <span className="text-slate-700">
                        {data.shiftDescriptions?.[i] || shift.defaultDesc}
                      </span>
                    )}
                  </div>
                </div>
        ...
        <div className="w-full overflow-x-auto touch-pan-x mb-2">
          <div className="border-t-2 border-black pt-2 mt-2 font-bold grid grid-cols-1 gap-1 min-w-[600px] md:min-w-[800px]">
            <div className="bg-[#fff9c4] border border-black p-1 text-[9px] md:text-[10px] text-center w-full uppercase leading-tight">
              Es obligacion salir con expendedora encendida y cerrar ventanillas del coche al
              guardar en el terreno
            </div>
