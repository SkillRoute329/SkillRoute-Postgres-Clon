    ...
    return (
        <div className="glass-panel p-5 rounded-2xl border border-slate-700/50 hover:border-primary-500/30 transition-all duration-300 group mb-4 shadow-lg shadow-black/20">
            ...
            <div className="flex justify-between items-start mb-3 border-b border-slate-700/50 pb-3">
                ...
                <span className={clsx("text-[10px] font-bold px-3 py-1 rounded-full border shadow-sm uppercase tracking-wide", getStatusColor(shift.status))}>
                    {shift.status === 'Created' ? 'Pendiente' :
                        shift.status === 'Public' ? 'Disponible' :
                            shift.status === 'Assigned' ? 'Asignado' : shift.status}
                </span>
            </div>
            ...
            <div className="flex justify-between items-end mb-4">
                <div>
                    <div className="px-3 py-1 rounded-lg bg-emerald-500/10 border border-emerald-500/20 text-emerald-400 font-black text-2xl inline-flex items-center gap-1">
                        <span className="text-lg opacity-70">$</span>{formattedTotal}
                    </div>
                    {shift.extraHours > 0 && (
            ...
            <div className="bg-slate-900/50 rounded-xl p-4 border border-slate-700/50 space-y-3 mb-5">
                <div className="flex items-center justify-between">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <div className="text-[10px] text-slate-500 uppercase font-bold tracking-wider">Línea / Coche</div>
                            <div className="text-white font-bold">{shift.line} <span className="text-slate-600">/</span> {shift.carNumber}</div>
                        </div>
                    </div>
                    <div className="text-right">
                        <div className="text-[10px] text-slate-500 uppercase font-bold tracking-wider">Fecha</div>
                ...
                <div className="flex items-center justify-between">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <div className="text-[10px] text-slate-500 uppercase font-bold tracking-wider">Horario</div>
                            <div className="text-white font-bold flex items-center gap-2">
                                {shift.time} Hs
                                {shift.endTime && <span className="text-slate-500 font-normal text-xs">➔ {shift.endTime} Hs</span>}
                            </div>
                        </div>
                    </div>
                    {shift.relief && (
                        <div className="text-right">
                            <div className="text-[10px] text-slate-500 uppercase font-bold tracking-wider">Relevo</div>
            ...
            <div className="flex flex-col gap-3">
                ...
                {variant === 'admin' && (shift.status === 'Assigned' || shift.status === 'Completed') && shift.assigneePhone && (
                    <button
                        onClick={() => {
                            const phone = shift.assigneePhone?.replace(/\D/g, '');
                            if (!phone) return alert('Número de teléfono inválido');

                            const message = `👋 Hola ${shift.assigneeName || 'Chofer'}, tienes un nuevo turno asignado:\n` +
