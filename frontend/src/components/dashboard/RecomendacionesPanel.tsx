...
import { RecomendacionEjecutiva } from '../../types/dashboard';
import { TrendingUp, Clock, Target, AlertTriangle, CheckCircle, Zap } from 'lucide-react';
...
export function RecomendacionesPanel({ recomendaciones }: RecomendacionesPanelProps) {
  ...
  const recomendacionesFiltradas = filtroUrgencia === 'todas'
    ? recomendaciones
    : recomendaciones.filter(r => r.urgencia === filtroUrgencia);

  // Ordenar por impacto (descendente)
  const recomendacionesOrdenadas = [...recomendacionesFiltradas].sort((a, b) => b.impacto - a.impacto);
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="bg-white rounded-lg shadow p-4 border-l-4 border-green-500">
          ...
          <p className="text-3xl font-bold text-green-600">${(resumen.impactoTotal / 1000).toFixed(0)}K</p>
        </div>
      </div>
              ...
              <div className="flex items-start justify-between mb-4">
                <div className="flex items-start gap-4">
                  ...
                  <div className="flex-1">
                    <h3 className={`text-lg font-bold mb-2 ${getUrgenciaTextColor(rec.urgencia)}`}>
                      {rec.titulo}
                    </h3>
              ...
              <div className="px-6 py-4 bg-white bg-opacity-40 border-t border-current border-opacity-30 space-y-4">
                ...
                <div className="grid grid-cols-3 gap-3">
                  <div className="bg-white bg-opacity-60 rounded p-3 text-center">
                    ...
                    <p className="text-2xl font-bold text-green-600">${(rec.impacto / 1000).toFixed(0)}K</p>
                  </div>
                  <div className="bg-white bg-opacity-60 rounded p-3 text-center">
                ...
                <div className="bg-white bg-opacity-60 rounded p-4">
                  <div className="flex items-center justify-between mb-2">
                    <p className="text-sm font-bold text-gray-900">ROI Estimado (Éxito × Impacto):</p>
                  ...
                  <div className="w-full bg-gray-300 rounded-full h-2">
                    <div
                      ...
                      style={{
                        width: `${(rec.probabilidadExito * rec.impacto) / (resumen.impactoTotal * 100) * 100}%`
                      }}
                    ></div>
                  </div>
                </div>
      ...
      {recomendacionesOrdenadas.length === 0 && (
        <div className="bg-green-50 border border-green-200 rounded-lg p-8 text-center">
          ...
          <p className="text-green-800 font-semibold">✓ No hay recomendaciones con los filtros seleccionados</p>
        </div>
      )}
    </div>
  );
