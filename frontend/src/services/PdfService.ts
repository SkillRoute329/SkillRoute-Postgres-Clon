...
export const PdfService = {
    generateBoletinOficial: async (data: any[], seasonName: string = 'VERANO 2026', dayType: string = 'HÁBIL') => {
        const doc = new jsPDF();
        ...
        const tableBody = await Promise.all(data.map(async (item) => {
            ...
            let qrImage = '';
            try {
                qrImage = await QRCode.toDataURL(qrData, { margin: 1, width: 64, errorCorrectionLevel: 'L' });
            } catch (e) {
        ...
        autoTable(doc, {
            startY: 45,
            head: [['Link', 'Línea', 'Servicio', 'Hora Salida', 'Tipo Coche', 'Variante / Recorrido', 'Chofer Asignado']],
            body: tableBody,
            theme: 'grid',
            styles: {
                font: 'helvetica',
