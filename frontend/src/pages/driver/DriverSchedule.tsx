        ...
        <div className="space-y-4 pb-24 animate-fade-in-up md:max-w-xl md:mx-auto">
            ...
            <div className="bg-slate-900/80 backdrop-blur sticky top-0 z-10 p-4 border-b border-slate-800 rounded-b-2xl shadow-lg">
                <div className="flex justify-between items-center mb-4">
                    ...
                    <div className="flex gap-2">
                        <button
                            ...
                            className={clsx("px-3 py-1.5 rounded-lg text-xs font-bold transition-colors", viewMode === 'week' ? "bg-indigo-600 text-white" : "bg-slate-800 text-slate-400")}
                        >
                            Semana
                        </button>
                        <button
                            onClick={() => setViewMode('month')}
                            className={clsx("px-3 py-1.5 rounded-lg text-xs font-bold transition-colors", viewMode === 'month' ? "bg-indigo-600 text-white" : "bg-slate-800 text-slate-400")}
                        >
                            Mes
                        </button>
                    </div>
                ...
                <div className="flex items-center justify-between bg-slate-800/50 rounded-xl p-2 md:p-3">
                    <button onClick={() => setCurrentDate(new Date(currentDate.setMonth(currentDate.getMonth() - 1)))} className="p-1 hover:bg-slate-700 rounded-lg"><ChevronDown className="rotate-90 w-5 h-5 text-slate-400" /></button>
                                ...
                                >
                                    ...
                                    <div className="flex justify-between items-start relative z-10">
                                        ...
                                        <div className="flex flex-col items-center justify-center bg-slate-900/50 rounded-xl p-2 min-w-[3.5rem] border border-slate-700/50">
                                            <span className="text-xs font-bold text-slate-400 uppercase">{item.dayOfWeek}</span>
                                        ...
                                        <div className="flex-1 px-4 flex flex-col justify-center">
                                            <div className="flex items-center gap-2 mb-1">
                                                <span className="text-3xl font-black text-white tracking-tighter">{item.startTime}</span>
                                                <span className="text-xs text-slate-500 font-medium">HS</span>
                                            </div>
                                            <div className="flex items-center gap-2">
                                                <span className={clsx("text-xs px-2 py-0.5 rounded font-bold uppercase", getShiftBadge(item.shiftType))}>
                                                    {item.shiftType}
                                                </span>
                                        ...
                                        <div className="flex flex-col items-end justify-center">
                                            <span className="text-xs text-slate-500 font-bold uppercase tracking-wider">Servicio</span>
                                    ...
                                    {expandedId === item.id && (
                                        <div className="mt-4 pt-4 border-t border-slate-700/50 space-y-3 animate-in fade-in slide-in-from-top-2">
                                            ...
                                            <div className="bg-slate-900/40 p-3 rounded-xl border border-slate-700/50">
                                                <span className="text-xs text-slate-500 block mb-1">Recorrido / Variante</span>
