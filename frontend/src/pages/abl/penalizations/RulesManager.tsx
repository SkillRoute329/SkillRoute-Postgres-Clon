...
const RulesManager = () => {
  ...
  const handleDelete = async (id: number) => {
    ...
    await PenaltyService.deleteRule(String(id));
    loadRules();
  };
    ...
    <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6">
      ...
      <div className="space-y-2">
        {rules.map((rule) => (
          <div
            key={rule.id}
            className="flex justify-between items-center p-3 bg-slate-800/50 rounded-lg border border-slate-800"
          >
            <div>
              <div className="font-medium text-white">{rule.name}</div>
              <div className="text-xs text-slate-400">
                {rule.type} &gt; {rule.threshold} min | Max: {rule.maxCount} veces en{' '}
                {rule.periodDays} días
              </div>
            </div>
            <button
        ...
        {!loading && rules.length === 0 && (
          <div className="text-center text-slate-500 text-sm">Sin reglas definidas.</div>
        )}
      </div>
    </div>
  );
