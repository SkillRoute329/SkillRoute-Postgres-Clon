...
const StatCard = ({ title, value, subtext, icon: Icon, colorClass }: any) => (
    <div className="bg-slate-900 border border-slate-800 p-6 rounded-2xl relative overflow-hidden group hover:border-slate-700 transition-colors">
        <div className={clsx("absolute top-0 right-0 p-4 opacity-10 group-hover:opacity-20 transition-opacity", colorClass)}>
...
const ABLPage = () => {
    ...
    const isAdmin = user?.role === 'Admin' || user?.role === 'SuperAdmin' || user?.role === 'TrafficChief';
    ...
    if (loading && !stats) return <div className="p-8 text-center text-slate-500">Cargando ABL...</div>;
        ...
        <div className="space-y-8 animate-fade-in-up pb-24">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                <StatCard
                    ...
                    subtext={stats?.onTimeRate > 90 ? "Excelente" : stats?.onTimeRate > 75 ? "Regular" : "Atención Requerida"}
                    icon={Clock}
                    colorClass={stats?.onTimeRate > 90 ? "text-emerald-500" : stats?.onTimeRate > 75 ? "text-yellow-500" : "text-red-500"}
                />
                <StatCard
                        ...
                        {busStats && (
                            <div className="grid grid-cols-3 gap-4 animate-fade-in">
                                <div className="bg-slate-800 p-4 rounded-xl text-center">
                                    ...
                                    <div className="text-2xl font-bold text-white">{100 - (busStats.avgDelay > 5 ? 20 : 0)}%</div>
                                </div>
                                <div className="bg-slate-800 p-4 rounded-xl text-center">
