...
const AdminConfig = () => {
    ...
    const [newHistoryPrice, setNewHistoryPrice] = useState({ effectiveDate: '', baseValue: '', extraHourValue: '' });
    ...
    const loadData = async () => {
        ...
        try {
            ...
            if (config && config.TRANSFORMA_FACIL_DISCOUNT) setTransformaFacilDiscount(config.TRANSFORMA_FACIL_DISCOUNT);
        } catch (error) {
    ...
    const handleAddHistory = async (e: React.FormEvent) => {
        ...
        try {
            ...
            setNewHistoryPrice({ effectiveDate: '', baseValue: newHistoryPrice.baseValue, extraHourValue: newHistoryPrice.extraHourValue });
    ...
    const handleDeleteCategory = async () => {
        if (!editingCategory) return;
        if (!confirm(`¿ELIMINAR DEFINITIVAMENTE la categoría "${editingCategory.name}"?\n\nEsta acción no se puede deshacer.`)) return;
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <div className="glass-panel p-6 rounded-2xl border border-slate-800">
                    ...
                    <div className="space-y-2">
                        {categories.map(cat => (
                            <div key={cat.id} className="group flex items-center justify-between p-4 bg-slate-900/40 rounded-xl border border-transparent hover:border-slate-700 transition-all hover:bg-slate-900/60">
                                <div>
                                    <div className="font-bold text-white text-base">{cat.name}</div>
                                    <div className="text-xs text-slate-500 mt-0.5 flex gap-3">
                                        <span>Base: <span className="text-slate-300 font-medium">${Number(cat.baseValue).toLocaleString()}</span></span>
                                        <span>Extra: <span className="text-slate-300 font-medium">${Number(cat.extraHourValue).toLocaleString()}</span></span>
                                    </div>
                                </div>
                                <button
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm animate-fade-in">
                    <div className="bg-slate-900 border border-slate-700 rounded-2xl w-full max-w-md p-6 shadow-2xl relative">
                        ...
                        <form onSubmit={handleCreateCategory} className="space-y-5">
                            <div>
                                <label className="text-sm text-slate-400 font-medium block mb-1.5">Nombre de la Categoría</label>
                                <input
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                <div>
                                    <label className="text-sm text-slate-400 font-medium block mb-1.5">Valor Base ($)</label>
                                    <input
                                ...
                                <div>
                                    <label className="text-sm text-slate-400 font-medium block mb-1.5">Hora Extra ($)</label>
                                    <input
                                        type="number" required step="0.01"
                                        value={newCategory.extraHourValue}
                                        onChange={e => setNewCategory({ ...newCategory, extraHourValue: e.target.value })}
                                        className="input-field w-full bg-slate-800 border-slate-600 text-white"
                                        placeholder="0.00"
                                    />
                                </div>
                            ...
                            <div className="flex justify-end gap-3 pt-4 border-t border-slate-800">
                                <button type="button" onClick={() => setIsCreatingCategory(false)} className="px-4 py-2 text-slate-400 hover:text-white transition-colors">Cancelar</button>
                                <button type="submit" className="bg-emerald-600 hover:bg-emerald-500 text-white px-6 py-2 rounded-lg font-bold shadow-lg shadow-emerald-900/20">Crear Categoría</button>
                            </div>
                        </form>
                    </div>
                </div>
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm animate-fade-in">
                    <div className="bg-slate-900 border border-slate-700 rounded-2xl w-full max-w-2xl max-h-[90vh] flex flex-col shadow-2xl overflow-hidden">
                        ...
                        <div className="p-4 border-b border-slate-800 flex justify-between items-center bg-slate-900">
                            ...
                            <button onClick={() => setEditingCategory(null)} className="text-slate-400 hover:text-white p-1 rounded-full hover:bg-slate-800 transition-colors">
                                <X className="w-6 h-6" />
                            </button>
                        </div>
                        ...
                        <div className="flex border-b border-slate-800 bg-slate-900/50">
                            <button
                                ...
                                className={clsx("flex-1 py-3 text-sm font-bold border-b-2 transition-colors", editTab === 'general' ? "border-primary-500 text-white bg-slate-800" : "border-transparent text-slate-500 hover:text-slate-300 hover:bg-slate-800/50")}
                            >
                                General
                            </button>
                            <button
                                onClick={() => setEditTab('history')}
                                className={clsx("flex-1 py-3 text-sm font-bold border-b-2 transition-colors flex items-center justify-center gap-2", editTab === 'history' ? "border-emerald-500 text-white bg-slate-800" : "border-transparent text-slate-500 hover:text-slate-300 hover:bg-slate-800/50")}
                            >
                        ...
                        <div className="p-6 overflow-y-auto flex-1">
                            ...
                            {editTab === 'general' && (
                                <div className="space-y-8">
                                    <form id="edit-form" onSubmit={handleSaveGeneralChanges} className="space-y-5">
                                        <div>
                                            <label className="text-sm text-slate-400 font-medium block mb-1.5">Nombre</label>
                                            <input
                                                type="text" required
                                        ...
                                        <div className="grid grid-cols-2 gap-4">
                                            <div>
                                                <label className="text-sm text-slate-400 font-medium block mb-1.5">Valor Base Actual ($)</label>
                                                <input
                                                    type="number" required step="0.01"
                                                    value={editFormData.baseValue}
                                                    onChange={e => setEditFormData({ ...editFormData, baseValue: e.target.value })}
                                                    className="input-field w-full bg-slate-800 border-slate-600 text-white"
                                                />
                                            </div>
                                            <div>
                                                <label className="text-sm text-slate-400 font-medium block mb-1.5">Valor H. Extra Actual ($)</label>
                                                <input
                                                    type="number" required step="0.01"
                                                    value={editFormData.extraHourValue}
                                                    onChange={e => setEditFormData({ ...editFormData, extraHourValue: e.target.value })}
                                                    className="input-field w-full bg-slate-800 border-slate-600 text-white"
                                                />
                                            </div>
                                        </div>
                                        <div className="bg-blue-900/10 border border-blue-500/20 rounded-lg p-3 flex gap-3 text-sm text-blue-300">
                                            ...
                                            <p>Nota: Modificar los valores aquí afectará el precio "por defecto" actual. Para programar un aumento a futuro, usa la pestaña "Historial".</p>
                                        </div>
                                    </form>

                                    {/* Danger Zone */}
                                    <div className="pt-8 border-t border-slate-800">
                                        <h4 className="text-red-400 text-xs font-bold uppercase tracking-wider mb-3">Zona de Peligro</h4>
                            ...
                            {editTab === 'history' && (
                                <div className="space-y-6">
                                    {/* Add History Form */}
                                    <form onSubmit={handleAddHistory} className="bg-slate-800/30 p-4 rounded-xl border border-slate-700">
                                        <h4 className="text-sm font-bold text-emerald-400 mb-3 flex items-center gap-2">
                                            <Plus className="w-4 h-4" /> Programar Cambio de Precio
                                        </h4>
                                        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                                            <div>
                                                <label className="text-[10px] text-slate-400 uppercase font-bold block mb-1">A partir de</label>
                                                <input
                                                    type="date" required
                                                    value={newHistoryPrice.effectiveDate}
                                                    onChange={e => setNewHistoryPrice({ ...newHistoryPrice, effectiveDate: e.target.value })}
                                                    className="w-full bg-slate-900 border border-slate-600 rounded-lg py-1.5 px-2 text-sm text-white"
                                                />
                                            </div>
                                            <div>
                                                <label className="text-[10px] text-slate-400 uppercase font-bold block mb-1">Nuevo Base</label>
                                                <input
                                                    type="number" required placeholder="0.00" step="0.01"
                                                    value={newHistoryPrice.baseValue}
                                                    onChange={e => setNewHistoryPrice({ ...newHistoryPrice, baseValue: e.target.value })}
                                                    className="w-full bg-slate-900 border border-slate-600 rounded-lg py-1.5 px-3 text-sm text-white"
                                                />
                                            </div>
                                            <div>
                                                <label className="text-[10px] text-slate-400 uppercase font-bold block mb-1">Nueva Extra</label>
                                                <input
                                                    type="number" required placeholder="0.00" step="0.01"
                                                    value={newHistoryPrice.extraHourValue}
                                                    onChange={e => setNewHistoryPrice({ ...newHistoryPrice, extraHourValue: e.target.value })}
                                                    className="w-full bg-slate-900 border border-slate-600 rounded-lg py-1.5 px-3 text-sm text-white"
                                                />
                                            </div>
                                        </div>
                                        <button type="submit" className="w-full mt-3 bg-emerald-600/20 hover:bg-emerald-600 text-emerald-400 hover:text-white border border-emerald-500/30 py-2 rounded-lg text-sm font-bold transition-all">
                                            Programar
                                        </button>
                                    </form>
                                    ...
                                    {/* List */}
                                    <div>
                                        <div className="text-xs text-slate-500 font-bold uppercase tracking-wider mb-2">Cronología de Precios</div>
                                        {isLoadingHistory ? (
                                            <div className="text-center py-4 text-slate-500">Cargando...</div>
                                        ) : history.length === 0 ? (
                                            <div className="text-center py-8 text-slate-600 border border-dashed border-slate-800 rounded-xl">No hay historial registrado.</div>
                                        ) : (
                                            <div className="space-y-2 max-h-[300px] overflow-y-auto pr-2 custom-scrollbar">
                                                {history.map((record) => (
                                                    <div key={record.id} className="flex items-center justify-between p-3 bg-slate-800 rounded-lg border border-slate-700/50">
                                                            ...
                                                            <div>
                                                                <div className="text-sm font-bold text-white flex items-center gap-2">
                                                                    ...
                                                                    {new Date(record.effectiveDate) > new Date() && (
                                                                        <span className="text-[10px] bg-blue-500/20 text-blue-400 px-1.5 py-0.5 rounded border border-blue-500/30">FUTURO</span>
                                                                    )}
                                                                </div>
                                                                <div className="text-[10px] text-slate-500">Definido el {new Date(record.createdAt).toLocaleDateString()}</div>
                                                            </div>
                                                        </div>
                                                        <div className="text-right">
                                                            <div className="text-white font-medium text-sm">${Number(record.baseValue).toLocaleString()}</div>
                                                            <div className="text-slate-500 text-xs">+${Number(record.extraHourValue).toLocaleString()}/h</div>
                                                        </div>
                                                    </div>
                                                ))}
                                            </div>
