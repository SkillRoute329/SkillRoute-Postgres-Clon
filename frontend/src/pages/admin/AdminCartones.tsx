...
const AdminCartones = () => {
  ...
  const loadFromSaved = (saved: any) => {
    ...
    if (uiData.line === 'UNKNOWN' || !uiData.line) {
      ...
      if (uiData.headers.length === 0) {
        ...
        if (archetype) {
          uiData.headers = archetype.headers.map((LOC: string, i: number) => ({
            id: `h${i}`,
            location: LOC,
            isStop: true,
          }));
  ...
  // FILTERS
  const [selectedSeason, setSelectedSeason] = useState('VERANO_2026');
  const [selectedDayType, setSelectedDayType] = useState('HABIL');
  ...
  }, [selectedSeason, selectedDayType]);

  const loadSavedCartons = async () => {
    setLoading(true);
    try {
      const data = await CartonService.getAll(selectedSeason, selectedDayType);
        ...
        <div className="flex flex-col">
          ...
          <div className="flex items-center gap-4 mt-2">
            <div className="flex items-center gap-2 bg-slate-800 p-1.5 rounded-lg border border-slate-700">
              ...
              <button
                onClick={() => setSelectedSeason('INVIERNO_2026')}
                className={clsx(
                  'px-3 py-1 rounded-md text-xs font-bold transition-all flex items-center gap-1.5',
                  selectedSeason.startsWith('INVIERNO')
            ...
            <div className="flex items-center gap-2 bg-slate-800 p-1.5 rounded-lg border border-slate-700">
              {['HABIL', 'SABADO', 'DOMINGO'].map((type) => (
                <button
                  key={type}
                  onClick={() => setSelectedDayType(type)}
                  className={clsx(
                    'px-3 py-1 rounded-md text-[10px] font-bold transition-all flex items-center gap-1',
      ...
      <div className="flex flex-col lg:flex-row gap-6 h-[calc(100vh-140px)]">
        ...
        <div className="w-full lg:w-80 shrink-0 bg-slate-900 border border-slate-800 rounded-2xl overflow-hidden flex flex-col">
          <div className="p-4 border-b border-slate-800 bg-slate-900 sticky top-0 z-10">
            <h3 className="font-bold text-slate-300 uppercase text-xs tracking-wider mb-2">
              Explorador de Flota
            </h3>
          ...
          <div className="flex-1 overflow-y-auto custom-scrollbar p-2 space-y-1">
            ...
            {(() => {
              ...
              savedCartons
                .slice()
                .sort((a: any, b: any) => parseInt(a.serviceNumber) - parseInt(b.serviceNumber))
                .forEach((s) => {
              ...
              <div className="flex gap-2">
                <button
                  onClick={async () => {
                    if (
                      !confirm(
                        'ATENCIÓN: Esto forzará la reparación de la base de datos maestra. ¿Continuar?',
                      )
                    )
                      return;
                    setLoading(true);
