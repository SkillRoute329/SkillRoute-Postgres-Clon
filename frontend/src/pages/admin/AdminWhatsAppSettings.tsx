...
const AdminWhatsAppSettings = () => {
  ...
  const handleRestart = async (clean = false) => {
    if (
      !confirm(
        clean
          ? '¿REINICIO TOTAL? Esto borrará la sesión actual y requerirá escanear de nuevo.'
          : '¿Estás seguro de que deseas reiniciar el servicio de WhatsApp?',
      )
    )
      return;
    setLoading(true);
      ...
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-slate-400">
            Escanea el código QR para vincular el bot y permitir el envío automático de mensajes.
          </p>
        </div>
