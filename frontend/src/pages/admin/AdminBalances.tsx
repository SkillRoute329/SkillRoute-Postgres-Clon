...
const AdminBalances = () => {
    ...
    if (error) {
        return (
            <div className="p-8 text-center">
                ...
                <button onClick={loadData} className="px-4 py-2 bg-slate-800 text-white rounded hover:bg-slate-700">Reintentar</button>
            </div>
        );
    }
    ...
    const handlePayBalance = async () => {
        ...
        if (!confirm(`¿Confirmas que ${actionText} el SALDO RESTANTE de $${absBalance}? \n\nEsta acción dejará el balance en $0.`)) return;
        ...
        try {
            ...
            if (res.ok) {
                const successMsg = balance > 0
                    ? 'Pago final registrado. Balance en $0.'
                    : 'Deuda saldada. Balance en $0.';
        ...
        <div className="space-y-6 animate-fade-in-up">
                    ...
                    <div className="glass-panel border border-slate-700 rounded-2xl w-full max-w-2xl max-h-[90vh] flex flex-col">
                        <div className="p-6 border-b border-slate-700 flex justify-between items-center">
                            <div>
                                ...
                                <p className="text-slate-400">Usuario: {selectedUserForPayout.firstName} {selectedUserForPayout.lastName} (#{selectedUserForPayout.internalNumber})</p>
                            </div>
                            <button onClick={() => setSelectedUserForPayout(null)} className="text-slate-400 hover:text-white"><ExternalLink className="rotate-45" /></button>
                        </div>
                            ...
                            {loadingPayout ? (
                                ...
                                <>
                                    <div className="bg-red-500/10 border border-red-500/30 rounded-xl p-4 text-center mb-6">
                                        ...
                                        <div className="text-3xl font-black text-white">${Math.abs(Number(selectedUserForPayout.balance)).toLocaleString()}</div>
                                    </div>

                                    <h3 className="font-bold text-white mb-2">Historial de Viajes Pendientes</h3>
                                    <div className="space-y-2">
                                        {payoutShifts.map(s => (
                                            <div key={s.id} className="bg-slate-800/50 p-3 rounded-lg border border-slate-700 flex justify-between items-center">
                                                <div>
                                                    <div className="text-white font-medium">{s.date.split('T')[0]} - {s.time}</div>
                                                    <div className="text-xs text-slate-400">Línea {s.line} / Coche {s.carNumber}</div>
                                                    <div className="text-xs text-slate-500">Creado por: {s.creatorFirstName} {s.creatorLastName}</div>
                                                </div>
                                        ...
                                        {payoutShifts.length === 0 && <div className="text-slate-500 text-center">No hay viajes pendientes.</div>}
                                    </div>
                                </>
                            )}
                        </div>
                        ...
                        <div className="p-4 md:p-6 border-t border-slate-700 bg-slate-900/50 rounded-b-2xl space-y-4">
                            ...
                            <div className="flex flex-col sm:flex-row gap-3 items-stretch sm:items-center">
                                <div className="relative flex-1">
                                    ...
                                    <input
                                        type="number"
                                        placeholder={Number(selectedUserForPayout.balance) < 0 ? "Monto a Cobrar..." : "Monto a Pagar..."}
                            ...
                            <div className="flex flex-col-reverse sm:flex-row justify-end gap-3 pt-4 border-t border-slate-800/50">
                                ...
                                >
                                    ...
                                    <span className="truncate max-w-[200px] sm:max-w-none">
                                        {Number(selectedUserForPayout.balance) < 0 ? "Saldar Deuda Total" : "Saldar Pago Total"}
                                    </span>
                                    <span className="opacity-80 ml-1">(${Math.abs(Number(selectedUserForPayout.balance)).toLocaleString()})</span>
                                </button>
                            </div>
                        </div>
                    </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                <div className="glass-panel p-4 rounded-xl border border-slate-800">
                    ...
                    <div className="text-xl font-bold text-white">${globals.totalCedidos.toLocaleString('es-AR', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</div>
                </div>
                <div className="glass-panel p-4 rounded-xl border border-slate-800">
                    <div className="flex items-center gap-2 text-slate-500 mb-1">
                    ...
                    <div className="text-xl font-bold text-white">${globals.totalTomados.toLocaleString('es-AR', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</div>
                </div>
                ...
                <div className={clsx(
                    ...
                    <div className="text-2xl font-black text-white">$ {balanceGlobal.toLocaleString('es-AR', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</div>
                </div>
            </div>
                        ...
                        <div className="relative z-10 flex flex-col md:flex-row justify-between items-center gap-6">
                            <div className="w-full md:w-auto text-center md:text-left">
                                <div className="flex items-center justify-center md:justify-start gap-3 mb-1">
                                    ...
                                    <span className="text-sm text-slate-400 font-bold uppercase tracking-wider">Cuenta Corriente</span>
                                </div>
                                <h2 className="text-2xl md:text-3xl font-black text-white capitalize">{activeUserCard.lastName} {activeUserCard.firstName}</h2>
                                <div className="text-primary-400 font-bold text-lg">Interno #{activeUserCard.internalNumber}</div>
                            </div>
                            ...
                            <div className="text-center md:text-right w-full md:w-auto bg-slate-800/50 md:bg-transparent p-4 md:p-0 rounded-2xl md:rounded-none">
                                <div className="text-xs text-slate-400 font-bold uppercase mb-1">Balance Actual</div>
                                <div className={clsx(
                                    "text-4xl font-black mb-2",
                                    Number(activeUserCard.balance) >= 0 ? "text-emerald-400" : "text-red-400"
                                )}>
                                    {Number(activeUserCard.balance) >= 0 ? '+' : ''}
                                    ${Number(activeUserCard.balance).toLocaleString('es-AR', { minimumFractionDigits: 2 })}
                                </div>
                                <div className="text-xs text-slate-500">
                                    {Number(activeUserCard.balance) >= 0 ? "A favor del chofer" : "Deuda con la administración"}
                                </div>
                            </div>
                        ...
                        <tbody className="divide-y divide-slate-800">
                            {filteredUsers.length > 0 ? (
                                filteredUsers.map((user: any) => {
                                    ...
                                    return (
                                        <tr key={user.internalNumber} className="hover:bg-white/5 transition-colors group">
                                            <td className="px-4 py-4 font-bold text-primary-400">#{user.internalNumber}</td>
                                            <td className="px-4 py-4 text-white capitalize">{user.lastName} {user.firstName}</td>
                                            <td className="px-4 py-4 text-right text-red-300/80 font-medium">${Number(user.cedidos).toLocaleString('es-AR', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</td>
                                            <td className="px-4 py-4 text-right text-emerald-300/80 font-medium">${Number(user.tomados).toLocaleString('es-AR', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</td>
                                            <td className={clsx(
                                                "px-4 py-4 text-right font-black text-sm",
                                                balance >= 0 ? "text-emerald-400" : "text-red-400"
                                            )}>
                                                {balance >= 0 ? '+' : ''}${balance.toLocaleString('es-AR', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                                            </td>
                                            <td className="px-4 py-4 text-right">
                                                <td className="px-4 py-4 text-right">
                                                    {/* Button removed. Use search to view detail/pay */}
                                                    <button className="p-2 text-slate-600 cursor-default" disabled title="Use el buscador para ver detalles">
                                                        -
                                                    </button>
                                                </td>
                                            </td>
                ...
                <div className="md:hidden space-y-3">
                    {filteredUsers.length > 0 ? (
                        filteredUsers.map((user: any) => {
                            const balance = Number(user.balance);
                            return (
                                <div key={user.internalNumber} className="glass-panel p-4 rounded-xl border border-slate-800 relative">
                                    <div className="flex justify-between items-start mb-3">
                                        <div>
                                            <span className="text-primary-400 font-bold text-sm">#{user.internalNumber}</span>
                                            <div className="text-white font-bold capitalize">{user.lastName} {user.firstName}</div>
                                        </div>
                                        <div className={clsx(
                                            "font-black text-lg",
                                            balance >= 0 ? "text-emerald-400" : "text-red-400"
                                    ...
                                    <div className="grid grid-cols-2 gap-2 text-xs">
                                        <div className="bg-red-900/10 border border-red-500/20 rounded-lg p-2 text-center">
                                            <div className="text-red-400/70 uppercase font-bold tracking-wider mb-1">Cedidos</div>
                                            <div className="text-white font-bold">${Number(user.cedidos).toLocaleString('es-AR')}</div>
                                        </div>
                                        <div className="bg-emerald-900/10 border border-emerald-500/20 rounded-lg p-2 text-center">
                                            <div className="text-emerald-400/70 uppercase font-bold tracking-wider mb-1">Realizados</div>
                                            <div className="text-white font-bold">${Number(user.tomados).toLocaleString('es-AR')}</div>
                                        </div>
                                    </div>
                                </div>
                            );
