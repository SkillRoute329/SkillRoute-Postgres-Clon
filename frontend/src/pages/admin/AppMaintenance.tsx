...
const AppMaintenance = () => {
  ...
  const handleUpdate = async () => {
    if (
      !confirm(
        '¿Desea sincronizar el sistema con los últimos cambios de código? El servidor puede reiniciarse.',
      )
    )
      return;
    setUpdating(true);
    try {
      ...
      alert(
        'Actualización Iniciada:\n' + ((res as { ok?: boolean; message?: string }).message || 'OK'),
      );
      loadData();
    } catch (e: any) {
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
        <div>
          ...
          <p className="text-slate-400 mt-1">
            Super Admin Dashboard - Diagnóstico de Salud y Actualización 1-Click
          </p>
        </div>
        <button
          ...
          className={clsx(
            ...
            updating
              ...
              : 'bg-primary-600 hover:bg-primary-500 text-white animate-pulse-slow',
          )}
        >
          ...
          <div className="space-y-3">
            ...
            <div className="grid grid-cols-2 gap-2 mt-4">
              ...
              <div className="bg-slate-950 p-3 rounded-2xl border border-slate-800">
                ...
                <span className="text-xl font-black text-white">OK</span>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="bg-slate-900 border border-slate-800 rounded-3xl p-6 relative overflow-hidden group">
          ...
          <div className="space-y-4">
            <div className="flex items-center gap-3 text-sm text-slate-300">
              ...
              <span>Auth JWT: OK</span>
            </div>
            <div className="flex items-center gap-3 text-sm text-slate-300">
              <CheckCircle2 className="w-4 h-4 text-emerald-500" />
              <span>Storage Rules: OK</span>
            </div>
            <div className="flex items-center gap-3 text-sm text-slate-300">
              <CheckCircle2 className="w-4 h-4 text-emerald-500" />
              <span>Real-time Listeners: ON</span>
            </div>
          </div>
        </div>
          ...
          <div className="space-y-3">
            <div className="bg-red-500/5 border border-red-500/10 p-4 rounded-2xl">
              ...
              <p className="text-slate-500 text-xs text-justify">
                La mayoría de errores de importación se deben a columnas mal nombradas. El sistema
                espera nombres exactos: <strong>Legajo, CI, Nombre</strong>. Si el error persiste,
                verifique que el archivo no supere los 10MB para optimización de túnel.
              </p>
            </div>
