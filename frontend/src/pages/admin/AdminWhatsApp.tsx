            ...
            <div className="glass-panel p-6 rounded-2xl border border-slate-700">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                    {filteredUsers.map(user => (
                        <div key={user.id} className="bg-slate-800/50 p-4 rounded-xl border border-slate-700 flex justify-between items-center group hover:border-slate-600 transition-colors">
                            <div>
                                <div className="font-bold text-white">{user.fullName}</div>
                                <div className="text-xs text-slate-400">Interno: {user.internalNumber}</div>
                                <div className="text-xs text-slate-500 mt-1">{user.phoneNumber || 'Sin celular'}</div>
                            </div>
                            <button
