...
const AdminShifts = () => {
  ...
  const handleAction = async (action: string, id: number | string) => {
    ...
    if (typeof id === 'string') {
      if (action === 'assign' || action === 'edit' || action === 'approve') {
        ...
        alert(
          'Este es un turno programado por rotación. Para modificarlo, primero guárdelo como turno real.',
        );
        return;
      }
      return;
    ...
    if (action === 'approve') {
      if (shift.transformaFacil) {
        ...
        if (discount !== null) {
          ...
          shift.totalValue =
            Number(shift.totalValue ?? 0) - Number(shift.transformaFacilDiscount ?? 0); // Apply discount to total
        }
      }
  ...
  const filteredShifts = shifts.filter((s) => {
    ...
    const matchesSearch =
      String(sn.serviceNumber ?? '')
        .toLowerCase()
        .includes(search) ||
      String(sn.carNumber ?? '')
        .toLowerCase()
        .includes(search) ||
      String(sn.assigneeName ?? '')
        .toLowerCase()
        .includes(search) ||
      String(sn.assigneeInternalNumber ?? '').includes(search) ||
      String(sn.creatorName ?? '')
        .toLowerCase()
        .includes(search) ||
      String(sn.line ?? '')
        .toLowerCase()
  ...
  const TabButton = ({ name, label, count }: { name: string; label: string; count: number }) => (
    <button
      ...
      className={clsx(
        ...
        activeTab === name ? 'text-primary-400' : 'text-slate-400 hover:text-white',
      )}
    >
      <span>{label}</span>
      <span
        ...
        if (timeSinceLast >= downloadInterval) {
          ...
          try {
            ...
            const todaysShifts = freshData.filter(
              (s: any) => new Date(s.date).toLocaleDateString() === today,
            );
    ...
    <div className="space-y-6 animate-fade-in-up">
      ...
      <div className="flex flex-col xl:flex-row xl:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-slate-400">
            <span>Administra, aprueba y asigna turnos.</span>
          </p>
        </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm animate-fade-in">
          <div className="bg-slate-900 border border-slate-700 p-6 rounded-2xl w-full max-w-md shadow-2xl scale-100 animate-scale-in">
            <h3 className="text-xl font-bold text-white mb-2">
              <span>
                {confirmAction.type === 'reject' ? '¿Rechazar Turno?' : '¿Eliminar Turno?'}
              </span>
            </h3>
      ...
      <div className="border-b border-slate-800 flex gap-4 overflow-x-auto pb-2 custom-scrollbar">
        <TabButton
          name="Created"
          label="Pendientes"
          count={shifts.filter((s) => s.status === 'Created').length}
        />
        <TabButton
          name="Public"
          label="Públicos"
          count={shifts.filter((s) => s.status === 'Public').length}
        />
        <TabButton
          name="Assigned"
          label="Asignados"
          count={shifts.filter((s) => s.status === 'Assigned').length}
        />
      </div>
      ...
      {isLoading ? (
        <div className="py-20 flex flex-col items-center justify-center text-slate-500 animate-fade-in">
          ...
          <span className="text-sm font-medium text-slate-400 animate-pulse">
            Cargando turnos en tiempo real...
          </span>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredShifts.length > 0 ? (
            filteredShifts.map((shift) => (
              <ShiftCard key={shift.id} shift={shift} variant="admin" onAction={handleAction} />
            ))
          ) : (
      ...
      {assigningShiftId && (
        <AssignShiftModal
          ...
          currentAssigneeId={
            Number(shifts.find((s) => s.id === assigningShiftId)?.assignedTo ?? 0) as number
          }
        />
      )}
    </div>
