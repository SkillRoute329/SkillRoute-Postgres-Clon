...
export const TrendCharts: React.FC<TrendChartsProps & { direction?: 'ida' | 'vuelta' | 'global' }> = ({
  ...
  return (
    <div className={`flex flex-col xl:flex-row gap-4 min-h-min pb-1`}>
      {/* Línea Base */}
      ...
      {trends.competitor_line && (
        <div className="flex-1 bg-slate-800/50 rounded-xl border border-slate-700 p-3 flex flex-col">
          <div className="text-xs text-slate-400 mb-0.5">Impacto en Rival</div>
          <div className="text-xl font-bold text-white mb-2">
          ...
          {/* KPI PRINCIPAL COMPETIDOR */}
          <div className={`mt-auto bg-slate-900/80 rounded-lg p-3 border ${direction ? 'border-rose-500/30' : 'border-rose-500/30 shadow-sm'}`}>
            <div className="text-[10px] uppercase text-rose-400 font-bold mb-2">
              {direction ? `VOLUMEN PASAJEROS (${direction.toUpperCase()})` : 'CONSOLIDADO GLOBAL (IDA + VUELTA)'}
            </div>
            <div className="flex flex-col gap-2">
              {(() => {
