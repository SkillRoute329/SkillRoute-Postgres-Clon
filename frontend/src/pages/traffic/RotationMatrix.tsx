...
const RotationMatrix = () => {
    ...
    // Swap Modal State
    const [swapModal, setSwapModal] = useState<{ open: boolean; service: ServiceItem | null }>({ open: false, service: null });
    ...
    const openSwapModal = (service: ServiceItem) => {
        ...
        setSelectedVehicleId(service.assignedVehicle?.id.toString() || "");
    };
    ...
    const handleSwapSubmit = async () => {
        ...
        setSwapLoading(true);
        try {
            const vid = selectedVehicleId === "null" || selectedVehicleId === "" ? null : Number(selectedVehicleId);
            await CartonService.swapVehicle(swapModal.service.id, vid);
    ...
    const getAvailableVehicles = () => {
        ...
        const assignedIds = new Set(
            services
                .filter(s => s.assignedVehicle && s.id !== swapModal.service?.id)
                .map(s => s.assignedVehicle!.id)
        );
    ...
    return (
        <div className="space-y-6 animate-fade-in-up pb-24 relative">
            ...
            <div className="flex flex-col md:flex-row gap-4 items-start md:items-center justify-between">
                ...
                <div className="flex gap-2">
                    <div className="flex items-center gap-2 bg-slate-900/50 px-3 py-1.5 rounded-lg border border-slate-700">
                        ...
                        <select className="bg-transparent text-sm font-bold text-white focus:outline-none"
                            value={filterSeason || ''} onChange={(e) => setFilterSeason(e.target.value ? Number(e.target.value) : undefined)}>
                    ...
                    <div className="flex items-center gap-2 bg-slate-900/50 px-3 py-1.5 rounded-lg border border-slate-700">
                        ...
                        <select className="bg-transparent text-sm font-bold text-white focus:outline-none"
                            value={filterDayType} onChange={(e) => setFilterDayType(e.target.value)}>
            ...
            {/* --- ACCORDION LIST --- */}
            {loading ? (
                <div className="flex justify-center py-20"><div className="animate-spin rounded-full h-12 w-12 border-b-2 border-indigo-500"></div></div>
                    ...
                    {orderedGroups.map((type) => {
                        ...
                        return (
                            <div key={type} className="bg-slate-800/40 border border-slate-700 rounded-xl overflow-hidden shadow-lg">
                                <button onClick={() => toggleGroup(type)} className={clsx("w-full flex items-center justify-between p-4 transition-colors hover:bg-slate-700/50", isExpanded ? "bg-slate-700/30" : "")}>
                                    ...
                                    <div className="flex items-center gap-4">
                                        <div className="hidden md:flex flex-col items-end">
                                            <div className="w-32 h-1.5 bg-slate-700 rounded-full overflow-hidden">
                                                <div className={clsx("h-full transition-all duration-500", assignedPercentage === 100 ? "bg-emerald-500" : "bg-amber-500")} style={{ width: `${assignedPercentage}%` }}></div>
                                            </div>
                                            <span className={clsx("text-xs font-medium mt-1", assignedPercentage === 100 ? "text-emerald-400" : "text-amber-400")}>{assignedPercentage}% Asignado</span>
                                        </div>
                                        <div className="bg-indigo-500/20 text-indigo-300 px-3 py-1 rounded-lg font-bold text-sm min-w-[3rem]">{totalVars}</div>
                                    </div>
                                </button>
                                                ...
                                                {items.map((srv) => {
                                                    ...
                                                    return (
                                                        <tr key={srv.id} className={clsx("hover:bg-indigo-500/5 transition-colors", isBlank && "opacity-50 bg-slate-900/30")}>
                                                            ...
                                                            <td className="px-4 py-3 text-slate-300 whitespace-nowrap">{details.t2 || '-'}</td>
                                                            <td className="px-4 py-3 text-slate-300 whitespace-nowrap">{details.t3 || (details.note ? <span className="text-yellow-500/80 italic">{details.note}</span> : '-')}</td>
                                                            <td className="px-4 py-3">
                                                                {!isBlank && (
                                                                    srv.assignedVehicle ? (
                                                                        <button
                                                                        ...
                                                                        >
                                                                            <div className="flex items-center gap-2">
                                                                                ...
                                                                                <span className="font-bold text-sm">Coche {srv.assignedVehicle.internalNumber}</span>
                                                                            </div>
                                                                            {/* Swap Icon on Hover */}
                                                                            <span className="opacity-0 group-hover:opacity-100 transition-opacity text-xs bg-emerald-500 text-white px-1.5 rounded">⇄</span>
                                                                        </button>
                                                                    ) : (
                                                                        <button
                                                                            onClick={() => openSwapModal(srv)}
                                                                        ...
                                                                        >
                                                                            ...
                                                                            <span className="bg-slate-900 px-1.5 py-0.5 rounded text-[10px]">+</span>
                                                                        </button>
                                                                    )
                                                                )}
                                                            </td>
            ...
            {swapModal.open && swapModal.service && (
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-in fade-in duration-200">
                    <div className="bg-slate-800 border border-slate-700 rounded-2xl shadow-2xl w-full max-w-md p-6 relative animate-in zoom-in-95 duration-200">
                        <button onClick={closeSwapModal} className="absolute top-4 right-4 text-slate-400 hover:text-white">✕</button>

                        <div className="mb-6">
                            <h2 className="text-xl font-bold text-white mb-1">Sustitución de Unidad</h2>
                            <p className="text-slate-400 text-sm">
                                Servicio <span className="text-white font-mono font-bold">{swapModal.service.serviceCode}</span> •
                        ...
                        <div className="space-y-4">
                            <div className="p-4 bg-slate-700/50 rounded-xl border border-slate-600">
                                <label className="block text-xs uppercase tracking-wider text-slate-400 font-bold mb-2">Unidad Actual</label>
                                {swapModal.service.assignedVehicle ? (
                                    <div className="flex items-center gap-3 text-emerald-400 font-bold text-lg">
                                        <Bus className="w-6 h-6" />
                                        Coche {swapModal.service.assignedVehicle.internalNumber}
                                    </div>
                                ) : (
                                    <div className="text-slate-500 italic flex items-center gap-2">
                                        <div className="w-6 h-6 rounded-full border border-dashed border-slate-500 flex items-center justify-center text-xs">?</div>
                                        Sin asignación
                                    </div>
                                )}
                            </div>
                            ...
                            <div>
                                <label className="block text-xs uppercase tracking-wider text-slate-400 font-bold mb-2">Nueva Unidad (Reemplazo)</label>
                                <select
                                ...
                                >
                                    <option value="" disabled>Seleccione un coche disponible...</option>
                                    <option value="null" className="text-red-400 font-bold">❌ Desvincular (Dejar vacío)</option>
