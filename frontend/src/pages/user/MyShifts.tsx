    ...
    <div className="space-y-6 animate-fade-in-up relative">
      ...
      <div>
        ...
        <p className="text-slate-400">
          <span>Historial de turnos asignados y realizados</span>
        </p>
      </div>
      ...
      {shifts.length === 0 ? (
        <div className="text-center py-20 bg-slate-800/30 rounded-2xl border border-slate-800 dashed border-2">
          ...
          <h3 className="text-xl font-bold text-slate-400">
            <span>No tienes turnos asignados</span>
          </h3>
          <p className="text-slate-500 mt-2">
            <span>Los turnos que tomes aparecerán aquí.</span>
          </p>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {shifts.map((shift) => (
            <ShiftCard key={shift.id} shift={shift} onAction={handleAction} />
          ))}
        </div>
      )}
    </div>
