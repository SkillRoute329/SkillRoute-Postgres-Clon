...
import type { Shift, User, Vehicle, ServiceItem } from '../../services/firestore/types';
...
const Distribution = () => {
  ...
  const handleGenerateDailyShifts = async () => {
    if (
      !confirm(
        "Se generarán los servicios REALES para el día de hoy basados en la Matriz 'VERANO 2026' (HÁBIL). ¿Continuar?",
      )
    )
      return;
  ...
  const coveragePercent =
    shifts.length > 0
      ? Math.round(((shifts.length - pendingShifts.length) / shifts.length) * 100)
      : 0;

  const displayDrivers =
    activeTab === 'all' ? drivers : activeTab === 'assigned' ? assignedDrivers : availableDrivers;
    ...
    <div className="space-y-6 animate-fade-in-up pb-24">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          ...
          <p className="text-slate-400 text-sm">
            Gestión Integral de Transporte • Distribución de Flota
          </p>
        </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        <div className="bg-slate-800/50 p-4 rounded-xl border border-slate-700">
          ...
          <div
            className={clsx(
              'text-2xl font-bold text-center mt-1',
              coveragePercent === 100 ? 'text-emerald-400' : 'text-amber-400',
            )}
          >
            {coveragePercent}%
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                    ...
                    {activeShift ? (
                      <div className="text-right">
                        ...
                        <div className="text-[10px] text-slate-500">
                          Serv {activeShift.serviceNumber}
                        </div>
                      </div>
                    ) : (
        ...
        <div className="space-y-4">
          <div
            className={clsx(
              'bg-slate-900 border rounded-2xl p-4 sticky top-4 transition-colors',
              pendingShifts.length > 0 ? 'border-amber-500/30 bg-amber-500/5' : 'border-slate-800',
            )}
          >
            <h3
              className={clsx(
                'font-bold mb-4 flex items-center gap-2',
                pendingShifts.length > 0 ? 'text-amber-400' : 'text-white',
              )}
            >
            ...
            {pendingShifts.length === 0 ? (
              ...
              <div className="space-y-3">
                {pendingShifts.map((shift) => (
                  <div
                    key={shift.id}
                    className="bg-slate-900 border border-slate-700 rounded-xl p-3 relative overflow-hidden group hover:border-amber-500/50 transition-colors"
                  >
